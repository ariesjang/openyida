...
- **表单延续应用设计风格**：设计与表单技能明确提交、编辑和详情页共同遵循 `design.md` 的应用风格，统一字体、材质、控件、分组和留白；根据业务选择单列、多列、主次列宽及标签位置，通过原生 Schema 实现布局，避免所有主题采用同一种白卡双列。
- **样式统一进入应用主题**：表单控件、背景、交互状态、详情表面和底栏规则归入同一份应用主题 CSS，禁止在表单或详情加载代码、动作模块和 iframe 脚本中注入主题。补充底栏背景与阴影随主题设计、主体与底栏对齐、窄屏及长表单验收规则。
- **补齐原生表单主题变量**：主题模板增加控件圆角、高度、字号、填充、边框和标签间距变量；新版无导航提交页支持消费应用外衬颜色与背景图，独立页面与抽屉 iframe 使用同一应用主题资源。
- **验证主题生成与增量保留**：补充 Fast、Plan 表单主题生成测试，检查变量不会被重复声明覆盖，调整表单密度时保留背景、品牌色和无关定制样式。

### 升级说明

- 已发布应用不会自动迁移。按新规则更新设计源并生成、上传应用主题；已有主题缺少提交页背景规则时，核实实际 DOM 后增量补齐，不重置整份 CSS。历史页面注入需在明确迁移范围后单独清理并验证。
