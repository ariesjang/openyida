...
- **模板配色按角色检查**：弱化波普大面积明黄和票据橙棕底，正文与辅助文字独立分级，修复可变主色导致导航状态难读、搜索误用水印色等问题。33 套命名主题与自由创意入口均纳入七组主色的配色回归；Canvas 链接在保留品牌填充的同时使用可读前景。
- **固定主题只维护一套配色**：全局和平台各导航模式共用一份 token 声明，避免浅色模式覆盖深色项目导航；更新旧主题时合并重复声明，保留其他自定义样式。`navTheme` 保留为平台配置，不再生成另一套颜色。午夜霓虹模板同时减弱菜单选中底色，将正文与辅助文字调整为分级灰白，青色用于品牌和交互强调。
- **整套应用共用设计源**：导航、应用框架、表单、详情和自定义页面统一消费 `design.md` 定义的应用主题，不再单独选择导航风格或按主色替换整套导航配色。33 套命名主题与自由创意入口的设计说明、CSS 模板及 Skill 同步调整。
- **默认导航更克制**：普通菜单采用连续列表，减少逐项边框、大圆角和选中投影。额外边框、阴影仅在设计明确声明时生成，省略时沿用平台表现；保留悬停文字、选中状态和展开层级线的主题配置。仅设置阴影时不再额外改写菜单高度和布局。
- **更新主题时保留项目定制**：移除设计中的可选导航边框或阴影后，同步移除对应自动生成规则，保留生成区域之外的自定义 CSS。
- **PC 与移动端品牌变量完整校验**：主题生成和上传检查 PC 七档品牌色阶及移动端四档品牌变量在全局 `:root` 中是否完整、可用，拒绝空值、无效颜色、循环引用和未展开的模板占位说明。移动端变量由 CLI 按应用色阶桥接，避免只替换主色后上传不可用主题。

### 校验诊断与修复流程

- **主题配色校验统一入口**：`check-design`、Plan 物化和使用完整 `design.md` 的主题生成共用 26 组文字与背景检查，支持透明颜色合成，要求对比度至少 4.5:1。已知不合格组合返回 `DESIGN_THEME_CONTRAST_LOW`，附带角色、token 和对比度；需修改设计源后重新生成，CLI 不自动改色，也不新增导航样式参数。命令清单、Skill 和相关测试同步对齐。
- **区分静态通过与视觉验收**：JSON 中的 `paletteContrast.unresolved` 列出无法计算的组合，不代表已通过；图片、渐变、局部样式和真实页面仍需浏览器检查。禁用文字、边框与装饰色不按正文对比度检查。
- **减少无进展的重复校验**：设计方案、设计文档及 Canvas 编译失败时补充源文件位置、输入指纹和错误指纹，便于 agent 集中修正源文件。Skill 统一初始化和后续校验的停止规则：输入未修改时不重复执行，连续两次修复仍返回相同错误集合时停止并说明原因；CLI 不保存跨任务重试计数。
- **一次返回更多可修复问题**：设计文档校验汇总问题及修复提示，Plan 指引拆分为补齐业务事实、补齐视觉事实、标记就绪、生成产物和展示确认，减少遗漏步骤。业务完整性、主题 Provider 和抽屉结构校验继续保留。
- **命令结果何时复用更明确**：命令清单补充 `design-plan materialize`、`configure-process` 和 `report inspect` 的结果复用契约。分别说明本地输入变化后重新生成、流程发布前回读并确认新更新，以及观察到远端修改后重新查询，避免把流程发布误当作可直接重放的操作。

### Canvas 主题与发布

- **主题迁移仅由显式参数触发**：`publish --fix-theme` 才改写固定品牌色，发布过程不再等待终端确认。迁移前确认控件位于已装配、已挂载的 `CanvasThemeProvider` 内，完成迁移与全部 Canvas 编译检查后才写回源码；检查失败保留原文件。
- **迁移保留周边源码与样式**：按语法 token 处理品牌色属性及分隔符，兼容属性旁的注释，保留其他 CSS、动态主题 Provider 和抽屉实现。
- **固定品牌色告警含义更明确**：默认保留覆盖并在普通输出及 JSON 中返回告警，不代表品牌色已跟随应用主题。`--strict-theme` 用于阻断固定色，`--allow-fixed-brand` 用于明确保留覆盖；CLI、命令文档和 Skill 同步说明参数用途及互斥关系。

### 应用入口与 Skill

- **保留业务后台完整页面地址**：登记后台入口时不再将 `/workbench/<页面ID>` 截短为工作台根路径；`--management-page` 生成包含页面 ID 的入口。`--frontend-page` 仅接受已发布的自定义页面，完整 URL 仍按地址参数处理。
- **发布技能更直白**：说明何时使用、所需输入和完成结果，补齐主题参数的命令示例及参数表，统一失败后的修复步骤。

### CLI 稳定性与发布检查

- **修复管道中的错误 JSON 被截断**：CLI 失败时设置退出码并等待输出排空，避免立即退出导致较长诊断只输出一部分，修复 macOS / Node 20 CI 的 JSON 解析失败；新增超过管道缓冲区的中文长诊断回归，验证内容完整且保留原退出码。
- **同步主题改造后的文件数预算**：新增 3 个主题运行时模块、移除 2 个旧样式清单后，发布包文件数上限由 607 调整为 608，修复 CI 的文件数超限错误；补充必需模块及旧清单排除检查，压缩包、解压体积和单文件大小限制保持不变。

### 升级说明

- 已发布应用的主题不会自动更新。采用新版导航默认样式需调整项目 `design.md`、重新生成并上传应用主题；历史 CSS 上传前需补齐 PC 与移动端品牌变量，并检查导航、表单、详情及移动端效果。
- 已有 Canvas 页面要获得新的链接前景配色，需更新页面内的 `CanvasThemeProvider` 并重新编译、发布；仅上传应用主题 CSS 不会更新嵌入的 Provider 代码。
- 已发布页面不会自动迁移。需要让固定品牌色跟随应用主题时，确认主题 Provider 后显式执行 `publish --fix-theme`，并检查实际使用的主题、主操作和抽屉效果。
- 历史后台入口若已被保存为工作台根地址，需要重新登记目标页面的完整 URL。

## [2026.9.22-1] - 2026-09-22

### 设计文档校验

- **`check-design` 锚点与页面八项要点校验返回可执行提示**：当设计文档缺少有效自闭合 HTML 锚点、锚点未落在第 3/5 章、页面锚点未按 `page-` 前缀命名，或八项要点未按格式写出时，校验错误会附带 `nextStep` 修复指引，说明正确写法（如 `<a id="component-button"></a>`、`#page-` 前缀、八项要点列表格式），帮助模型和开发者按相同标准自纠。同步更新 en/zh 及 11 个翻译包文案，设计技能文档新增最小对照示例。

## [2026.9.22] - 2026-09-22

### 自定义页面主题守卫

- **写死品牌色默认不再阻断发布**：自定义页面在 `ConfigProvider` 里写死品牌交互色（如 `colorPrimary: '#1677ff'`）时，编译期守卫由硬失败改为告警后继续发布，页面运行时仍由应用主题接管，避免历史页面卡在发布环节。需要在 CI 强制拦截的团队可加 `--strict-theme` 或设置 `OPENYIDA_CANVAS_STRICT_THEME=1` 恢复硬失败。
- **一键迁移写死的品牌色**：`publish` 新增 `--fix-theme`，自动移除源码中写死的品牌交互色覆盖，交由应用主题解析（un-pin 迁移）。交互终端下会先提示待迁移项并询问确认再改写源码；非交互（agent/CI/管道）环境不会挂起等待，默认跳过并提示改用 `--fix-theme` 显式迁移。
- **保留按原样发布的显式旁路**：`--allow-fixed-brand` 或 `OPENYIDA_CANVAS_ALLOW_FIXED_BRAND=1` 可按原样发布写死品牌色并给出告警，便于旧页面先上线再迁移；该旁路将在后续版本移除。

### 发布安全与稳定性

- **修复非交互环境发布卡住**：迁移确认改用真实 TTY 判定，agent、CI 与管道运行不再因无人应答的交互提问而挂起。
- **平台安全（WAF）误拦的临时规避指引**：新增 `waf-safe-authoring.md` 参考文档并接入 `yida-canvas-custom-page`、`yida-publish-page` 技能。仅当发布返回 HTTP 200 但响应体非 JSON 且包含 `waf_block` 时判定为安全拦截，指导用二分法定位命中片段，并给出不写正则字面量、危险 HTML/脚本串、注入特征串等规避写法；常规生成不套用，接口加白后可下线。

### 应用访问入口

- **收尾按需登记访问入口的说明更完整**：`yida-app` 搭建收尾补充「登记应用访问入口」步骤，明确前台与业务工作台是两个独立可选入口，按本轮实际场景与回读证据决定是否登记，不因 CLI 返回 `appUrl`/`workbenchUrl` 就自动登记，不登记 `/admin` 开发后台，不为补齐配置而开启公开访问。

### 升级说明

- 已发布页面保持原样。写死品牌色的自定义页面建议重新执行 `publish --fix-theme` 迁移到应用主题解析，或手动迁移到 canvas-theme；需要沿用旧行为可临时加 `--allow-fixed-brand`。启用 CI 主题强制拦截请显式加 `--strict-theme` 或设置对应环境变量。

## [2026.9.21] - 2026-09-21

### 应用入口与导航

- **按需登记应用访问入口**：新增 `app-entry get/set`，可单独登记前台或业务工作台，也可同时登记两类入口；应用列表保留已登记的入口信息。支持完整页面地址、公开地址和短链接，保留有效参数，方便后续从应用卡片进入实际业务页面（需平台支持入口配置接口）。
- **更新入口时保留已有配置**：只修改本次指定的入口，移除入口需显式操作；保存前检查配置版本，保存后重新读取核对。搭建收尾只登记已经发布并验证的入口，不自动补造业务后台，也不会为了登记而开启公开访问。
- **按实际用法安排前后台**：搭建方案明确区分“不分前后台”“分前后台”“只有访问前台”和“只有访问后台”，分别说明谁使用、默认打开什么页面、有哪些菜单和操作。业务后台用于处理订单、工单等日常任务，应用开发配置后台单独提供入口；只搭建前台时，会明确提交后的业务由哪里承接。
- **主题之间的导航差异更鲜明**：导航与应用框架、表单、自定义页和详情页一起设计，菜单可呈现方角粗框、圆润胶囊、票据双线、终端虚线等外观。波普印刷、北欧云白、复古票据、终端像素四套案例的导航样式已纳入公共模板，后续搭建可以直接复用。
- **当前菜单更容易识别**：顶部和侧边菜单支持普通、悬停、选中状态的边框，以及可独立配置的选中阴影；圆角、菜单高度和间距共同体现风格。自定义导航同步读取主题中的尺寸和留白，顶部菜单增加长名称截断与阴影展示空间。

### 主题与页面一致性

- **所有主题都能完整导出**：33 套命名主题和自由创意入口统一提供设计文档、应用主题 CSS、表单布局三个文件，可通过 `application-style` 导出后调整。历史方案引用的旧主题路径继续兼容。
- **品牌色由项目决定**：主题模板保留品牌主色和悬停色占位，生成应用主题时再按项目主色计算，方便将同一套风格用于不同品牌。
- **设计方案与生成结果更一致**：Plan 沿用所选主题的导航明暗和配色，改变品牌色时保留主题的导航特征；菜单布局仍按业务需要安排。项目调整的边框、阴影、内距和顶栏尺寸会体现在生成的设计说明与主题 CSS 中；自由创意可单独设计导航明暗。
- **抽屉文字与外壳配色协调**：表单抽屉的标题、正文和操作按钮跟随应用框架文字颜色，改善导航与内容采用不同明暗时的可读性；抽屉外壳沿用应用框架背景，内部表单保持自身页面主题。

### 公开表单与免登录提交

- **公开页面的提交指引更完整**：补充自定义页面在免登录状态下向普通表单提交数据的配置与实现说明，明确页面和目标表单都需开启公开访问，并配置匿名提交权限。新增登录与匿名状态的处理示例，帮助避免公开页仍调用登录接口而提交失败。
- **提交结果与适用范围更清楚**：指引要求提交后读取新记录并核对字段值，避免只看“成功”提示却留下空记录；同时说明匿名提交不支持发起流程、部分组织与关联字段，以及唯一性校验等限制，便于按业务需要选择匿名或登录页面。

### 配置可靠性与升级说明

- **导航设置保存后再次核实**：修改导航明暗、布局、显示状态或 Logo 来源后，CLI 会读取实际保存结果；未确认生效时明确报错，减少命令显示成功但页面配置未更新的情况。
- **命令参数错误更容易定位**：`design-plan`、`sample` 和 `update-app` 对未知参数、缺少值及不支持的参数组合给出明确提示，帮助及时修正脚本和生成命令。
- **已有应用按需升级**：更新 CLI 后，已发布页面保持原样。应用新的导航外观需要更新设计、重新生成并上传应用主题；采用自定义导航或抽屉的页面，还需合入新版模板并重新发布。升级后请检查业务访问页中的导航、表单、详情和移动端效果。
