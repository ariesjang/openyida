...
- **介绍区按任务选择**：应用风格模板不再预置介绍栏与占位标题；杂志风、行政风的表单也先按业务、说明价值和抽屉实际宽度选择直接填写、顶部说明、就近提示或侧栏。保留侧栏能力，避免重复标题、空泛说明和压缩字段空间。

- **18 套完整应用风格与自由创意**：从表单实验延展出应用级 `design.md`、`app_theme.css` 和原生 `form-layout.json`，覆盖杂志、墨金行政、铂金商务、票据、工单等方向，明确导航、自定义页面、表单和详情的共同材质、布局、密度及交互规则。介绍栏与章节通过原生分栏和 Divider 标题/说明表达，不注入页面代码。
- **模板消费与独立创作入口**：目录区分模板与 `creativeOption`，新增 `sample yida-design application-style --style-id ... --output <目录>` 导出成对资产；Fast 与 Plan 复用应用 CSS 配方。自由创意按业务独立决定构图、字体、材质和表单布局，缺少决策及关键 Token 时拒绝生成，不自动套用模板。导出资产是设计起点，原生布局需补真实字段，在线效果仍须按实际平台验收。
...
- **样式统一进入应用主题**：表单控件、背景、交互状态、详情表面和底栏规则归入同一份应用主题 CSS，由应用全局样式统一作用于应用框架、表单、详情页和自定义页面。补充底栏背景与阴影随主题设计、主体与底栏对齐、窄屏及长表单验收规则。
- **补齐控件状态与内容背景**：主题模板增加控件圆角、高度、字号、填充、边框和标签间距变量，补充 hover/focus 边框及 focus 背景；通过 `--yida-form-content-bgcolor` 将表单内容接到应用页面背景。新版无导航提交页支持消费应用外衬颜色与背景图。
- **底栏按实际场景设计**：普通提交和详情按钮栏使用 `--pod-page-footer-bg-color`、`--pod-sticky-footer-box-shadow`；数据管理内的表单视图使用 `--pod-formView-stickyFooter-*`。技能和设计示例明确各自的背景、阴影、边框与圆角，不以导航显隐判断底栏类型，也不统一强制透明。
- **固定提交栏占位适配**：模板根据实际底栏标记，将新版 PC 固定提交栏的底部占位从 margin 改为 padding，支持有导航和无导航提交页；区分数据管理、抽屉提交与容器内 `sticky-v2`，避免把同一定位规则套到所有表单。正文与底栏优先使用平台已有内容宽度和间距变量。
- **补充新主题消费边界**：对照 `yida-shell`、`vc-deep-yida` 和 `yc-common` 的 `trunk-theme-gray` 源码，列出提交/编辑、管理页、详情卡片、记录历史、门户及移动端的 token 消费位置，区分新版主题、legacy 路径和仍有硬编码的控件。
- **验证主题生成与增量保留**：补充 Fast、Plan 表单主题生成测试，覆盖两类底栏变量，检查变量不会被重复声明覆盖，调整主题中的表单密度变量时保留背景、品牌色和无关定制样式。

### 升级说明

- 已发布应用不会自动迁移。按新规则更新设计源并生成、上传应用主题；已有主题缺少提交页背景规则时，核实实际 DOM 后增量补齐，不重置整份 CSS。历史页面注入需在明确迁移范围后单独清理并验证。
- 布局仍需通过字段 JSON 和原生 Schema 实现，本次没有新增从 `design.md` 自动生成或核对逐表单栏数、列宽及间距的能力；`--theme compact|comfortable` 不代表完整应用主题，也不保证新版页面的内距随之改变。
- 当前验证覆盖主题生成、表单构建和静态契约；真实页面仍需检查控件状态、导航展开/折叠、管理页底栏、抽屉和移动端。富文本等控件存在硬编码样式，不能仅凭 token 声明认定全部视觉已适配。
