...
- **方案支持局部修改**：调整已有 Plan 时，可一次修改多个字段，只更新 PRD、设计文档、方案 HTML 和主题 CSS 中受影响的内容，保留其他内容与自定义样式。未变化的文件不重复写入，仅调整方案数据时不渲染 HTML；遇到本地修改冲突时停止写入并提示位置。
...
- **数据格式与命令说明**：明确 `DateField` 传毫秒时间戳数字、`CascadeDateField` 传时间戳数组，补充按业务时区转换的示例。同步 CLI manifest、帮助、Skill 和中英文 README，完整列出流程创建的两种用法及 `--replace`，将流程创建和数据命令的输出类型标为 JSON。
