...
- 完整应用交付恢复开发者管理后台入口：不再因云端注入登录态或仅前台范围省略 `/admin`；按实际应用提供前台、可选业务后台与开发者管理后台 2–3 类入口。`create-app`、`app-list` 输出明确的 `appUrl`、`workbenchUrl` 和 `adminUrl`，同步 CLI 契约、技能指引与回归测试。

- 自定义顶部导航默认采用贴顶通栏，悬浮胶囊仅按明确设计选用；首屏背景可延伸至导航背后，导航初始透明，滚动后增加遮罩底色，回到顶部恢复透明。
- 完善连续展示页与业务工作区的布局模板，支持按页面边界固定导航，并统一导航保留、滚动位置恢复和页面切换状态的生成规则。
- 自定义页面抽屉默认使用应用背景色 `--pod-shell-theme-bg-color`，内容容器继承抽屉背景，主题切换时保持一致，并保留显式背景覆盖能力。
- 视觉设计在保留用户指定风格、品牌和既有主题的基础上比较候选方向，改善构图、排版和色彩协调；待办与进度说明聚焦业务内容。
- 允许外链的素材经格式和尺寸检查后优先直接使用，减少下载与重复上传；需要托管时检查转存许可，上传失败时可回退到已验证且允许外链的原地址。
- 素材调度先核实宿主的后台 Agent 与后台命令能力：支持时派发后继续业务搭建；仅支持同步工具时先推进独立业务，再分批准备素材。仅在对应页面接图和验收时汇合，其他页面以及布局、数据绑定和交互开发继续推进。
- 素材任务优先处理必需图片和首屏图片，增加任务复用与采集时间预算，减少重复派发和长时间等待。
- 面向用户的搭建计划与交付文案不再展示内部修订序号，保留内部版本与确认校验机制。
- 新增管理员业务工作台入口片段，按当前应用访问者的管理身份展示主题按钮，支持明确指定管理页面和视图；未知身份隐藏，避免误用字符串权限值、跳入开发后台或返回前台首页。
- 优化 Tabs、分段筛选、单选按钮和普通按钮的默认状态：选中项采用轻品牌底与清晰文字，实心主按钮按可解析主题色检查文字对比度；主题预览补充可交互的分类与标签页示例。

### 问题修复

- 修复初次生成和内部补全导致方案版本提前递增的问题；相同内容重复生成不再使已有确认失效，同时继续检查过期规划片段。
- 编译和发布前核对受支持图标包的运行时导出，拦截引用不存在图标的源码，并提示具体位置与替代图标，减少页面打开时出现 React 组件类型错误。
- 修复本页视图菜单被平台导航过滤后显示“无可用导航”的问题；本页切换使用本地视图数据，保留实际访问权限校验，并提前提示错误的导航接入方式。
- 修复连续展示页内容高度被压缩导致无法正常滚动的问题，补充布局检查，并改善嵌套滚动容器中的导航遮罩和顶部留白处理。
- 完善视觉字段对象格式、本页菜单承载页面与视图绑定的错误提示；兼容 `auth profile list`，正式入口仍为 `auth profiles`。
- 发布前拦截尚未展开的主题装配标记，明确直接使用主题模板、手动接入与脚本装配的适用方式，避免误将生成主题文件作为所有页面的必经步骤。
- 完成度检查新增素材执行记录核验，识别后台派发证据不足、素材与业务没有时间重叠、同步流程将素材整包前置等情况，标记为待复核；同步 CLI、技能说明、多语言提示与回归测试。
- 新增统一页面跳转片段，区分本页 Tab、独立前台、管理页与表单入口；编译前拦截可确定的缺少应用前缀的跳转，以及 ConfigProvider 中固定品牌交互色的配置，并补齐主题链接的悬停与按下色阶。

### 升级说明

- 已发布应用保持现状；导航、布局与抽屉背景优化需更新已有页面中使用的模板代码并重新发布后生效。
- 后台素材执行依赖宿主实际提供的工具与参数，不根据宿主名称推断支持；执行记录校验不等同于宿主真实运行验证，缺少记录的旧产物不会被视为已验证并发。
