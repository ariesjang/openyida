...
- 自定义顶部导航默认采用贴顶通栏，悬浮胶囊仅按明确设计选用，避免将顶部导航一律生成浮导。
- 完善首屏背景与导航的一体化设计规则：有背景图时导航初始透明，滚动后增加遮罩底色，回到顶部恢复透明。
- 完善连续展示页与业务工作区的布局示例，以及导航保留、滚动位置恢复和页面切换状态的生成规则，减少背景断层与重复滚动。

### 问题修复

- 修复搭建计划首次展示就成为第 2 版的问题；新计划首次生成和内部补全保持第 1 版，已展示方案发生实质修改时才升版。
- 相同内容重复生成不再使已有确认失效，版本号相同时仍检查规划片段是否过期；同步 CLI、技能说明与回归测试。

### 升级说明

- 已发布应用保持现状，页面生成规则与布局优化需在更新并重新发布对应页面后生效。
