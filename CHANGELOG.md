...
- **方案局部修改**：已有方案按字段批量调整，CLI 同步 PRD、设计文档、方案 HTML 和主题 CSS 的变化部分，保留未改内容与自定义样式；相同产物不重复写入，本地修改冲突时保留原文件，并统一相关技能的修改流程。
- **项目主题可按需扩展**：明确平台变量是基础契约，支持增加材质、布局、字体、动效和组件状态等变量；允许平台变量引用同一主题中已声明的扩展变量，保留未定义引用与循环引用检查。
- **统一设计主题库**：Fast、Plan 和单页设计共用 15 套主题，先比较风格摘要，再读取选中的完整主题，减少重复选型与多套规则之间的差异。
- **设计交接更具体**：统一 `design.md` 的主题变量、页面引用与五章正文格式；Plan 生成最终方案前，要求每个自定义页补齐首屏焦点、布局、主操作、响应式和验收条件。
- **新增设计文档检查**：提供 `openyida check-design`，检查文档格式、主题变量和页面引用；支持 `--prd` 核对业务文档中的设计引用，以及 `--base-dir` 指定引用的项目根目录。
- **Fast、Plan 使用同一套 UI 规则**：共用主题配色与逐页设计检查，拒绝用“按主题执行”等空泛描述代替页面决定；移除 Plan 独有的 `surfaceTone` 自动染色。应用根背景渐变使用已有的 `--pod-app-root-bg-image`，单页装饰放在页面自身的背景层。

### 问题修复

- 修正图标与底色接近的生成倾向：统一前景/背景配对规则，设计与主题生成检查已声明配色的对比度，并要求核对页面实际的描边、颜色继承和透明度。
- 补齐前后台导航交付：每个入口都规划并验证菜单顺序、默认页面与首屏任务；自定义导航和仅前台应用也需落实排序，平台排序回读不再被当成首页正确的证明。
- 补齐全应用风格交接：将已选方向的画布、面板、字体和辅助色落实为共享主题变量，自定义页面引用同一份变量；修正直接修改生成 CSS 的步骤，并要求分别核对原生页与自定义页，避免把主题绑定成功误当成视觉一致。
- 修正后台首页规划与技能路由：按角色任务选择门户式工作台、原生业务页或分析页；移除“工作台”强制触发经营看板的规则，待办、操作队列与常用入口不再自动变为驾驶舱。Fast、Plan 和技能索引使用同一判断。

- 修复主题生成与更新中的导航配色处理，分别维护浅色、深色导航的背景与文字状态，统一选中项图标和文字颜色。
- 主题生成和上传前检查 CSS 括号、字符串与注释是否闭合，提前提示结构错误；生成失败时保留原主题文件和更新记录。
- 完善设计文档解析、变量引用与跨文档路径检查，支持带引号等特殊字符的项目名称，减少设计文件读取和交接错误。
- 修复自定义输出目录生成的设计引用，使生成结果可直接通过 `check-design --prd`；导航章节直接列出项目最终配色，避免同时出现默认浅色与项目深色两套说明。
- 补齐 Plan 示例中的主操作字段，并让技能编写步骤与 CLI 返回的业务、视觉任务及依赖保持一致。

### 升级说明

- 旧计划引用已移除的主题时，需要从当前主题索引重新选择主题并更新绑定；缺少逐页设计内容的计划，补齐后才能生成最终方案。
- 已发布应用不会自动换肤。设计检查通过不代表页面视觉和交互已验收，更新主题或页面后仍需检查实际效果。
