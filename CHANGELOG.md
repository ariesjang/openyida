...
- 优化较大自定义页面的读取、编辑和发布，并减小页面体积。
- 交付说明更清晰，集中展示已完成的业务功能、使用入口和待完善事项。

### 问题修复

- 改善表单创建异常后的恢复处理和结果提示，减少重复创建与无效重试。
- 修复部分配色要求未正确应用，以及页面编辑错误提示不完整的问题。
- 改善含多条明细记录的表单设计，减少业务数据结构识别错误。
...
- 修复流程加签、转交配置在设计器与运行配置之间不一致的问题；开启加签时同步生成加签位置和结果规则，支持前加签、后加签及是否参与审批。
- 流程发布回读增加按钮权限和加签参数校验，避免配置缺失时仍报告验证成功；无效操作权限在创建或发布流程前拦截。
- 兼容原始动作列表和已有按钮权限，未配置加签、转交时保持历史默认值。

### Changed

- 补齐流程 CLI 帮助、命令与能力清单、创建/配置流程技能和中英文文档，明确通过流程定义的 `nodes[].actions` 配置操作权限。

### 升级说明

- 更新 CLI 不会自动修改已发布流程或历史实例；需在确认完整流程配置后重新发布，并验证真实审批人的加签、转交行为。
