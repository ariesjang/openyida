...
- **主题命令与技能对齐**：统一三文件导出、主题生成及上传的参数说明；拒绝缺值、未知和冲突参数，自绘导航同步消费菜单边框与选中阴影，抽屉说明统一使用应用外壳背景。

- **导航形状**：主题菜单支持普通、悬停和选中边框，侧栏与顶部菜单共用圆角和选中阴影；模板明确区分细线、粗线、双线、虚线、胶囊和不对称题签。

### 应用导航与整体风格

- **案例导航沉淀到模板**：波普印刷、北欧云白、复古票据、终端像素四套案例的圆角、三态边框、选中阴影和密度写回公共维护源、设计文档及主题 CSS，重新生成模板后保持一致。
- **项目导航取值完整展示**：Plan 生成的设计文档展示菜单边框、内距、顶栏尺寸和框架留白；项目覆盖值替换模板示例描述，避免设计正文与实际主题参数冲突。
- **自绘导航尺寸同步**：侧栏与顶部菜单分别读取对应高度和内距，菜单间距、侧栏留白和长名称宽度随主题生效；顶部滚动区增加阴影留白。
- **主题目录统一**：全部 34 个主题入口采用 `design.md`、`app_theme.css`、`form-layout.json` 三文件结构，均可通过 `application-style` 导出；历史方案的旧主题路径仍可读取。
- **品牌颜色按项目生成**：主题设计与 CSS 保留 `--color-brand1-6` 的项目主色占位和 `--color-brand1-1` 的悬停色生成占位，其他品牌色阶同源派生；完成项目设计后生成实际颜色，模板不固定品牌色。
- **导航纳入应用设计**：34 个主题入口统一规划导航、应用框架、原生表单、自定义页和详情页的风格；各模板提供独立的导航配色与样式，CLI、命令清单和技能说明使用一致的主题契约。
- **导航选中阴影可独立配置**：补齐 `--pod-nav-menu-item-selected-shadow` 从主题模板、生成与覆盖到原生菜单样式的完整链路，保留各模板的圆角设置；切换选中菜单时同步更新阴影效果。
- **详情页与抽屉分层配色**：详情页画布与工具栏跟随内容区域主题；表单抽屉外壳使用 `--pod-shell-theme-bg-color`，标题和按钮跟随应用框架文字颜色，内部 iframe 继续使用平台页面主题。

### 方案生成与配置校验

- **视觉选择保留更完整**：`design-plan` 根据主题推导导航风格，保留显式选择、项目覆盖和业务布局理由；同步生成的设计文档与主题 CSS，并对未知参数、缺失参数值明确报错。
- **导航保存后回读确认**：`update-app` 分别验证主题和导航配置，支持最多三次读取重试；导航配置未持久化时返回明确错误，读取重试不会重复写入。
- **模板与说明同步维护**：补齐设备维护和员工服务页面规格，更新应用设计、表单、导航相关技能说明与多语言文案，并调整发布包体积预算。

### 升级说明

- 已发布应用不会自动更新。需要新的导航样式或选中阴影时，请更新设计源、重新生成并上传应用主题，再检查导航、表单、详情页和自定义页的实际效果。
- `design-plan` 现在严格校验参数；已有脚本中拼错或未支持的参数需要修正。
