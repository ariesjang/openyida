...
- **已有方案按需修改**：支持一次提交多项调整，将变化写回 PRD、设计文档、方案 HTML 和主题 CSS，保留无关内容、本地备注与自定义样式。遇到修改冲突时指出位置，停止本次写入。
- **减少 Plan 重复处理**：同一次执行复用输入相同的主题结果和设计校验，减少重复解析；合并本地文档修改后仍重新检查。未变化的文件不重复写入，只保存方案数据时不渲染 HTML。
- **Fast、Plan 和单页设计共用主题**：统一使用 15 套主题及相同的设计规则，先比较风格摘要，再读取选中的主题。应用主题 CSS 与自定义页面共用背景、面板、字体等变量；平台变量作为基础，项目可按需扩展样式变量，减少两套风格不一致的问题。
- **首页和导航按业务安排**：完善需求分析与页面规划规则，根据用户角色和主要任务选择门户、工作台或业务页面；办事场景优先安排待办、操作队列和常用入口，经营分析场景再规划驾驶舱。前台和后台都明确默认页面、菜单顺序与首屏重点。
- **设计文档支持统一检查**：新增 `openyida check-design`，检查文档结构、主题变量、页面引用及逐页设计说明；通过 `--prd` 核对业务文档，通过 `--base-dir` 指定项目根目录。
- **常用命令帮助更完整**：`create-process --help` 展示新建、复用两种用法及输出含义；`data --help` 说明 `DateField` 使用毫秒时间戳数字、`CascadeDateField` 使用毫秒时间戳数组，并按业务时区转换。命令清单、Skill 和多语言提示同步对齐。
- **素材清单保留授权依据**：每张图片可记录图片 ID、作者、来源页、许可链接、核对日期及必要授权凭证，失败重试时仍保留。未取得的信息留空，记录本身不代表已认证可商用。

### 问题修复

- **表格加载黑边闪现**：共享 Canvas 主题模板固定加载遮罩边框为零，仅保留透明度过渡，避免刷新页面时延迟加载的基础样式触发黑边动画；CLI 示例与主题装配脚本同步生效，已有页面需更新 Provider 后重新发布。
- **导航和图标配色**：分开处理浅色、深色导航的配色，统一选中项图标与文字颜色；补充图标前景与底色的配对规则，拦截已声明且可解析的配色中低于 3:1 的组合。
- **主题文件检查**：主题生成和上传前检查未闭合的括号、字符串及注释；局部更新仍检查已有主题，避免损坏的文件被复用逻辑绕过。
- **设计文件与预览**：修复特殊字符项目名、自定义输出目录下的设计引用，以及 Plan 预览遗漏已接受页面草稿的问题。
- **流程复用输出**：`create-process` 新增 `formMode=create|reuse`。复用时的 `formTitle`、`fieldCount` 改为 `null`，表示未查询，避免占位名称和 `0` 被误认成真实信息；成功与配置失败输出使用相同约定。
- **流程参数诊断**：缺参、重复参数或未知选项在读文件和登录前报错，返回具体参数与原因，避免错误参数被当作文件路径或静默忽略。
- **入口校验误报**：缺少 `entry.sceneKey` 时，不再同时误报 `menu.resource`；确定关联页面后再检查资源名称是否匹配。

### 升级说明

- 旧主题计划需切换到当前主题并补齐逐页设计。背景和渐变通过主题变量或页面样式设置，不再使用 `surfaceTone`。
- 读取流程创建结果的脚本需适配复用模式的空值，依据 `success` 与 `verificationLevel` 判断结果。
- 已发布应用需更新主题或重新发布页面后生效；静态检查只覆盖文档与已声明配色，仍需验收页面实际效果和交互。
