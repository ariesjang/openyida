...
- **表单组件树更灵活**：表单可创建 19 种业务字段、`Divider` 和 `ColumnContainer`，支持在顶部、侧栏、主体及字段之间组合内容。更新已有表单时保留 Tab、操作按钮组、图片和状态区等现有组件。
- **创建参数统一**：`create-form` 的布局、密度、标签位置、图标、语言和打开页面选项已在 CLI、帮助、命令清单与技能中保持一致，单列、双列、卡片和分区布局可直接选择。
- **表单配置更安全**：表单和详情页使用原生组件与应用主题完成布局和样式，不再提供 CSS、HTML、JavaScript 或 DOM 注入路径；移除已停止支持的 `PageSection`、`GroupContainer` 创建能力。

### 应用主题与设计

- **内容与导航明暗独立设置**：新增 `contentTone` 与 `navTheme` 双轴，页面内容、表单、详情、自定义页面和浮层的明暗不再与平台导航框架绑定，支持四种自由组合。
- **全局风格保持一致**：应用主题统一作用于应用框架、表单、详情页和自定义页面，并补齐详情页只读字段数据框的背景、边框、阴影、文字、间距与尺寸变量。
- **应用风格模板完善**：提供 18 套完整应用风格和自由创意入口，设计文档、应用主题与原生表单布局使用同一套视觉决策；模板与技能说明进一步去重，关键规则集中维护。

### 升级说明

- 已发布应用不会自动迁移。需要新版主题或表单布局时，请更新设计源、重新生成并上传应用主题，再按实际页面检查导航、表单、详情、自定义页面、抽屉和移动端效果。
- 更新已有表单会保留现有 Tab、按钮组、图片和状态区；历史代码注入不会由本次升级自动清理，需明确迁移范围后单独处理并验证。
...
- **统一主题与设计交接**：Fast、Plan 和单页设计共用 15 套主题，选型先比较摘要，选定后再读取完整模板。PRD、设计文档和方案预览共用页面引用与设计事实，减少不同流程各用一套规则的问题。
- **应用与页面共用配色**：平台页面和自定义页面共用应用主题 CSS；平台变量作为基础，项目可扩展自己的样式变量。指标卡、按钮和图标按所在表面搭配颜色，深色面板和近黑按钮不再作为通用默认。
- **首页与导航按任务安排**：前台、后台分别明确默认页面、菜单顺序和首屏内容；办事场景围绕待办、操作队列和常用入口组织工作台，有经营分析需求时再规划驾驶舱。沿用平台导航时，自定义页面不重复生成管理菜单。
- **分割线支持 23 种样式**：字段 JSON 的 `dividerType` 支持全部可见样式，不接受 `none`。按业务场景选择，同页同层级保持一致，不同页面优先使用合适的不同样式；无法判断时在生成阶段随机轮换。CLI 未传值时固定使用 `bold-with-thin`，非法值明确报错。
- **素材清单展示授权依据**：保留并展示图片 ID、作者、来源页、许可链接、核对日期及必要授权凭证，素材重试时不丢失记录。缺失信息留空，留证记录不等于已确认可商用。

### 方案修改与校验

- **局部修改保留已有内容**：`design-plan patch` 支持一次提交多项修改；配合 `--materialize` 按变化更新 PRD、design、方案 HTML 和主题 CSS，保留无关章节、本地备注及自定义样式。与本地修改冲突时返回文件和位置，停止本次写入。
- **减少重复生成与写入**：同次执行复用相同输入的主题结果和设计校验，未变化的文件不重复写入；不带 `--materialize` 时只校验并保存方案数据，不渲染 HTML。合并本地修改后仍检查设计结果。
- **补齐设计检查与诊断**：新增 `check-design` 检查文档结构、主题变量、页面引用和逐页设计；检查已声明且可解析的图标前景与背景对比度。主题生成和上传前检查 CSS 括号、字符串及注释完整性；修复特殊字符项目名、自定义输出目录和预览遗漏已接受页面草稿的问题。

### 页面与 CLI 修复

- **隐藏导航以回读结果为准**：`create-page --hide-nav` 写入后读取实际导航配置，不一致时最多修正一次；仍未确认时返回失败、已创建页面 ID 和恢复命令，避免输出 `hideNav=true` 但实际仍显示导航。`get-form-config` 同步提供 `renderNav`。
- **表单入口统一使用 iframe 抽屉**：强化标准抽屉模板、调用 hook 与容器挂载的编译检查，拦截可识别的桌面端直接跳转和缺失抽屉结构；保留移动端原生入口。CLI、模板与 Skill 使用同一约定。
...
- **页面状态恢复有明确规则与样例**：新增 `canvas-view-state` 可提取片段，支持从 URL 恢复当前任务的视图、非敏感枚举筛选和页码；筛选或视图变化后重置分页，重复选择不增加历史，保留平台参数、hash 和历史元数据。同步说明已有路由、iframe、跨任务缓存与未保存编辑的接入边界。
- **草稿与敏感状态单独管理**：技能明确页内切换、刷新、详情及跨页返回的恢复规则；敏感筛选默认仅存内存，草稿通过稳定父层或已验证的草稿服务保存，避免静默丢失或写入 URL。
- **业务写操作先核对授权**：技能新增动作权限清单，要求核对业务角色、记录范围、可写字段和状态约束，并验证普通用户直接调用、替换记录、重复提交等场景；明确管理员按钮显隐不能替代服务端授权，未具备真实约束的动作不得作为已完成交付。

### 维护与验证

- 简化 `sample` 命令的嵌套提示表达式，降低修改提示文案时引入括号错误的风险。
- 对齐样例索引、CLI manifest、技能与状态恢复测试，覆盖参数白名单、历史恢复、连续切换、事件清理及样例提取编译。

## [2026.9.16-1] - 2026-09-16

### 体验优化

- **导航与页面更连贯**：自定义顶部导航默认贴顶通栏，悬浮样式按设计选用；首屏背景可延伸至导航背后，滚动时增加遮罩，改善导航、正文和页面切换之间的割裂感。
- **控件状态更清晰**：完善标签页、分段筛选、单选按钮、普通按钮和链接的选中、悬停、按下及禁用状态；实心按钮按可解析的主题色检查文字对比度，改善选中文字与背景难以区分的问题。
- **抽屉跟随应用背景**：自定义页面抽屉默认使用 `--pod-shell-theme-bg-color`，标题栏与正文承接同一背景，并保留显式覆盖能力。
- **管理入口更明确**：新增管理员业务工作台按钮模板，根据当前应用访问者的管理身份显示入口，支持指定业务管理页面和视图；身份未知时隐藏入口。
- **交付地址更完整**：完整应用默认提供开发者管理后台 `/admin`，不再因云端登录方式省略。有前后台时交付前台、业务后台、开发者管理后台三个地址；仅前台或统一工作区时交付对应业务入口和开发者管理后台两个地址。
- **素材与业务搭建协同推进**：分别核实宿主后台 Agent 和后台命令能力，支持时后台准备素材并继续搭建；仅支持同步工具时先推进独立业务，再分批准备素材。图片仅在对应页面接图与验收时汇合，其他页面及布局、数据和交互开发继续进行。
- **减少素材等待**：允许外链的图片经格式与尺寸检查后优先直接使用，减少下载和重复上传；优先准备必需图片与首屏图片，增加任务复用和采集时间预算，需要托管时按转存许可处理。
- **设计与方案表达更自然**：保留用户指定风格、品牌及既有主题，完善候选设计方向比较；待办、进度和交付说明聚焦业务内容，用户可见的搭建计划不再展示内部修订序号。

### 问题修复

- 减少业务管理页面的重复导航：已有平台菜单时，自定义页面默认只生成业务内容，跨模块切换交给平台菜单；保留同一任务的分类、状态 Tab 和上下文操作。同步 Fast/Plan、页面交接、模板提示与验收规则，并修正笼统要求隐藏页面导航的指引。CLI manifest 与页面交接共用导航策略，独立页未规划菜单时不再默认生成菜单；导航随整体方案讨论，Fast 不增加额外确认步骤。
- 编译和发布前校验受支持图标包的实际导出，拦截不存在的图标引用并提示位置与替代项，减少由未定义组件引起的 React 渲染错误。
- 新增统一页面跳转模板，区分本页切换、独立前台、业务管理页和表单入口；编译前拦截可确定的缺少应用前缀的跳转，减少按钮和链接打开错误地址的问题。
- 加强主题接入检查，拦截 ConfigProvider 中可确定的固定品牌交互色和尚未展开的主题装配标记，明确不同主题接入方式的适用范围。
- 修复本页视图菜单被平台导航过滤后显示“无可用导航”、连续展示页高度压缩导致无法滚动等问题，完善嵌套滚动容器中的导航遮罩与顶部留白处理。
- 修复首次生成和内部补全导致方案提前升版的问题；相同内容重复生成保持已有确认，同时继续校验过期规划片段。
- 完成度检查增加素材执行记录核验，将后台派发证据不足、宣称并行但没有时间重叠，以及同步流程把素材整包前置等情况标记为待复核。
- 完善视觉字段对象格式、本页菜单承载页面、视图与权限依赖的提示；兼容 `auth profile list`，正式入口仍为 `auth profiles`。同步相关 CLI、模板、技能、12 种语言提示和回归测试。

### 升级说明

- 已发布页面不会自动迁移；导航、控件状态、抽屉和跳转优化需要更新对应页面中的模板代码并重新发布后生效。更严格的发布校验可能要求先修复此前未被拦截的源码问题。
- `create-app`、`app-list` 新增 `appUrl`、`workbenchUrl` 和 `adminUrl`，保留原有 `url` 或 `systemLink` 字段。入口展示和地址生成不代表授予权限，实际访问仍由平台校验。
- 后台素材执行取决于宿主实际工具能力，执行记录检查不等于真实宿主验证；静态编译检查也不能替代页面运行与多身份权限验收。
- 缺少展示记录的旧计划沿用保守升版策略，避免误用历史确认。

## [2026.9.16] - 2026-09-16

### 体验优化

- 需求确认更聚焦：优先确认核心功能和实际用法，已提供的信息直接沿用，减少重复提问。
- 优化方案规划，减少功能与页面设计中的重复整理，更快形成可供查看和调整的搭建方案。
- 根据使用场景安排前台、后台和导航入口，更清晰地区分对外服务与内部管理。
- 多个页面和图片位置可同时准备素材，页面搭建同步推进；图片获取失败时及时切换来源，减少反复等待。
- 图片准备完成后直接用于已确认的方案，无需再次确认；功能或设计发生变化时再确认调整。
- 统一表单、自定义页面和弹窗的主题表现，改善按钮配色、页面背景和换肤效果的一致性。
- 交付说明更清晰，集中展示已完成的业务功能、使用入口和待完善事项。
...
- 已发布应用保持现状；页面主题和交互优化在更新并重新发布对应页面后生效。

## [2026.9.15] - 2026-09-15

### 体验优化

- 应用搭建完成后，集中说明已完成的业务功能、使用入口和待完善事项，便于直接开始使用。
- 根据当前账号的实际访问能力提供应用入口，减少交付链接无法使用的情况。
- 按已确认的功能范围检查完成情况，页面、表单和流程的剩余工作会明确列出。

### 问题修复

- 改善表单创建中断后的恢复处理，优先继续已有表单，减少重复创建。
- 表单保存结果暂时无法确认时，先检查实际状态，再继续后续搭建，避免误报完成。

## [2026.9.14] - 2026-09-14

### 体验优化

- 根据业务场景安排导航方式，用户描述实际需求即可，减少技术选项的确认。
- 优化较大自定义页面的读取、编辑和保存，并压缩页面内容，减小发布体积。
- 方案确认更聚焦，集中展示当前方案和需要决定的事项。

### 问题修复

- 改善订单明细等多条记录的表单设计，减少将多条业务记录误设为单个字段的问题。
- 页面内容读取失败或不完整时给出明确提示，减少基于不完整内容继续编辑的情况。
- 跨应用自动化存在能力限制时，提前说明影响和待解决事项。

## [2026.9.13] - 2026-09-13

### 问题修复

- 自定义页面检查失败时保留具体出错位置和原因，便于准确修复，减少反复尝试。
- 改善页面发布前的检查提示，区分页面内容错误与环境问题。

## [2026.9.12] - 2026-09-12

### 体验优化

- 自定义页面支持在发布前完成本地检查，提前发现页面问题，减少发布后返工。
- 完善初始业务数据的准备流程，分别检查普通表单和审批流程，减少数据写入错误。

## [2026.9.11-3] - 2026-09-11

### Added

- 新增页面配图流程，可按需选用用户图片或图库素材；所用 AI 工具支持生图时，也可生成首页主视觉、封面和场景配图。

### Changed

- 图片默认保存到宜搭图片附件，减少对第三方图片地址的依赖，无需自行配置图片存储服务。
- 按页面需要准备图片，表单、审批、台账等以信息录入和管理为主的页面可直接继续搭建，减少不必要的配图步骤。
- 已准备好的图片可重复使用；缺图时明确提示需要补充的内容，不影响其他图片已齐备的页面继续搭建。
- 图片下载失败时自动跳过；超过 20 MiB 的外链图片保留原链接。

### Fixed

- 改善图片格式不正确、尺寸不足或素材缺失时仍被当作可用图片的问题。

### 升级说明

- 使用图片上传功能需先登录宜搭并指定目标应用；升级不会自动更换已发布页面中的图片。
- 通过脚本调用旧版素材命令的用户，请参考 `openyida asset resolve --help` 更新调用方式。

## [2026.9.11-2] - 2026-09-11

### 问题修复

- 修复审批流程设置了加签、转交后，实际可用操作与设置不一致的问题。
- 完善前加签、后加签及加签人是否参与审批的配置，并在发布后检查是否保存成功。
- 保留已有审批按钮设置，减少更新流程时意外改变操作权限的情况。

### 体验优化

- 补充加签与转交的使用说明，配置审批操作更清晰。

### 升级说明

- 升级后，已有流程保持原状；调整加签或转交设置后，需重新发布流程并检查实际审批操作。

## [2026.9.11-1] - 2026-09-11

### Changed

- 优化导航选项，明确“宜搭原生导航”和“自定义导航”的选择。
- 增强 Agent 根据业务场景规划导航的能力，自动选择合适的顶部、侧边或 L 型布局，让导航更贴合应用的使用需求。

### Fixed

- 修复旧应用更新主题或导航设置时丢失原导航布局的问题，保留已有导航样式和布局。

## [2026.9.11] - 2026-09-11

### 问题修复

- 批量搭建表单时，已创建但尚未配置完成的表单可在同一轮继续恢复；恢复成功后接着搭建关联表单，减少手动重启和重复创建。

## [2026.9.10] - 2026-09-10

### 体验优化

- 搭建过程中持续沿用已选择的 Fast 或 Plan 方式，减少重复选择和流程切换。
- 批量搭建表单前检查字段和关联关系，提前提示需要修正的内容。

### 问题修复

- 改善批量表单创建失败后的诊断与恢复，保留已完成结果，减少重复处理。
...
- 自定义页面与应用使用统一的页面和卡片底色，减少原生页面与自定义页面之间的视觉割裂。
- 优化自定义页面的配色规则：主按钮、链接和选中态跟随应用主题，次要操作保持中性，危险操作保留醒目的提示色。
- 表单抽屉标题栏与平台头部高度保持一致，背景与抽屉整体协调。

### Fixed

- 改善表单抽屉、批量录入和图表页面模板中部分控件未跟随应用主题变化的问题。
- 修复表单提交页和详情页嵌入抽屉时出现双滚动条、多余留白和外层白色卡片的问题。

### 升级说明

- 已发布页面不会自动改变；需要更新对应页面的主题适配或抽屉实现，并重新发布后生效。

## [2026.9.8-1] - 2026-09-08

### 体验优化

- 改善批量表单搭建的进度记录和中断恢复，继续搭建时优先复用已有结果。
- 方案确认内容更完整，业务范围、页面和表单安排统一呈现。

### 问题修复

- 加强表单保存和流程发布后的结果检查，减少配置未生效却提示完成的情况。

## [2026.9.8] - 2026-09-08

### 问题修复

- 修复自动化流程中明细表来源、字段和关联表单识别不完整的问题。
- 改善自动化流程读取和新增明细记录的配置，减少编辑时选不到明细表或字段的情况。
- 修复部分审批实例查询条件保存后无法正确使用的问题。

## [2026.9.7] - 2026-09-07

### Changed

- 发布结果明确区分服务端写后回读与浏览器运行态验证，避免把尚未执行的页面运行检查表述为已完成。
- 完整应用工作流贯穿用户明确禁止的主题、源码和发布动作，并收紧普通创建/发布请求误入应用上线/下线技能的路由。
- CLI 能力诊断增加构建、提交、bundle 与命令清单身份，便于确认测试环境实际加载的版本。

### Fixed

- 在发布前拒绝 Canvas 依赖自引用和运行时代码与依赖清单不一致，减少发布成功后页面无法运行的问题。
- 流程表单转换后进行有限只读回查；结果仍不确定时保留原资源并返回不可安全重试状态，避免重复创建。

## [2026.9.7-beta.0] - 2026-09-07

### 预发布体验

- 提供先查看方案再搭建的 Plan 模式，并优化方案预览、导航和表单抽屉体验。
- 改善应用主题的一致性，以及页面发布和连接器配置后的结果检查。
- 修复 Windows 下方案预览的中文显示问题。
...
## [2026.9.4-beta.0] - 2026-09-04

### 预发布体验

- 自定义页面发布时压缩内容，减小页面体积。
- 业务规划与视觉设计可同时准备，并统一整理应用入口。
- 加强列表排序和关联数据的检查，减少字段使用错误。
...
- 统一应用主题与页面设计流程，让页面配色和整体风格更协调。

### 问题修复

- 加强自定义页面列表的排序字段检查，减少排序失效和数据查询错误。
...
- 延续较大自定义页面的读取与保存能力，并同步应用主题和表单图标优化。
- 优化安装和更新后的技能准备流程，减少重复加载。

## [2026.9.2-beta.0] - 2026-09-02

### 预发布体验

- 支持读取和保存较大的自定义页面，方便继续编辑已有页面。
- 页面内容下载失败或不完整时给出明确提示。
...
- 新增表单详情页增强链路，支持真实实例打开校验和详情链接校验，提升表单详情入口的稳定性与可控性。
...
### Fixed
- 修复表单详情实例相关入口与链接校验问题，避免无效链接或错误打开方式导致的体验异常。
- 收拢移动端颜色、页面背景、图文丰富度和样式主题规则，减少生成页面在不同端上的视觉漂移。
- 修正文案与输出约束，强化 emoji、样例信息和文件干扰等生成规范。

### Tests
- 新增或更新 `canvas-compile`、`create-form`、`sample`、`page-linter`、`skill-contracts`、`eval-routing` 和设计 PRD 质量相关测试。
...
- 表单创建能力增强，支持更丰富的组件树布局、主题配置、字段属性、规则和增量更新，便于生成更接近真实业务的表单结构。
