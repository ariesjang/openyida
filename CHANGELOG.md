...
- **统一主题设计**：Fast、Plan 和单页设计共用 15 套主题及同一套配色、组件和页面设计规则。先比较风格摘要，再读取选中主题，减少重复选型和不同流程之间的设计差异。
- **全应用共用主题变量**：将背景、面板、字体和辅助色统一写入应用主题 CSS，要求原生页和自定义页按对应角色使用。平台基础变量之外，可按需增加材质、布局、动效和组件状态变量，并检查未定义引用与循环引用。
- **方案修改更省步骤**：支持批量修改已有方案，同步更新 PRD、设计文档、方案 HTML 和主题 CSS 的相关部分，保留无关内容与自定义样式；内容不变时不重复写入，修改冲突时停止并保留原文件。仅修改方案数据时跳过 HTML 渲染。
- **页面设计更具体**：统一设计文档结构与页面引用，要求每个自定义页明确首屏重点、布局、主操作、响应式和验收条件。新增 `openyida check-design`，检查文档、主题变量与页面引用，并可通过 `--prd` 核对业务文档中的设计引用。
- **明确首页与导航顺序**：完善前台、后台和共用工作区的规划与交付规则，按角色任务确定默认页面、菜单及分组顺序、首屏内容；平台导航和自定义导航都需落实排序并检查实际入口。

### 问题修复

- **流程与入口诊断更准确**：`create-process` 明确区分新建与复用表单，未查询的名称和字段数返回 `null`，避免将复用误判为空表单；入口缺少 `sceneKey` 时，不再连带误报已填写的 `menu.resource`。数据写入命令附近补充日期毫秒时间戳提示与时区转换示例。
- **纠正工作台默认变成驾驶舱的倾向**：需求分析和技能路由按实际任务选择页面。集中办事使用待办、操作队列、业务列表和常用入口；只有明确的经营分析或监控需求才规划驾驶舱。
- **修复导航配色混用**：主题生成与更新分别处理浅色、深色导航的背景和文字状态，统一选中项图标与文字颜色；设计文档展示项目最终配色，减少默认配色与项目配色相互矛盾的说明。
- **补充图标对比度检查**：图标与底盒分别取色，设计校验和主题生成拦截已声明配色中低于 3:1 的组合；页面验收增加实际描边、填充和父级透明度检查，减少图标与背景过于接近的问题。
- **提前拦截损坏的主题 CSS**：生成和上传前检查括号、字符串及注释是否闭合；生成失败时保留原主题文件与更新记录。
- **修复设计文件读取与交接问题**：统一主题和素材读取设计文档的方式，处理带引号等特殊字符的项目名称，修正自定义输出目录中的设计引用；Plan 预览保留已接受的页面草稿，最终生成时再检查设计完整性。

### 升级说明

- 旧主题已替换，引用旧主题的计划需重新选择当前主题并补齐逐页设计。移除 Plan 专有的 `surfaceTone` 自动染色，背景与渐变改由主题变量或页面样式明确设置。
- 已发布应用不会自动换肤，需更新主题或重新发布页面后生效。设计检查只覆盖文档与可解析的配色，实际视觉和交互仍需在页面中验收。
