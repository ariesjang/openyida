...
- **模板配色按角色检查**：弱化波普大面积明黄和票据橙棕底，正文与辅助文字独立分级，修复可变主色导致导航状态难读、搜索误用水印色等问题。设计校验新增文字与背景对比度诊断，模板回归覆盖七组主色；Canvas 链接在保留品牌填充的同时使用可读前景。

- **固定主题只维护一套配色**：全局和平台各导航模式共用一份 token 声明，避免浅色模式覆盖深色项目导航；更新旧主题时合并重复声明，保留其他自定义样式。午夜霓虹模板同时减弱菜单选中底色，将正文与辅助文字调整为分级灰白，青色用于品牌和交互强调。
