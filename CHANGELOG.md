...
- **方案支持局部修改**：调整已有 Plan 时，可一次修改多个字段，只更新 PRD、设计文档、方案 HTML 和主题 CSS 中受影响的内容，保留其他内容与自定义样式。同次执行复用相同输入的主题结果和设计校验，减少重复解析；未变化的文件不重复写入，仅调整方案数据时不渲染 HTML；遇到本地修改冲突时停止写入并提示位置。
- **三种设计流程共用主题**：Fast、Plan 和单页设计统一使用 15 套主题及相同的配色、组件和页面设计规则。先比较风格摘要，再读取选中主题，减少重复阅读和流程之间的设计差异。
- **统一全应用主题规则**：完善应用主题 CSS 与自定义页面的共用规则，将背景、面板、字体和辅助色落实为共享变量。平台变量作为基础，项目可按需扩展材质、布局、动效和组件状态变量，统一检查缺失与循环引用。
- **首页按实际任务规划**：完善需求分析与导航规则，办事场景优先安排待办、操作队列、业务列表和常用入口，有明确分析需求时才规划驾驶舱。前台、后台和共用工作区都需确定默认页面、菜单顺序及首屏重点，并验证实际入口。
- **设计交接可检查**：统一设计文档格式，逐页明确布局、主操作、响应式和验收条件。新增 `openyida check-design`，检查文档结构、主题变量及页面引用；支持通过 `--prd` 核对业务文档，通过 `--base-dir` 指定引用的项目根目录。

### 问题修复

- **导航配色**：分别维护浅色、深色导航的背景与文字状态，统一选中项图标和文字颜色；设计文档只展示项目最终配色，避免与模板默认说明冲突。
- **图标辨识度**：补充图标前景与底色的配对规则。设计校验和主题生成会拦截已声明配色中低于 3:1 的组合，页面验收同时检查实际描边、填充和父级透明度。
- **主题与设计文件处理**：主题生成、上传前拦截未闭合的括号、字符串和注释，生成失败时保留原文件；修复特殊字符项目名、自定义输出目录的设计引用，以及 Plan 预览遗漏已接受页面草稿的问题。
- **流程创建结果**：`create-process` 新增 `formMode`，明确区分新建与复用。复用模式的 `formTitle`、`fieldCount` 返回 `null`，表示未查询，避免占位名称和 `0` 被误认为真实表单信息；成功和配置失败的输出使用同一约定。
- **入口校验误报**：缺少 `entry.sceneKey` 时，不再把尚无法核对的 `menu.resource` 一并报错；关联页面确定后仍检查资源名称是否匹配。
- **数据格式与命令说明**：明确 `DateField` 传毫秒时间戳数字、`CascadeDateField` 传时间戳数组，补充按业务时区转换的示例。同步 CLI manifest、帮助、Skill 和中英文 README，完整列出流程创建的两种用法及 `--replace`，将流程创建和数据命令的输出类型标为 JSON。命令帮助直接展示日期格式和复用输出说明；流程参数缺失、重复或拼错时，在读文件和登录前指出具体参数。

### 升级说明

- 引用旧主题的计划需改用当前主题并补齐逐页设计。Plan 不再使用 `surfaceTone` 自动染色，背景和渐变通过主题变量或页面样式设置。
- 读取 `create-process` 输出的脚本需允许名称和字段数为 `null`；流程是否成功应依据 `success` 与验证结果，不能用字段数判断。
- 已发布应用需更新主题或重新发布页面后才会生效。文档检查和配色检查不能替代实际页面的视觉、交互验收。
