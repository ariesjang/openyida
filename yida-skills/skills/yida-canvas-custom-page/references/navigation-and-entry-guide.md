...
抽屉外壳默认使用 `var(--pod-page-bg-color, var(--color-white, #fff))`，标题栏与正文容器透明承接同一底色，不使用 `--pod-card-bg-color` 铺满抽屉。CanvasThemeProvider 同步设置 antd Drawer 的组件级背景；其他卡片、弹窗保留各自表面。只有明确的设计覆盖才传 CanvasDrawer.background 或 openForm 的 background，不主动生成卡片色覆盖。标题、正文和标题栏按钮默认使用 `--color-text1-4`，显式的 drawer 颜色变量仍可覆盖对应角色。抽屉明暗跟随内容画布，`navTheme` 只决定导航明暗。传入 `background` 时同时检查标题、按钮和正文的可读性。iframe 内页面继续使用平台主题。
