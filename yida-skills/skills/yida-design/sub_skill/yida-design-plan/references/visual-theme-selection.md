...
先根据项目需求设计风格，再匹配主题索引中的模板。匹配成功后，将方案提供给用户选择。
...
用户确认主题前，只读取主题索引或 `openyida design-plan catalog --json` 返回的 `themes[].styleSummary`，不读取任何候选模板全文。该字段由模板风格摘要精简为约 150–250 字，保留整体外观、核心差异、关键偏离风险及内容适用条件，直接用于主题比较与推荐；主题名称用于识别，不能替代摘要中的视觉事实。

1. 从用户要求和参考材料中提取表面层次、边界、形状分工、文字节奏、强调方式等具体视觉特征。
2. 对照摘要的“核心特征”和“可见表现”比较候选，再用“主要偏离风险”检查是否违反用户已确定的要求。比较特征之间的关系，不按关键词命中数量打分，也不按行业、默认品牌色或主题名称直接套用。
3. 保留摘要里的适用条件。图表、指标、记录栈等按真实内容启用；页面没有这些内容，不代表不能使用该主题。摘要未说明的实现细节留到主题确认后的设计阶段处理，不据此补读候选全文或断言不支持。
4. 根据摘要展示候选，推荐理由说明用户要求与具体视觉特征的对应关系，以及候选之间的实际差异。只有用户确认主题后，才通过 `templatePath` 读取选中的一份完整模板，用于项目设计；不预读其他候选模板。

业务规划决定页面内容和操作；主题选择决定这些内容如何呈现，不能为匹配摘要中的示例而新增业务模块。
