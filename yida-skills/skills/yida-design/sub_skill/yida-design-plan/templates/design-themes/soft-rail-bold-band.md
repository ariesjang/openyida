---
name: "{{PROJECT_NAME}}"
description: "浅灰导航衬托白色工作区，白色圆角面板以细边框分组；由同一主色派生的深色横幅与明亮强调建立焦点，低对比线框纹理、大数字与轻量比较信息保持清晰层级，不限定具体色相。"
...
tokens:
  application-global:
    appearance:
      ...
      navigation:
        "--pod-shell-theme-bg-color": "#F5F5F5" # neutral-gray；导航浅底，不引入第二色相
        "--pod-nav-item-text-color": "#181818" # neutral-gray；普通菜单文字与图标
        "--pod-nav-item-text-hover-color": "var(--color-brand1-6)" # 保留设计稿主题色文字，关联唯一主色
        "--pod-nav-item-text-selected-color": "var(--color-brand1-6)" # 选中文字与图标沿用设计稿的主题色强调
        "--pod-nav-menu-bg-hover-color": "#EFEFEF" # neutral-gray；中性悬停表面
        "--pod-nav-menu-bg-selected-color": "#FFFFFF" # neutral-gray；选中菜单白面
    colors:
    ...
    rounded:
      ...
      "--corner-circle": 50% # 头像、事件图标底圆和图例圆点
...
浅灰导航包围一块明亮的白色工作区。内容依靠宽松内边距、细灰边框和适度圆角形成清晰分组，普通面板保持平整，数字大而不厚重。存在核心摘要时，用主色的深色派生面集中建立焦点，同一主色的明亮派生色承担关键动作。模板保留深浅对比关系，不限定深青、翠绿或其他具体色相；实际主色由用户或项目流程中的 AI 确定。低对比线框只出现在强调区域，日常表格、表单和说明仍保持朴素。

| 核心特征 | 可见表现 | 主要偏离风险 |
| --- | --- | --- |
| 浅灰框架与白色选中面 | 浅导航配深文字，当前菜单呈白色小面板；内容画布继续保持白色 | 改成深导航，或把内容区铺成大面积灰底而依赖重阴影分层 |
...
默认导航为浅色。六项颜色独立成组：`--pod-shell-theme-bg-color` 为浅灰底，`--pod-nav-item-text-color` 为普通深色菜单文字，悬停用 `--pod-nav-item-text-hover-color` 配 `--pod-nav-menu-bg-hover-color`，选中用 `--pod-nav-item-text-selected-color` 配 `--pod-nav-menu-bg-selected-color`。图标跟随各状态文字颜色。原生导航仅配置这些开放颜色，尺寸、布局、收纳、折叠由平台负责。

项目确定自绘导航时，菜单项使用 `--corner-2`，左侧线性图标与标签对齐；组标题用小号、低强调文字，与菜单之间保留 `--s-2`，组间用 `--s-6` 或 `--s-8` 分开。选中项为白面并可采用前述局部弱阴影，不增加饱和色整行背景。导航内侧留白使用 `--s-3` 或 `--s-4`；尺寸与实际收纳方式只在项目方案中确定。

已有品牌标志以小比例出现在导航顶部，不复制参考材料的商标。已有设置、帮助和身份信息可按产品顺序放在底部，以细线分组；身份块是小白面板，头像为圆形。开关、奖励数或提醒只在对应功能真实存在时出现，不从截图生成新功能。

导航沿用设计稿的浅灰底、白色选中面与主题色文字。悬停和选中文字引用 `var(--color-brand1-6)`；背景与文字的状态配对保持不变，换主题时只更新关联色值。导航弹出层沿用白底深字。
...
全篇只有 `{{PRIMARY_COLOR}}` 一个主色入口，由用户指定或项目流程中的 AI 推导。横幅使用主色加黑得到的深色档，重点动作使用主色加白得到的明亮档；边界、辅助文字和需要带色的中性表面，由同一主色低比例混入无彩灰生成。主题约束明暗、饱和度与使用面积，不锁定截图的深青、翠绿或蓝灰，也不增加独立强调色种子。标准黑白、无彩灰和平台状态色保持各自语义。
...
全局变量用于原生与自定义页面，页面层仅扩展同源强调、焦点表面、数据色和数值排版；扩展的是使用角色，不是主题色来源。按两个作用域递归读取以 `--` 开头的标量叶子，不把分组名称拼入变量名；全局变量不得引用页面层变量。输入、卡片与 pop-up 即使同为白色也保持各自语义引用。
...
- 导航悬停和选中文字引用 `var(--color-brand1-6)`；其他导航色保留无彩色。实例化只解析品牌依赖链，不根据导航整体明暗另选文字色。
...
- [ ] **风格表达：** 浅灰导航、白色细边框面板、圆角分工和主题灰辅助文字成立；图表序列、操作强调和状态色各有明确职责。
