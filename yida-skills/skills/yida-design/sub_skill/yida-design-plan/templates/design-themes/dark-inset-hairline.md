...
默认采用独立的炭黑导航配色，与内容的暗色方向一致，但导航六项颜色独立维护：背景使用 `--pod-shell-theme-bg-color`，普通文字及图标使用 `--pod-nav-item-text-color`，悬停与选中文字分别使用 `--pod-nav-item-text-hover-color`、`--pod-nav-item-text-selected-color`，对应背景分别使用 `--pod-nav-menu-bg-hover-color`、`--pod-nav-menu-bg-selected-color`。选中项以中性底面和更亮文字识别，不铺满品牌色。
...
导航保持设计稿的暗底灰浅字、选中略亮灰底和亮字配对；换主色不自动切换导航明暗，也不改写内容画布、卡片或覆盖层。
...
YAML 是 Token 数值的唯一来源。全局变量供原生及自定义页面使用，页面层仅补充黑色内嵌面、工作区表面、操作前景、明亮中性操作底、数据序列和独立指标字号等缺失语义；递归读取以 `--` 开头的标量叶子，分组名不拼入 CSS 变量名，不重复定义同一变量。全局变量不得反向依赖页面变量。
