---
name: "{{PROJECT_NAME}}"
description: "主题色微染浅灰画布承托平整白色圆角卡片，顶部导航和双行摘要形成清楚层级；少量主题色深色指标面与近黑任务面拉开明暗主次，胶囊操作、细横条和单点明亮数据色补充信息焦点。"
...
tokens:
  application-global:
    appearance:
      ...
      navigation:
        "--pod-shell-theme-bg-color": "var(--color-brand1-3)" # 独立导航角色；主题色极浅变体，与画布连续
        "--pod-nav-item-text-color": "#777777" # neutral-gray；普通菜单文字
        "--pod-nav-item-text-hover-color": "var(--color-brand1-5)" # 主题色深色变体；悬停文字
        "--pod-nav-item-text-selected-color": "var(--color-brand1-5)" # 主题色深色变体；选中文字，通过下划线区分
        "--pod-nav-menu-bg-hover-color": "var(--color-brand1-2)" # 主题色浅填充；悬停背景
        "--pod-nav-menu-bg-selected-color": "var(--color-brand1-3)" # 主题色极浅变体；选中背景与导航连续
    colors:
  ...
  custom-page:
    colors:
      ...
      "--oyd-on-action-color": "#FFFFFF" # neutral-gray；深色胶囊操作前景，不借用基础表面色
      ...
      "--oyd-action-ink": "<生成实际色值：--color-brand1-6 12% + #141414 88%，sRGB 逐通道混合>" # 主题色近黑变体；主要胶囊操作背景
      "--oyd-action-ink-hover": "<生成实际色值：--color-brand1-6 18% + #292929 82%，sRGB 逐通道混合>" # 主题色近黑变体；主要胶囊操作悬停
...
| 胶囊操作与轻量图形 | 近黑主按钮、轮廓次按钮、细圆头横条和小标签 | 按钮、卡片、输入与图形统一成一种圆角和粗细 |
...
导航默认与微染色画布连续。原生导航只配置六项颜色：背景使用 `--pod-shell-theme-bg-color`，普通文字使用 `--pod-nav-item-text-color`，悬停与选中分别使用各自的文字和背景专用变量。图标随文字状态配色，结构、尺寸、折叠与收纳由平台处理，不通过主题额外添加侧栏或入口。
...
已有品牌符号可以置于近黑小圆中，身份头像保持圆形，工具按钮为白底圆形描边控件。只使用真实品牌、身份和功能入口，不复制参考商标或虚构通知数。页面标题和总览摘要属于内容区，不因此增加导航层级。

六项导航颜色保持独立职责，不绑定内容 Text 或 Fill；其中主题色角色直接引用品牌浅深变体，中性文字保留无色相灰。导航切换明暗时成组更新前景和背景，不改变内容与覆盖层表面。

### 2.3 页面标题与操作

普通页面标题使用全局标题规格。页面确有日期、问候或工作总览摘要时，可采用独立展示排版：第一行近黑，第二行用 `--color-text1-3` 降低强调，两行共同形成文字焦点。第二行必须来自真实摘要，不为填满布局增加工作量、成交数或宣传文案。

页首已有操作放在摘要另一侧，以胶囊按钮成组排列，视觉上对齐摘要末行。主要动作使用近黑实底，次要动作用轮廓；同组间隔为 `--s-2` 或 `--s-3`。空间不足时将完整操作组移到文字下方，保持文字自然换行，不强行压缩成单行。
...
品牌色由项目输入确定，承担品牌链接、明确完成量及数据主色；重点面板由种子混入中性暗灰形成，渐变方向约为 135°，限制在该面板内部。近黑任务面板和主要胶囊动作采用更低的品牌占比，保留明暗主次并随品牌换色。
...
深色指标与任务面板只在内容确实需要焦点或优先处理时使用。多个深色对象应有明确主次，普通列表和表单继续使用白色表面；不以深卡数量作为页面完成标准。

### 按钮、链接与输入

主要胶囊操作使用 `--oyd-action-ink` 配 `--oyd-on-action-color`，轮廓次按钮使用所在浅表面与清楚的细边框。页首强调次按钮可用主文字色描边，白卡内部次按钮使用 `--color-line1-2`。品牌文本入口使用品牌深色表达并配明确文字，可带小箭头，不为所有跳转添加实底。
...
YAML 是变量值的唯一事实源，正文规定消费关系。全局变量供原生与自定义页面使用，页面层只补充独立深色内容表面、数据色、展示衬底与数值排版。按作用域递归读取以 `--` 开头的标量叶子，分组名称不拼入变量名；全局变量不得反向依赖页面层变量。
...
- 近黑任务表面、主按钮和数据焦点均随品牌种子生成，前景按实际底面配套。数据色系通过独立角色消费，不引用交互悬停或按下变量；主题色焦点不代替成功状态。平台状态继续按浅、深表面分别配套可读文字。
