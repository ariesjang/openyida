...
tokens:
  application-global:
    appearance:
      ...
      navigation:
        "--pod-shell-theme-bg-color": "#FFFFFF" # 独立纯白导航背景
        "--pod-nav-item-text-color": "#737373"
        "--pod-nav-item-text-hover-color": "#181818"
        "--pod-nav-item-text-selected-color": "#FFFFFF"
        "--pod-nav-menu-bg-hover-color": "var(--color-brand1-3)" # 主题浅填充；导航角色独立，不引用内容 Fill
        "--pod-nav-menu-bg-selected-color": "var(--color-brand1-5)" # 主题深色变体；原生导航使用实色
    colors:
...
默认导航为纯白背景，常态灰字、悬停近黑字，选中项深底白字。六项颜色成组独立配置：`--pod-shell-theme-bg-color` 是导航底；`--pod-nav-item-text-color`、`--pod-nav-item-text-hover-color`、`--pod-nav-item-text-selected-color` 分别对应文字状态；`--pod-nav-menu-bg-hover-color`、`--pod-nav-menu-bg-selected-color` 分别对应状态底。导航图标跟随对应菜单文字。
...
导航弹出菜单回到独立白色覆盖层；其文字使用浅表面前景，避免继承选中菜单的白字。导航保持设计稿的白底灰字、悬停近黑字和选中深底白字；换主色不自动切换这些明暗关系。
...
YAML 是变量值的唯一来源；正文只定义角色和消费关系。全局变量供原生与自定义页面使用，页面层只补充局部强调材质、连续数据编码和独立展示文字等缺失语义。提取时递归读取以双连字符开头的标量叶子，分组名不拼入 CSS 变量名。`--oyd-page-bg` 单向桥接全局页面底；全局 Token 不反向依赖页面层。
