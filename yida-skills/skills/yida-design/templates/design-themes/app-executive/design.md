...
tokens:
  application-global:
    appearance:
      ...
      navigation:
        "--pod-shell-theme-bg-color": "#222826"
        "--pod-nav-item-text-color": "#d9c49a"
        "--pod-nav-item-text-hover-color": "#d9c49a"
        "--pod-nav-item-text-selected-color": "#d9c49a"
        "--pod-nav-menu-bg-hover-color": "#151919"
        "--pod-nav-menu-bg-selected-color": "#151919"
      native-form:
...
桌面业务工作区：四分之一介绍栏与四分之三主工作区。短字段建议 12，字段间距 30px、章节间距 60px、内容内距 40px；这些是项目起点，必须在每张表单设计中按字段长度确定。窄屏与半屏抽屉优先单列，介绍区在字段前面；长文本、附件和子表占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

## 3. 基础组件表达

### 表单、表格与记录集合

单行控件 42px、圆角 4px，边框 1px；字段 labelAlign=top（标签在字段上方）。多行文本、附件、日期范围、子表按自身布局。详情使用相同章节顺序与表面，保留只读语义。表格为有真实数据的紧凑记录集合，不能给每个单元格加独立大卡。按钮、输入、浮层和导航消费同源色彩与形状。
