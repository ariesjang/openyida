...
| 核心特征 | 可见表现 | 主要偏离风险 |
| --- | --- | --- |
| 连贯的深灰材质 | 导航、画布和面板彼此接近，利用微小明度差区分 | 改成纯黑与亮灰强反差、蓝灰玻璃或大面积渐变会改变整体气质 |
...
默认导航为独立炭灰表面：背景 `--pod-shell-theme-bg-color`，普通文字与图标 `--pod-nav-item-text-color`；悬停文字与底色分别使用 `--pod-nav-item-text-hover-color`、`--pod-nav-menu-bg-hover-color`，选中文字与底色分别使用 `--pod-nav-item-text-selected-color`、`--pod-nav-menu-bg-selected-color`。选中项只比导航底色略亮，通过浅色文字和细轮廓识别，不额外提亮成突出灰块，也不用整条高饱和品牌色。

原生导航只配置这六项颜色，结构、尺寸、排列和折叠由平台管理。导航与内容明暗独立确定；本主题壳层使用中性灰，需要主题色时引用相应 Brand Token，保留导航的独立角色。

项目已确定自绘导航时，采用细线图标与紧凑单行文字，图标和文字间隔 `--s-2`，菜单左右内边距 `--s-3`、上下 `--s-2`，圆角 `--corner-2`。分组之间以 `--s-5` 留白和弱横线分隔；分组标题使用 caption 规格，不降到禁用灰。选中项可加微弱上缘高光，主要识别仍来自明亮文字与专用选中背景。
...
YAML 是变量值唯一事实源；全局变量供原生及自定义页消费，页面层只补充抬起正面、高光、独立前景、数据分类色和重点数值排版。递归读取以 `--` 开头的标量叶子，分组名称不拼入变量名；全局不反向引用页面层，不重复定义已有角色。
