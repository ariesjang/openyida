  ...
  application-global:
    appearance:
      ...
      navigation:
        ...
        "--pod-nav-menu-item-selected-shadow": "5px 5px 0 #F07098"
        "--pod-page-header-bg-color": "var(--pod-shell-theme-bg-color)"
        "--pod-nav-l-sub-main-bg-color": "var(--pod-shell-theme-bg-color)"
        "--pod-nav-top-main-border-width": "3px"
        "--pod-nav-top-main-border-color": "#211C21"
        ...
        "--pod-nav-logo-border": "1px solid #211C21"
        ...
        "--pod-nav-search-border-color": "#211C21"
        ...
        "--pod-nav-action-border-color": "#211C21"
        "--pod-nav-action-border": "1px solid #211C21"
        ...
        "--pod-nav-menu-item-border": "3px solid #211C21"
        "--pod-nav-menu-item-hover-border": "3px solid #211C21"
        "--pod-nav-menu-item-selected-border": "3px solid #211C21"
        "--pod-nav-menu-font-size": "14px"
        "--pod-nav-menu-item-selected-font-weight": "700"
        "--pod-nav-menu-line-height": "20px"
        "--pod-nav-menu-gap": "16px"
        "--pod-shell-lshape-border-radius": "0px"
        "--pod-nav-top-tab-height": "44px"
        "--pod-nav-top-tab-item-max-width": "240px"
        "--pod-nav-top-tab-item-padding": "0 20px"
        "--pod-nav-platform-header-height": "68px"
        "--pod-nav-top-header-padding": "10px 20px"
      native-form:
...
明黄导航使用方角粗墨线菜单，选中项黑底黄字并带粉色硬偏移阴影；顶栏为较高的印刷色带，宽菜单留出完整标题，与内容区粗线和硬阴影呼应。
...
导航示例：顶部使用 68px 高的黄色导航带；菜单为 3px 墨黑方框，选中项黑底黄字并带 5px 粉色硬阴影；菜单间距 16px，文字完整显示。 实际导航布局沿用业务规划。
...
桌面业务工作区：不对称二比一工作区与粗线信息带。短字段建议 8:4，字段间距 26px、章节间距 52px、内容内距 32px；这些是项目起点，必须在每张表单设计中按组件树和字段长度确定。窄屏与半屏抽屉按实际可用宽度重排顶部、左侧、主体、右侧及字段间区域，侧栏组件可移到主体前后或折叠为合适的窄屏结构；长文本、附件和子表通常占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

### 2.5 自定义页面设计

自定义页面以 不对称二比一工作区与粗线信息带 组织真实业务内容，以 大字印刷标题与偏置章号 建立标题与首屏焦点，并用 方角、粗墨线、硬偏移阴影 统一页面表面。页面根据 PRD 安排标题与操作区、状态摘要、筛选、主要内容、上下文信息和反馈区；列表、表格、图表、详情抽屉和表单入口只在业务需要时出现。宽度、列数、高度和滚动方式按内容增长确定，移动端按阅读与操作顺序重排。
...
单行控件 46px、圆角 0px，边框 3px；字段 labelAlign=top（标签在字段上方）。多行文本、附件、日期范围、子表按自身布局。详情页沿用相同章节顺序与表面；只读字段的数据框使用 --pod-field-preview-bg-color、--pod-field-preview-border-radius、--pod-field-preview-shadow、--pod-field-preview-indicator-color、--pod-field-preview-text-color、--form-element-medium-font-size、--pod-field-preview-gap、--pod-field-preview-line-height、--pod-field-preview-min-height 和 --pod-field-preview-padding，延续应用的色彩、形状、字体与密度。表格保持紧凑，按钮、输入、浮层和导航消费同源色彩与形状。

### 表单组件与版式结构

表单支持在顶部、左侧、主体、右侧和字段之间放置分栏、Divider、图片/图形、Tab/切换、按钮组/操作入口、状态区和字段。各组件分别承担导航、操作、视觉焦点、反馈、层级、节奏、装饰或采集作用。普通业务分组和章节分隔使用 Divider，横向字段组合使用 ColumnContainer。form-layout.json 提供 8:4 空字段列，columnGap=26px、rowGap=26px、display=VERTICAL；按任务与容器宽度补充组件并调整布局。不因主题名称自动套 3:9，不重复已有标题，不用空泛文案凑版式。
