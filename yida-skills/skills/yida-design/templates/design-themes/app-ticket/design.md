---
name: "{{PROJECT_NAME}}"
description: "窄单据纵向排布与对齐的键值清单；暖纸、双分割线、印章式操作。适合订单、交付与报销。"
themeId: "app-ticket"
tokens:
  application-global:
    appearance:
      ...
      navigation:
        "--pod-shell-theme-bg-color": "#EAD9B4"
        "--pod-nav-item-text-color": "#733C26"
        "--pod-nav-item-text-hover-color": "#572711"
        "--pod-nav-item-text-selected-color": "#542B1B"
        "--pod-nav-menu-bg-hover-color": "#F7EACF"
        "--pod-nav-menu-bg-selected-color": "#FFF9EB"
        "--pod-nav-menu-item-selected-shadow": "4px 4px 0 #9D714C"
        ...
        "--pod-nav-top-main-border-color": "#A7744D"
        ...
        "--pod-nav-logo-border": "3px double #733C26"
        "--pod-nav-logo-border-radius": "0px"
        "--pod-nav-sub-divider-color": "#A7744D"
        ...
        "--pod-nav-l-search-border-color": "#A7744D"
        ...
        "--pod-nav-search-border-color": "#A7744D"
        ...
        "--pod-nav-action-border-color": "#A7744D"
        "--pod-nav-action-border": "1px solid #A7744D"
        ...
        "--pod-nav-menu-item-height": "50px"
        "--pod-nav-menu-item-radius": "0px"
        "--pod-nav-menu-item-border": "5px double #AD8963"
        "--pod-nav-menu-item-hover-border": "5px double #733C26"
        "--pod-nav-menu-item-selected-border": "5px double #542B1B"
        ...
        "--pod-nav-menu-item-padding": "10px 14px"
      ...
      native-form:
        ...
        "--input-bg-color": "#fff2d5"
        "--input-border-width": "1px"
        "--input-border-color": "#a7744d"
        ...
        "--pod-field-preview-bg-color": "#fff2d5"
  ...
  custom-page:
    ...
    "--oyd-radius": "0px"
    "--oyd-content-width": "660px"
    "--oyd-content-padding": "28px"
    "--oyd-field-gap": "16px"
    "--oyd-section-gap": "32px"
...
窄单据纵向排布与对齐的键值清单；暖纸、双分割线、印章式操作。适合订单、交付与报销。
...
米黄纸色导航使用棕墨文字和方角双线菜单框，选中项浅纸底配棕色硬偏移阴影；菜单边框与单据、详情字段的纸张层次呼应。
...
导航示例：米黄色侧栏内使用 5px 棕色双线菜单框；菜单完全方角、行高 50px，选中项浅纸底配棕色偏移阴影，保持墨色正文。普通、悬停、选中边框均为双线，强度逐级提升。 实际导航布局沿用业务规划。
...
票据抬头、单据编号与细密说明。标题字体消费 --oyd-heading-font 与 --oyd-heading-size，辅助说明按正文层级。主操作与当前任务相邻；不堆造虚假标签、指标或章号。

### 2.4 布局和移动端

桌面业务工作区：窄单据纵向排布与对齐的键值清单。短字段建议 12，字段间距 16px、章节间距 32px、内容内距 28px；这些是项目起点，必须在每张表单设计中按组件树和字段长度确定。窄屏与半屏抽屉按实际可用宽度重排顶部、左侧、主体、右侧及字段间区域，侧栏组件可移到主体前后或折叠为合适的窄屏结构；长文本、附件和子表通常占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

### 2.5 自定义页面设计

自定义页面以 窄单据纵向排布与对齐的键值清单 组织真实业务内容，以 票据抬头、单据编号与细密说明 建立标题与首屏焦点，并用 暖纸、双分割线、印章式操作 统一页面表面。页面根据 PRD 安排标题与操作区、状态摘要、筛选、主要内容、上下文信息和反馈区；列表、表格、图表、详情抽屉和表单入口只在业务需要时出现。宽度、列数、高度和滚动方式按内容增长确定，移动端按阅读与操作顺序重排。
...
单行控件 40px、圆角 0px，边框 1px；字段 labelAlign=left（横向标签，长标签换行；窄屏回到顶部）。多行文本、附件、日期范围、子表按自身布局。详情页沿用相同章节顺序与表面；只读字段的数据框使用 --pod-field-preview-bg-color、--pod-field-preview-border-radius、--pod-field-preview-shadow、--pod-field-preview-indicator-color、--pod-field-preview-text-color、--form-element-medium-font-size、--pod-field-preview-gap、--pod-field-preview-line-height、--pod-field-preview-min-height 和 --pod-field-preview-padding，延续应用的色彩、形状、字体与密度。表格保持紧凑，按钮、输入、浮层和导航消费同源色彩与形状。

### 表单组件与版式结构

表单支持在顶部、左侧、主体、右侧和字段之间放置分栏、Divider、图片/图形、Tab/切换、按钮组/操作入口、状态区和字段。各组件分别承担导航、操作、视觉焦点、反馈、层级、节奏、装饰或采集作用。普通业务分组和章节分隔使用 Divider，横向字段组合使用 ColumnContainer。form-layout.json 提供 12 空字段列，columnGap=16px、rowGap=16px、display=VERTICAL；按任务与容器宽度补充组件并调整布局。不因主题名称自动套 3:9，不重复已有标题，不用空泛文案凑版式。
...
暖纸、双分割线、印章式操作。窄单据纵向排布与对齐的键值清单。图表、图片、时间线只为真实内容出现；没有媒体时以排版与章节构图成立，不为“丰富”捏造指标或入口。
