---
name: "{{PROJECT_NAME}}"
description: "密集双列与连续数据表、横向标签；石墨层次、细行线、低圆角。适合明确要求暗色的数据运营。"
themeId: "app-graphite"
tokens:
  application-global:
    appearance:
      surfaces:
        "--pod-app-root-bg-color": "#171d23"
        "--pod-app-root-bg-image": "none"
        "--pod-page-bg-color": "#171d23"
        "--pod-card-bg-color": "#242d35"
        "--pod-table-cell-color": "var(--pod-card-bg-color)"
      navigation:
        "--pod-shell-theme-bg-color": "#191C20"
        "--pod-nav-item-text-color": "#AFB8C2"
        "--pod-nav-item-text-hover-color": "#DFE6ED"
        "--pod-nav-item-text-selected-color": "#F1F5F8"
        "--pod-nav-menu-bg-hover-color": "#292F36"
        "--pod-nav-menu-bg-selected-color": "#3A4653"
        ...
        "--pod-nav-top-main-border-color": "#4C5864"
        ...
        "--pod-nav-logo-border": "1px solid #4C5864"
        "--pod-nav-logo-border-radius": "2px"
        "--pod-nav-sub-divider-color": "#4C5864"
        ...
        "--pod-nav-search-border-color": "#4C5864"
        ...
        "--pod-nav-action-border-color": "#4C5864"
        "--pod-nav-action-border": "1px solid #4C5864"
        ...
        "--pod-nav-menu-item-border": "1px solid #4C5864"
        "--pod-nav-menu-item-hover-border": "1px solid #AFB8C2"
        "--pod-nav-menu-item-selected-border": "1px solid #AFB8C2"
      ...
      native-form:
        ...
        "--input-bg-color": "#242d35"
        "--input-border-width": "1px"
        "--input-border-color": "#485863"
        ...
        "--pod-field-preview-bg-color": "#242d35"
    ...
    colors:
      ...
      "--color-line1-1": "#485863"
      "--color-line1-2": "#485863"
      "--color-fill1-1": "#171d23"
      "--color-fill1-2": "#171d23"
      "--color-fill1-3": "#485863"
...
密集双列与连续数据表、横向标签；石墨层次、细行线、低圆角。适合明确要求暗色的数据运营。
...
蓝石墨导航以低圆角、密集行距和细分隔接续数据表，较亮石墨选中底与浅字保留连续工作区层次。

菜单轮廓：圆角 2px；普通边框 1px solid #4C5864；悬停边框 1px solid #AFB8C2；选中边框 1px solid #AFB8C2。这些值同时作用于侧栏和顶部菜单；保持各状态边框宽度一致，用线型、颜色和选中标记表达状态。
...
小标题、信息计数与细行分隔。标题字体消费 --oyd-heading-font 与 --oyd-heading-size，辅助说明按正文层级。主操作与当前任务相邻；不堆造虚假标签、指标或章号。

### 2.4 布局和移动端

桌面业务工作区：密集双列与连续数据表、横向标签。短字段建议 6:6，字段间距 10px、章节间距 20px、内容内距 24px；这些是项目起点，必须在每张表单设计中按组件树和字段长度确定。窄屏与半屏抽屉按实际可用宽度重排顶部、左侧、主体、右侧及字段间区域，侧栏组件可移到主体前后或折叠为合适的窄屏结构；长文本、附件和子表通常占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

### 2.5 自定义页面设计

自定义页面以 密集双列与连续数据表、横向标签 组织真实业务内容，以 小标题、信息计数与细行分隔 建立标题与首屏焦点，并用 石墨层次、细行线、低圆角 统一页面表面。页面根据 PRD 安排标题与操作区、状态摘要、筛选、主要内容、上下文信息和反馈区；列表、表格、图表、详情抽屉和表单入口只在业务需要时出现。宽度、列数、高度和滚动方式按内容增长确定，移动端按阅读与操作顺序重排。
...
单行控件 36px、圆角 4px，边框 1px；字段 labelAlign=left（横向标签，长标签换行；窄屏回到顶部）。多行文本、附件、日期范围、子表按自身布局。详情页沿用相同章节顺序与表面；只读字段的数据框使用 --pod-field-preview-bg-color、--pod-field-preview-border-radius、--pod-field-preview-shadow、--pod-field-preview-indicator-color、--pod-field-preview-text-color、--form-element-medium-font-size、--pod-field-preview-gap、--pod-field-preview-line-height、--pod-field-preview-min-height 和 --pod-field-preview-padding，延续应用的色彩、形状、字体与密度。表格保持紧凑，按钮、输入、浮层和导航消费同源色彩与形状。

### 表单组件与版式结构

表单支持在顶部、左侧、主体、右侧和字段之间放置分栏、Divider、图片/图形、Tab/切换、按钮组/操作入口、状态区和字段。各组件分别承担导航、操作、视觉焦点、反馈、层级、节奏、装饰或采集作用。普通业务分组和章节分隔使用 Divider，横向字段组合使用 ColumnContainer。form-layout.json 提供 6:6 空字段列，columnGap=10px、rowGap=10px、display=VERTICAL；按任务与容器宽度补充组件并调整布局。不因主题名称自动套 3:9，不重复已有标题，不用空泛文案凑版式。
...
石墨层次、细行线、低圆角。密集双列与连续数据表、横向标签。图表、图片、时间线只为真实内容出现；没有媒体时以排版与章节构图成立，不为“丰富”捏造指标或入口。
