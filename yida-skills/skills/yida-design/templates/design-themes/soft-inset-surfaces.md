---
name: "{{PROJECT_NAME}}"
description: "以主题色轻染的浅灰外衬、近白画布和柔圆白面板构成低对比分层；浅灰导航以中性底色标记选中，细线图标、浅色圆形标记与局部主题色强调组织阅读，浮层以独立白面和克制投影浮起。"
...
浅灰应用外衬包围柔圆的内容区域，导航略深于近白画布，白面板以留白、极淡边界和轻微底影分层。界面保持中等信息密度：标题短而清晰，主要数值用深灰突出，辅助文字退后；色彩集中于小图标底盒、短标签和数据图形。普通操作保留圆角矩形，圆形与胶囊只用于有相应用途的小元素。

| 核心特征 | 可见表现 | 主要偏离风险 |
| --- | --- | --- |
| 浅色表面的细微层差 | 外衬、导航、画布和白面板依次分离，面板看起来嵌在宽松的浅色底面中 | 所有底面同白会丢失分区；整页加深投影会使面板过度浮起 |
| 独立的中性导航 | 普通菜单为灰色，默认侧栏以淡灰底和深色文字表示选中；自定义导航按形态选择协调的中性标记，品牌色只在少量识别元素出现 | 把选中菜单铺成高饱和品牌色，或把内容区一并染成导航色 |
...
导航保持浅灰表面，与白色内容面板通过色差和细边界分开。原生导航只应用下列六项独立配色，结构、尺寸、折叠与收纳均沿用平台：

| 导航角色 | 专用变量 |
| --- | --- |
| 壳层背景 | `--pod-shell-theme-bg-color` |
| 普通菜单文字及图标 | `--pod-nav-item-text-color` |
| 悬停文字及图标 | `--pod-nav-item-text-hover-color` |
| 选中文字及图标 | `--pod-nav-item-text-selected-color` |
| 悬停背景 | `--pod-nav-menu-bg-hover-color` |
| 选中背景 | `--pod-nav-menu-bg-selected-color` |
...
自绘导航共用尺度：行内图标与文字间距为 `--s-2`，菜单行高可取 `--s-9` 或 `--s-10`，左右内边距为 `--s-3`；分组间距用 `--s-6`，分组名用 caption 文字。此处尺度为推断。选中项文字加深，图标可实心强调，普通项保持细线。已有子级入口用缩进与浅色连接线表达；不要为复制构图增加分组、入口、推广区或模式开关。导航与内容之间的边界按所选容器关系确定。
...
独立识别图标可放入直径 `--s-8` 或 `--s-9` 的圆底；有更强识别需求的记录可用 `--s-10`。图标使用语义前景，底色按该前景 8% 与白色 92% 的 sRGB 通道混合生成，属于组件局部材质规则。语义来自品牌、真实类别或平台状态，不随机配色。圆底不使用强阴影，至多采用 `--oyd-shadow-resting`。
...
YAML 是变量值的唯一事实源，正文定义消费关系。全局层供应用外观与内容组件使用；自定义页仅以 `--oyd-page-bg` 桥接背景，并补充独立的核心数值、深色操作、分类数据色与局部弱阴影。递归读取以 `--` 开头的标量叶子，分组名不拼入变量名，不重复定义或跨层覆盖。全局不反向依赖自定义页变量。
