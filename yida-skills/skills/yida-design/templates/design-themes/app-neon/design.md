...
tokens:
  application-global:
    appearance:
      ...
      navigation:
        "--pod-shell-theme-bg-color": "#111b31"
        "--pod-nav-item-text-color": "#b4f5f1"
        "--pod-nav-item-text-hover-color": "#b4f5f1"
        "--pod-nav-item-text-selected-color": "#b4f5f1"
        "--pod-nav-menu-bg-hover-color": "#080f21"
        "--pod-nav-menu-bg-selected-color": "#080f21"
      native-form:
...
桌面业务工作区：横向监控总览与双列任务区。短字段建议 6:6，字段间距 22px、章节间距 44px、内容内距 32px；这些是项目起点，必须在每张表单设计中按字段长度确定。窄屏与半屏抽屉优先单列，介绍区在字段前面；长文本、附件和子表占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。
