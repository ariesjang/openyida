---
name: "{{PROJECT_NAME}}"
description: "密集双列、水平标签与对齐金额；淡蓝图纸、清晰细边界、低圆角。适合预算、风险与资金计划。"
themeId: "app-finance"
tokens:
  application-global:
    appearance:
      surfaces:
        "--pod-app-root-bg-color": "#edf2f7"
        "--pod-app-root-bg-image": "none"
        "--pod-page-bg-color": "#edf2f7"
      ...
      navigation:
        ...
        "--pod-nav-item-text-hover-color": "#274e78"
        "--pod-nav-item-text-selected-color": "#274e78"
        "--pod-nav-menu-bg-hover-color": "#edf2f7"
        "--pod-nav-menu-bg-selected-color": "#edf2f7"
      native-form:
    ...
    colors:
      ...
      "--color-line1-1": "#c4d1df"
      "--color-line1-2": "#c4d1df"
      "--color-fill1-1": "#edf2f7"
      "--color-fill1-2": "#edf2f7"
      "--color-fill1-3": "#c4d1df"
      "--color-fill1-5": "#ffffff"
      "--color-fill1-10": "#262626"
      "--color-text1-5": "#FFFFFF"
      "--color-text1-4": "#274e78"
      "--color-text1-10": "#274e78"
      "--color-text1-3": "#274e78"
      "--color-text1-2": "#858781"
      "--color-text1-1": "#c4d1df"
    typography:
...
密集双列、水平标签与对齐金额。淡蓝图纸、清晰细边界、低圆角。画布消费 --oyd-page-bg，内容消费 --oyd-surface，文字消费 --oyd-ink，边界消费 --oyd-border。内容最大宽度 --oyd-content-width，内距 --oyd-content-padding；宽屏不放大成空白 KPI 卡。
...
技术抬头与编号章节。标题字体消费 --oyd-heading-font 与 --oyd-heading-size，辅助说明按正文层级。主操作与当前任务相邻；不堆造虚假标签、指标或章号。

### 2.4 布局和移动端

桌面业务工作区：密集双列、水平标签与对齐金额。短字段建议 6:6，字段间距 16px、章节间距 32px、内容内距 28px；这些是项目起点，必须在每张表单设计中按字段长度确定。窄屏与半屏抽屉优先单列，介绍区在字段前面；长文本、附件和子表占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

## 3. 基础组件表达

### 表单、表格与记录集合

单行控件 36px、圆角 4px，边框 1px；字段 labelAlign=left（横向标签，长标签换行；窄屏回到顶部）。多行文本、附件、日期范围、子表按自身布局。详情使用相同章节顺序与表面，保留只读语义。表格为有真实数据的紧凑记录集合，不能给每个单元格加独立大卡。按钮、输入、浮层和导航消费同源色彩与形状。

### 原生结构与介绍区

原生表单先按填写任务决定是否需要介绍，不直接复制应用首页的构图。form-layout.json 只提供 6:6 字段分栏，不预置介绍栏或占位标题；columnGap=16px、rowGap=16px、display=VERTICAL。简单新增/编辑可直接从字段开始；一句必要提示放顶部或字段旁；多步骤任务可用步骤提示；只有独立且需持续参考的材料清单、规则或上下文，加上足够的容器宽度，才考虑侧栏。不能因主题名称自动套 3:9，不能让介绍重复弹窗标题或首组标题。介绍与章节可用 Divider.title/description，业务文案不放 CSS content；无需说明时直接省略，不注入 HTML/JS。
...
技术抬头与编号章节是有相应内容时的可选构图，不是每张表单的固定结构。应用首页、详情与表单继承字体、材质和标题语法，不要求同一版式。先判断无介绍、顶部短说明、分组内提示、步骤提示或侧栏哪一种能帮助当前任务；不得为多样性随机轮换。原生 form-layout.json 仅含字段分栏；自定义页面可用 .oyd-style-intro 与 .oyd-style-section，在 .oyd-style-workspace 内按任务组织 .oyd-style-layout。侧栏需在 design.md 写清独立内容、持续参考价值和容器宽度依据；抽屉空间不足时移到顶部或省略，不压缩字段。伪元素只画装饰，不承载文案。

### R2 材质与信息密度

淡蓝图纸、清晰细边界、低圆角。密集双列、水平标签与对齐金额。图表、图片、时间线只为真实内容出现；没有媒体时以排版与章节构图成立，不为“丰富”捏造指标或入口。
