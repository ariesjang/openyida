  ...
  application-global:
    appearance:
      ...
      navigation:
        ...
        "--pod-nav-logo-border-radius": "12px"
        ...
        "--pod-nav-popup-border-radius": "12px"
        ...
        "--pod-shell-lshape-border-radius": "12px"
    colors:
...
浅色导航以柔圆菜单和轻染色搜索区延续内容面板，当前入口采用深色底与白字形成局部焦点。

菜单轮廓：圆角 20px；普通边框 none；悬停边框 none；选中边框 none。这些值同时作用于侧栏和顶部菜单；保持各状态边框宽度一致，用线型、颜色和选中标记表达状态。
