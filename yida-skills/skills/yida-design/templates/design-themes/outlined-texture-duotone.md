  ...
  application-global:
    appearance: # 应用外观：应用背景、内容表面与导航配色，共 11 个变量
      ...
      navigation: # 导航独立配色；六项成组确定，不引用内容区色值；以下为独立深色导航配套值
        "--pod-shell-theme-bg-color": "#141414" # neutral-gray；独立导航背景，与内容画布同明度但职责分开
        "--pod-nav-item-text-color": "#AAAAAA" # neutral-gray；普通菜单及图标
        "--pod-nav-item-text-hover-color": "#DDDDDD" # neutral-gray；悬停文字
        "--pod-nav-item-text-selected-color": "#F2F2F2" # neutral-gray；选中文字
        "--pod-nav-menu-bg-hover-color": "#191919" # neutral-gray；悬停底色
        "--pod-nav-menu-bg-selected-color": "#141414" # neutral-gray；选中底色保持近似壳层，以双层轮廓和亮字识别
...
导航的品牌标识和实际存在的辅助说明区分别表达，不因样式新增推广入口、权限入口或消息功能。导航弹出层使用 `--color-fill1-5` 与内容前景；导航切换明暗只成组调整六项颜色，不改写内容文字与填充。
