---
name: "{{PROJECT_NAME}}"
description: "窄幅单列、连续正文与留白章节；无浮起卡片、平面纸白、强调下划线。适合申请、访谈与简洁审批。"
themeId: "app-line"
tokens:
  application-global:
    appearance:
      surfaces:
        "--pod-app-root-bg-color": "#f2f1ee"
        "--pod-app-root-bg-image": "none"
        "--pod-page-bg-color": "#f2f1ee"
        "--pod-card-bg-color": "#ffffff"
        "--pod-table-cell-color": "var(--pod-card-bg-color)"
      navigation:
        "--pod-shell-theme-bg-color": "#F4F2EE"
        "--pod-nav-item-text-color": "#57524C"
        "--pod-nav-item-text-hover-color": "#242323"
        "--pod-nav-item-text-selected-color": "#242323"
        "--pod-nav-menu-bg-hover-color": "#EDEAE4"
        "--pod-nav-menu-bg-selected-color": "#E7E2D9"
        ...
        "--pod-nav-top-main-border-color": "#AAA5A0"
        ...
        "--pod-nav-logo-border": "1px solid #AAA5A0"
        "--pod-nav-logo-border-radius": "0px"
        "--pod-nav-sub-divider-color": "#AAA5A0"
        ...
        "--pod-nav-l-search-border-color": "#AAA5A0"
        ...
        "--pod-nav-search-border-color": "#AAA5A0"
        ...
        "--pod-nav-action-border-color": "#AAA5A0"
        "--pod-nav-action-border": "1px solid #AAA5A0"
      ...
      native-form:
        ...
        "--pod-form-label-color": "#242323"
        "--form-top-label-margin-b": "8px"
        "--yida-form-content-bgcolor": "#ffffff"
        "--pod-page-content-max-width": "680px"
        ...
        "--pod-formView-stickyFooter-bg-color": "#f2f1ee"
        "--pod-formView-stickyFooter-box-shadow": "none"
        "--pod-formView-stickyFooter-border": "none"
        "--pod-formView-stickyFooter-border-top": "1px solid #aaa5a0"
        ...
        "--pod-field-preview-text-color": "#242323"
    ...
    colors:
      ...
      "--color-line1-1": "#aaa5a0"
      "--color-line1-2": "#aaa5a0"
      "--color-fill1-1": "#f2f1ee"
      "--color-fill1-2": "#f2f1ee"
      "--color-fill1-3": "#aaa5a0"
      "--color-fill1-5": "#ffffff"
      "--color-fill1-10": "#262626"
      "--color-text1-5": "#FFFFFF"
      "--color-text1-4": "#242323"
      "--color-text1-10": "#242323"
      "--color-text1-3": "#242323"
      "--color-text1-2": "#858781"
      "--color-text1-1": "#aaa5a0"
    typography:
  ...
  custom-page:
    ...
    "--oyd-radius": "0px"
    "--oyd-content-width": "680px"
    "--oyd-content-padding": "48px"
    "--oyd-field-gap": "38px"
    "--oyd-section-gap": "76px"
...
窄幅单列、连续正文与留白章节；无浮起卡片、平面纸白、强调下划线。适合申请、访谈与简洁审批。
...
窄幅单列、连续正文与留白章节。无浮起卡片、平面纸白、强调下划线。画布消费 --oyd-page-bg，内容消费 --oyd-surface，文字消费 --oyd-ink，边界消费 --oyd-border。内容最大宽度 --oyd-content-width，内距 --oyd-content-padding；宽屏不放大成空白 KPI 卡。

### 2.2 应用导航

暖白导航沿用纯文字和下划线节奏，菜单保持直角与宽行距，浅底选中项和细指示线延续单列阅读。

菜单轮廓：圆角 0px；普通边框 none；悬停边框 none；选中边框 none。这些值同时作用于侧栏和顶部菜单；保持各状态边框宽度一致，用线型、颜色和选中标记表达状态。
...
纯文字标题与短红色侧线。标题字体消费 --oyd-heading-font 与 --oyd-heading-size，辅助说明按正文层级。主操作与当前任务相邻；不堆造虚假标签、指标或章号。

### 2.4 布局和移动端

桌面业务工作区：窄幅单列、连续正文与留白章节。短字段建议 12，字段间距 38px、章节间距 76px、内容内距 48px；这些是项目起点，必须在每张表单设计中按组件树和字段长度确定。窄屏与半屏抽屉按实际可用宽度重排顶部、左侧、主体、右侧及字段间区域，侧栏组件可移到主体前后或折叠为合适的窄屏结构；长文本、附件和子表通常占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

### 2.5 自定义页面设计

自定义页面以 窄幅单列、连续正文与留白章节 组织真实业务内容，以 纯文字标题与短红色侧线 建立标题与首屏焦点，并用 无浮起卡片、平面纸白、强调下划线 统一页面表面。页面根据 PRD 安排标题与操作区、状态摘要、筛选、主要内容、上下文信息和反馈区；列表、表格、图表、详情抽屉和表单入口只在业务需要时出现。宽度、列数、高度和滚动方式按内容增长确定，移动端按阅读与操作顺序重排。
...
单行控件 48px、圆角 0px，边框 1px；字段 labelAlign=top（标签在字段上方）。多行文本、附件、日期范围、子表按自身布局。详情页沿用相同章节顺序与表面；只读字段的数据框使用 --pod-field-preview-bg-color、--pod-field-preview-border-radius、--pod-field-preview-shadow、--pod-field-preview-indicator-color、--pod-field-preview-text-color、--form-element-medium-font-size、--pod-field-preview-gap、--pod-field-preview-line-height、--pod-field-preview-min-height 和 --pod-field-preview-padding，延续应用的色彩、形状、字体与密度。表格保持紧凑，按钮、输入、浮层和导航消费同源色彩与形状。

### 表单组件与版式结构

表单支持在顶部、左侧、主体、右侧和字段之间放置分栏、Divider、图片/图形、Tab/切换、按钮组/操作入口、状态区和字段。各组件分别承担导航、操作、视觉焦点、反馈、层级、节奏、装饰或采集作用。普通业务分组和章节分隔使用 Divider，横向字段组合使用 ColumnContainer。form-layout.json 提供 12 空字段列，columnGap=38px、rowGap=38px、display=VERTICAL；按任务与容器宽度补充组件并调整布局。不因主题名称自动套 3:9，不重复已有标题，不用空泛文案凑版式。
...
无浮起卡片、平面纸白、强调下划线。窄幅单列、连续正文与留白章节。图表、图片、时间线只为真实内容出现；没有媒体时以排版与章节构图成立，不为“丰富”捏造指标或入口。
