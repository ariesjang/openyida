---
name: "{{PROJECT_NAME}}"
description: "紧凑双列与可扫描的日志清单；像素直角、点线边界、等宽排版。适合明确要求终端风格的技术工具。"
themeId: "app-terminal"
tokens:
  application-global:
    appearance:
      surfaces:
        "--pod-app-root-bg-color": "#0e1813"
        "--pod-app-root-bg-image": "none"
        "--pod-page-bg-color": "#0e1813"
        "--pod-card-bg-color": "#15231a"
        "--pod-table-cell-color": "var(--pod-card-bg-color)"
      navigation:
        "--pod-shell-theme-bg-color": "#09150F"
        "--pod-nav-item-text-color": "#9BCDA5"
        "--pod-nav-item-text-hover-color": "#C5F4CB"
        "--pod-nav-item-text-selected-color": "#D2FFD9"
        "--pod-nav-menu-bg-hover-color": "#203627"
        "--pod-nav-menu-bg-selected-color": "#203D2B"
        "--pod-nav-menu-item-selected-shadow": "inset 5px 0 0 #9FE5AD, inset -5px 0 0 #9FE5AD"
        "--pod-page-header-bg-color": "var(--pod-shell-theme-bg-color)"
        "--pod-nav-l-sub-main-bg-color": "#101E15"
        "--pod-nav-top-main-border-width": "2px"
        "--pod-nav-top-main-border-color": "#6BAC7B"
        ...
        "--pod-nav-search-border-color": "#558966"
        ...
        "--pod-nav-action-border-color": "#558966"
        "--pod-nav-action-border": "1px solid #558966"
        ...
        "--pod-nav-menu-item-border": "1px dashed #42654C"
        "--pod-nav-menu-item-hover-border": "1px dashed #9FE5AD"
        "--pod-nav-menu-item-selected-border": "1px dashed #9FE5AD"
    ...
    colors:
      ...
      "--color-line1-1": "#558966"
      "--color-line1-2": "#558966"
      "--color-fill1-1": "#0e1813"
      "--color-fill1-2": "#0e1813"
      "--color-fill1-3": "#558966"
      "--color-fill1-5": "#15231a"
      "--color-fill1-10": "#262626"
      "--color-text1-5": "#FFFFFF"
      "--color-text1-4": "#9fe5ad"
      "--color-text1-10": "#9fe5ad"
      "--color-text1-3": "#9fe5ad"
      "--color-text1-2": "#7f8986"
      "--color-text1-1": "#558966"
    typography:
  ...
  custom-page:
    ...
    "--oyd-radius": "0px"
    "--oyd-content-width": "960px"
    "--oyd-content-padding": "24px"
    "--oyd-field-gap": "12px"
    "--oyd-section-gap": "24px"
    "--oyd-heading-font": "'SFMono-Regular', Consolas, monospace"
    "--oyd-heading-size": "32px"
    "--oyd-rule-style": "dotted"
...
紧凑双列与可扫描的日志清单；像素直角、点线边界、等宽排版。适合明确要求终端风格的技术工具。
...
紧凑双列与可扫描的日志清单。像素直角、点线边界、等宽排版。画布消费 --oyd-page-bg，内容消费 --oyd-surface，文字消费 --oyd-ink，边界消费 --oyd-border。内容最大宽度 --oyd-content-width，内距 --oyd-content-padding；宽屏不放大成空白 KPI 卡。

### 2.2 应用导航

深绿导航使用紧凑方角和虚线菜单，选中项保持暗底亮字，左右内阴影形成终端括号；L 形框架分清顶栏与侧栏，等宽日志延续相同节奏。
...
导航示例：顶部与侧栏组成 L 形深绿框架；侧栏菜单 36px 紧凑行高、零圆角、1px 虚线；选中项保留深色底，亮绿文字，左右各一条 5px 内阴影形成终端括号。 实际导航布局沿用业务规划。
...
命令行式短标题与流程提示。标题字体消费 --oyd-heading-font 与 --oyd-heading-size，辅助说明按正文层级。主操作与当前任务相邻；不堆造虚假标签、指标或章号。

### 2.4 布局和移动端

桌面业务工作区：紧凑双列与可扫描的日志清单。短字段建议 6:6，字段间距 12px、章节间距 24px、内容内距 24px；这些是项目起点，必须在每张表单设计中按组件树和字段长度确定。窄屏与半屏抽屉按实际可用宽度重排顶部、左侧、主体、右侧及字段间区域，侧栏组件可移到主体前后或折叠为合适的窄屏结构；长文本、附件和子表通常占整行。不把屏幕断点当 iframe 宽度，不用 fixed 卡片截断自然内容。

### 2.5 自定义页面设计

自定义页面以 紧凑双列与可扫描的日志清单 组织真实业务内容，以 命令行式短标题与流程提示 建立标题与首屏焦点，并用 像素直角、点线边界、等宽排版 统一页面表面。页面根据 PRD 安排标题与操作区、状态摘要、筛选、主要内容、上下文信息和反馈区；列表、表格、图表、详情抽屉和表单入口只在业务需要时出现。宽度、列数、高度和滚动方式按内容增长确定，移动端按阅读与操作顺序重排。
...
单行控件 40px、圆角 0px，边框 1px；字段 labelAlign=top（标签在字段上方）。多行文本、附件、日期范围、子表按自身布局。详情页沿用相同章节顺序与表面；只读字段的数据框使用 --pod-field-preview-bg-color、--pod-field-preview-border-radius、--pod-field-preview-shadow、--pod-field-preview-indicator-color、--pod-field-preview-text-color、--form-element-medium-font-size、--pod-field-preview-gap、--pod-field-preview-line-height、--pod-field-preview-min-height 和 --pod-field-preview-padding，延续应用的色彩、形状、字体与密度。表格保持紧凑，按钮、输入、浮层和导航消费同源色彩与形状。

### 表单组件与版式结构

表单支持在顶部、左侧、主体、右侧和字段之间放置分栏、Divider、图片/图形、Tab/切换、按钮组/操作入口、状态区和字段。各组件分别承担导航、操作、视觉焦点、反馈、层级、节奏、装饰或采集作用。普通业务分组和章节分隔使用 Divider，横向字段组合使用 ColumnContainer。form-layout.json 提供 6:6 空字段列，columnGap=12px、rowGap=12px、display=VERTICAL；按任务与容器宽度补充组件并调整布局。不因主题名称自动套 3:9，不重复已有标题，不用空泛文案凑版式。
...
像素直角、点线边界、等宽排版。紧凑双列与可扫描的日志清单。图表、图片、时间线只为真实内容出现；没有媒体时以排版与章节构图成立，不为“丰富”捏造指标或入口。
