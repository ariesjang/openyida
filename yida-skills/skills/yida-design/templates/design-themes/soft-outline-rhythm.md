...
导航沿用项目已确定的原生或自绘方式，以及顶部、侧边或 L 型结构。

**导航壳层配色**

导航与内容区分别确定配色，可以采用深色导航与浅色内容。以下六个导航变量使用独立色值，不引用内容区的文字或填充变量；生成项目时，根据导航背景成组确定文字、悬停与选中配色。

下表展示模板默认的浅色导航。原生导航的壳层配色使用这六个变量；尺寸、菜单排布和折叠方式由平台处理。

| 对象或状态 | 视觉表现（模板默认） | 使用变量 |
| --- | --- | --- |
| 导航背景 | 默认近白底色，视觉弱于正文；项目生成时单独确定色值 | `--pod-shell-theme-bg-color` |
| 普通菜单文字 | 较弱的文字色 | `--pod-nav-item-text-color` |
| 鼠标悬停的菜单 | 背景略微加深，文字更清楚 | 背景 `--pod-nav-menu-bg-hover-color`，文字 `--pod-nav-item-text-hover-color` |
| 当前选中的菜单 | 浅灰背景，较深文字 | 背景 `--pod-nav-menu-bg-selected-color`，文字 `--pod-nav-item-text-selected-color` |

**自绘导航的补充规则**

仅在项目已确定自行绘制导航时使用：

- 壳层背景与其中的菜单颜色沿用上表；导航与正文之间用 `--color-line1-1` 细线分隔。图标尺寸用 `--s-5`，颜色跟随菜单文字，线型沿用 2.5。
...
- 应用根背景用于兜底；导航壳层、页面画布、卡片和 pop-up 分别使用各自的背景变量，不因导航换色而联动。`--oyd-page-bg` 单向引用 `--pod-page-bg-color`，全局变量不反向依赖页面变量。
