...
搭建模式选项平等说明，不推荐、不预选，不添加“（推荐）”或“默认”。详细计划问题的“是”对应 Plan，“否”对应 Fast；两者都复用已有需求。用户明确要求代为决定模式时，记录选择和依据。

### 导航设计

Fast 与 Plan 均由 AI 根据业务场景确定导航方式、布局和呈现样式。

1. 用户已明确要求宜搭原生导航、自定义导航或具体布局时优先沿用；只说“平台导航”视为宜搭原生导航，只指定顶部、侧边或 L 型时保留布局，由 AI 补齐导航方式。后续阶段沿用同一决策，不因默认规则覆盖用户要求。
2. 已有业务应用的局部增改沿用现有导航；显式窄范围遵守上文执行边界，不因 AI 判断扩大资源范围或修改应用设置。
3. 新建完整应用以原生表单、审批、数据管理和多模块业务操作为主，且平台导航能满足入口组织时，采用宜搭原生导航。需要在同一自定义页面中持续保留菜单和内容状态，或业务要求品牌化菜单、特殊导航交互时，可采用自定义导航，并在 `reason` 写明需求依据。仅出现“门户 / 看板 / 首页”或页面内 tab 不足以判定需要自定义应用导航；信息不足以支持自绘时采用宜搭原生导航。
4. 将导航方式、布局和判断依据写入 brief，交给 PRD 与视觉设计共用。Plan 将导航方案纳入整体方案展示和最终确认；Fast 按已记录的判断继续实施。
...
这些是布局判断依据，不是固定模板或用户问卷；不得为适配布局增删业务模块，也不能因布局偏好改变已选导航归属。自定义形态细化参考 [导航壳形态目录](../../yida-nav-shell/references/nav-shell-patterns.md)。自定义顶部未指定样式时默认浮导；侧边及顶部＋侧边在 PRD/design 中写入折叠/展开和拖拽调宽。
...
- `navigation` 记录 `type/source/reason`；用户明确指定导航方式时 `source` 为 `user_selected`；AI 判断方式时标记 `ai_default`。`reason` 区分用户明确要求与 AI 对方式、布局的判断依据，不将推断记成用户指定。保存 confirmed brief 前按上表补齐可执行的布局枚举，`type` 使用精确枚举；自定义导航的 `variant` 为 `side/top/mixed/dock`。顶部浮导使用 `top`，默认浮导或用户指定的通栏样式写入 `reason`，PRD 与视觉设计共同沿用；`dock` 表示底部悬浮胶囊。导航明暗由视觉选择记录。
...
```json
{
  ...
  "navigationStyle": {"structure": "side", "tone": "light", "source": "ai_default", "selectionReason": "以原生表单操作为主且模块分组较多，AI 采用平台侧边布局与浅色导航"}
}
```
