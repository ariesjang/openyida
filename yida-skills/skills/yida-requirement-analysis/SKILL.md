...
| `navigation` | AI 按场景判断导航方式与布局，用户明确要求优先。判断规则见 [整理需求事实](workflow/prepare-brief.md#导航设计)，`source` 区分 `ai_default` 与 `user_selected`。应用导航决策：`type` 只能是 `platform-l-shape/platform-top/platform-side/custom`，并记录 `source/reason`；自定义导航增加 `variant: side/top/mixed/dock`；未决时 type 为 null，规划前补齐 |
