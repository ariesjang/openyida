...
| 当前结果 | 状态事实 | 下一动作 |
| --- | --- | --- |
| background pending | 原任务仍在执行，task/state/lock 保持为同一操作 | 等待运行时投递完成结果 |
...
`url` 是兼容字段，与表单入口 `formUrl` 等价；`appUrl` 表示整个应用入口。普通表单在创建保存成功后进入可用状态；`publish-page` 对应自定义展示页的发布生命周期。
...
`resume` 的保存端 HTTP 5xx 恢复流程固定为：精确回读目标表单；目标字段已存在时收口为成功，目标字段缺失且无冲突时绑定最新服务端 revision 执行一次保存，再做最终回读。成功 JSON 中的真实 `formUuid` 和 `url` 构成交付证据；用户要求资源 ID 时，将已验证 `formUuid` 写入终态 artifact 的可见 `description`。
