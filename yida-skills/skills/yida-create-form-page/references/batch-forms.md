...
- 当前批次返回 `success:false` 时，`results`、子命令诊断和 `.state.json` 共同构成恢复事实。`recoveryAction=rerun_unchanged_plan` 表示使用原任务文件和原参数恢复同一 fingerprint；CLI 从 state 读取已知 ID，并继续未完成项。
- `recoveryAction=inspect_unknown_write_then_reconcile` 表示至少一个写结果缺少资源 ID。先回读远端资源，再创建新的 reconcile 任务文件：已确认的完整表单携带 `formUuid` 进入复用路径，确认尚未创建的表单保留创建任务。
- `.lock` 防止同一任务重复启动。进程异常退出遗留锁时，确认原进程已结束、核对已创建资源后再清理。
- 首次 batch 返回 background pending 时，当前执行单元保持运行中，由运行时投递最终结果；后续动作以该最终结果为准。
