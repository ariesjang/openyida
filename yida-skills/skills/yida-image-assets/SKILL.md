...
- 同一候选尝试一次。图片失效、尺寸不合适或来源信息获取失败时，直接换下一张。
- 来源出现 401、403、429、反爬挑战或超时，标记为本轮不可用，当前及后续槽位立即换其他来源。失败判断按来源记录；换工具、请求头或页面地址仍计入同一来源。
- 候选用尽后，`generationAllowed=true` 且允许示意图的槽位可用宿主生图；可选槽位采用设计允许的无图布局；必需真实图片保留缺口，继续已就绪页面。
- 优先按场景批量搜索，选取图片、摄影师及许可信息齐全的候选，一批查看后分配到槽位。每个槽位保留一个首选和一个备选，首选可用即结束选图。
...
- 退出码 `2`、`ASSET_MATERIAL_NOT_FINAL`：清单已写入，读取 `gaps`；下载失败就换 `input`，登录或上传能力恢复后再处理对应项。其他错误先修参数或输入文件。
- 缺原图就补 `input`，尺寸不够就换图；来源信息优先复用已取得的真实记录，获取失败则换候选。最终状态由 CLI 校验产生。

替换失败素材或恢复所需能力后，再处理清单：
