...
| 新增/提交入口 | <PC 侧边抽屉 iframe 承载页面级隐藏导航的 `submission/{formUuid}?isRenderNav=false`，抽屉默认半屏 `50vw`；移动端整页或新页打开；仅全应用自定义导航且计划明确要求时用 `update-form-config` 持久化；混合前后台不为嵌入效果修改共用表单> |
...
前后台均指访问态。按 [访问态入口契约](../../yida-app/references/entry-navigation.md) 分别记录菜单、默认落点和资源/视图/操作权限；管理端可直接进入原生列表，不要求首页。下方资源和页面表格均按有效范围裁剪。
...
| 分组 | 页面顺序 | 导航呈现 | 放置原则 |
| --- | --- | --- | --- |
| 默认管理任务 | <高频管理列表 / 明确需要的管理概览> | <平台导航> | 按管理端默认任务排序，独立前台不自动置顶 |
