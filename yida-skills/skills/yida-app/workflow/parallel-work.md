...
| 图片要求就绪（Plan 已确认） | 搜图与创建应用、表单、无图页面同时推进；同页各图片位置也并发搜索 | 每页草稿就绪即检查直链；上传时使用真实 appType；图片只在所属页面接图和验收时汇合 |
...
1. Plan 当前方案确认后，复用已有素材任务，按物化结果的 `assetTasks` 补充尚未启动的任务；Fast 从 `design.md.assetStrategy` 建立同样任务。每项给出 pageId、图片位置、独立输出路径和完成条件，主流程立即继续创建应用、表单和无图页面。
...
4. 无图页面照常开发；有图页面先做布局、文字和交互，真实图片到位后接入并验收。页面任务先接收已启动素材任务的结果，剩余缺口按原轮次继续。
5. 记录搜索请求的开始、结束，以及业务任务的开始时间。检查同页独立搜索确有时间重叠，且其他业务在素材结束前已开始；资源或宿主限制导致等待时记录具体原因。
