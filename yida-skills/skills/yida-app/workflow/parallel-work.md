...
1. Plan 当前方案确认后，按物化结果的 `assetTasks` 用宿主工具派发后台任务；Fast 从 `design.md.assetStrategy` 建立同样任务。按 [后台启动与接收](../../yida-image-assets/SKILL.md#后台启动与接收) 记录真实任务编号、页面、输出和截止时间，复用已有任务。派发成功后主流程立即继续创建应用、表单和页面。
