...
- **登记 URL 必须是发布返回并回读确认的那条完整访问地址，原样传入 `app-entry set`**：直接复用 `publish` 成功返回与 `get-form-config` 回读到的完整运行时 URL，保留其域名、路径段（如 `/workbench/{formUuid}` 或 `/custom/{formUuid}`）和有效参数。不得手工改写、截断或凭 `appType`/`formUuid` 拼接一条未经发布回读的路径来登记；`saveAccessEntries` 是按这条完整路径核验入口页归属的。
...
- 版本冲突时重新读取并核对，不盲目重试覆盖。接口未部署、保存或回读失败时，明确说明入口尚未登记，不能宣称应用卡片已可访问。返回「入口页面不存在或不属于当前应用」时，先核对传入的是不是发布回读确认的那条完整 URL，不要反复更换路径形态盲目重试。
