...
`explicitScope.allowInferredResources=false` 且本轮只有一个普通表单时走最短路径：以 Write 创建字段文件及其父目录，随后调用一次 `openyida create-form create`。成功结果中的 `formUrl` 是表单入口，`appUrl` 是应用入口；按用户要求的交付范围选择对应入口后进入交付终态。普通表单在创建保存成功后进入可用状态，自定义展示页则遵循页面发布生命周期。结果已包含本次验收所需 ID 和链接时直接交付；缺少明确验收证据时执行一次针对性回读。
