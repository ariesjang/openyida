...
function parseArgs(args) {
  const positional = [];
  let existingFormUuid = null;
  let replace = false;
  const invalid = (argument, reason, extra = {}) => {
    throw new CliError([t('create_process.invalid_argument', argument), t('create_process.usage'), t('create_process.usage2')].join('\n'), {
      code: 'CREATE_PROCESS_INVALID_ARGUMENTS', details: { argument, reason, ...extra },
    });
  };
  const directArgs = args || [];
  for (let index = 0; index < directArgs.length; index++) {
    const value = directArgs[index];
    if (value === '--replace') {
      if (replace) {invalid(value, 'duplicate_option');}
      replace = true;
    } else if (value === '--formUuid') {
      if (existingFormUuid !== null) {invalid(value, 'duplicate_option');}
      const next = directArgs[++index];
      if (!next?.trim() || next.startsWith('-')) {invalid(value, 'missing_value');}
      existingFormUuid = next;
    } else if (value.startsWith('-')) {
      invalid(value, 'unknown_option', value === '--form-uuid' ? { suggestion: '--formUuid' } : {});
    } else {
      positional.push(value);
    }
  }
  const fields = existingFormUuid === null
    ? ['appType', 'formTitle', 'fieldsJsonFile', 'processDefinitionFile']
    : ['appType', 'processDefinitionFile'];
  if (positional.length > fields.length) {invalid(positional[fields.length], 'unexpected_argument');}
  for (let index = 0; index < fields.length; index++) {
    if (!positional[index]?.trim()) {invalid(fields[index], 'missing_value');}
  }
  return {
    appType: positional[0],
    formTitle: existingFormUuid === null ? positional[1] : null,
    fieldsJsonFile: existingFormUuid === null ? positional[2] : null,
    processDefinitionFile: positional[fields.length - 1],
    existingFormUuid,
    replace,
  };
}
...
async function run(args) {
  ...
  let formUuid;
  // Reuse does not read form metadata; unknown is not an empty form.
  let fieldCount = null;
  ...
  if (useExistingForm) {
    ...
    try {
      ...
      fieldCount = createFormResult.fieldCount ?? null;
  ...
  try {
    ...
    const failureOutput = {
      ...
      formUuid: formUuid,
      formMode: useExistingForm ? 'reuse' : 'create',
      formTitle: formTitle,
      appType: appType,
