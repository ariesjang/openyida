...
const { readDesignAssetStrategy } = require('./asset-plan');
...
子命令:
  status    检测素材能力
  resolve   校验图片，优先使用允许外链的原地址并生成清单

允许外链的图片检查通过后直接使用；本地图片和需托管的图片上传，最大 20 MiB。
--app-type 用于上传图片；直链检查无需登录或创建应用。省略时上传读取项目 config.json。
--page-id 只处理该页槽位；按页并发时，每页使用独立的 --manifest 文件。
--upload-assets 请求上传允许转存的图片；上传失败时保留已验证且允许外链的原地址。--offline 不联网、不上传。
...
function parseArgs(args) {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (arg === '--help' || arg === '-h') {
      ...
      parsed.usedOptions.push('--app-type');
      parsed.appType = args[++i] || '';
      if (!parsed.appType || parsed.appType.startsWith('--')) {parsed.errors.push('ASSET_APP_TYPE_REQUIRED');}
...
function runStatus(options) {
  ...
  console.log('素材能力检测');
  console.log('  图片交付   : 优先使用已验证且允许外链的图片地址');
  console.log('  上传目标   : 宜搭图片附件（需要登录态和 appType）');
  console.log('  上传条件   : ' + (status.canUpload ? '联网已启用；实际上传时验证登录态和应用权限' : '离线，不上传'));
...
async function runResolve(options) {
  ...
  const draft = options.input ? readDraft(options.input) : {};
  const assetStrategy = options.design ? readDesignAssetStrategy(path.resolve(options.design)) : draft.assetStrategy;
  const assets = [
    ...(draft.assets || []),
