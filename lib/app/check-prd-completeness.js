...
const { auditAssetExecution } = require('../asset/asset-execution');
...
async function evaluatePrdCompleteness(markdown, options = {}, deps = {}) {
  ...
  const items = [];
  const assetExecution = auditAssetExecution(options.buildManifest?.assetExecution, options.buildManifest?.assetTasks);
  ...
  for (const issue of assetExecution.issues) {
    addItem('asset_execution_' + issue.code, 'build_manifest.assetExecution', 'needs_review', 'warning',
      t('asset.executionReview', issue.code), { actual: issue });
  }
