...
// vc-deep-yida: components/vs-divider-style + components/yida-divider.
// 这里只定义平台能力，不按业务场景排序；样式由页面设计选择。
const DIVIDER_TYPES = Object.freeze([
...
const DEFAULT_DIVIDER_TYPE = 'solid';
...
function validateDivider(field, path, diagnostics) {
  ...
  const styleInputs = ['dividerType', 'dividerStyle', 'styleType', 'typeStyle']
    .map(key => [key, field[key]]);
  styleInputs.push(['props.type', field.props?.type]);
  for (const [key, value] of styleInputs) {
    if (value === undefined || value === null || value === '') {continue;}
    if (typeof value === 'string' && DIVIDER_TYPES.includes(value.trim())) {continue;}
    pushDiagnostic(
      diagnostics,
      'INVALID_DIVIDER_TYPE',
      path + '.' + key,
      DIVIDER_TYPES.join(' | '),
      typeof value === 'string' ? value : describeActual(value),
      t('create_form.divider_type_invalid', String(value), DIVIDER_TYPES.join(', ')),
