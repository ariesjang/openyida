...
// Omitting an authored override restores the platform binding in that exact
// scope. Do not overwrite declarations that the user has since customized.
function restoreRemovedNavigation(css, source, tokens, previous) {
  const removed = Object.fromEntries(Object.entries(previous || {}).filter(([name]) => isNavigation(name) && !(name in tokens)));
  if (!Object.keys(removed).length) {return css;}
  if (removed['--pod-shell-theme-bg-color'] && !previous['--pod-page-header-bg-color']) {
    removed['--pod-page-header-bg-color'] = removed['--pod-shell-theme-bg-color'];
  }
  const blocks = /(^:root|^\.pod-premium\.(?:nav|is)-(?:light|dark|white|gray))\s*\{([^}]+)\}/gm;
  const defaults = new Map();
  let rootIndex = 0;
  for (const [, selector, body] of source.matchAll(blocks)) {
    defaults.set(selector === ':root' ? `root-${rootIndex++}` : selector, declarationValues(body));
  }
  rootIndex = 0;
  return css.replace(blocks, (block, selector, body) => {
    const values = defaults.get(selector === ':root' ? `root-${rootIndex++}` : selector) || {};
    const restored = body.replace(DECLARATIONS, (line, name, value) => {
      if (!(name in removed) || value.trim() !== removed[name]) {return line;}
      return name in values ? line.replace(value, () => values[name]) : '';
    });
    return block.replace(body, () => restored);
  });
}
...
function applyNavigationTokens(css, tokens, changed, tone, previousTone) {
  ...
  const navigationNames = Object.keys(tokens).filter(isNavigation);
  const navigationChanged = Object.fromEntries(navigationNames.filter(name => name in changed).map(name => [name, tokens[name]]));
  ...
  for (const mode of ['light', 'dark']) {
    ...
    // The selected marker is a mode-independent shape, not a navigation palette.
    const names = (resetMode ? navigationNames : (mode === tone ? Object.keys(navigationChanged) : []))
      .filter(name => name !== '--pod-nav-menu-item-selected-shadow');
    const values = Object.fromEntries(names.map(name => [name, mode === tone ? tokens[name] : modeDefaults[name]])
      .filter(([, value]) => value !== undefined));
    if (!Object.keys(values).length) {continue;}
    const isForeground = name => name.startsWith('--pod-nav-') || name === '--pod-page-header-text-color';
    const background = Object.fromEntries(Object.entries(values).filter(([name]) => !isForeground(name)));
    // The platform header already references the shell background; keep that link.
    ...
    const foregrounds = Object.fromEntries(Object.entries(values).filter(([name]) => isForeground(name)));
...
function applyDesignTokens(template, markdown, previousMarkdown) {
  ...
  // Older generated files have the selected-shadow rule only. Bring in the
  // shared consumer once when a design opts into menu borders, retaining all
  // existing project rules and declarations during incremental updates.
  if (['--pod-nav-menu-item-border', '--pod-nav-menu-item-hover-border', '--pod-nav-menu-item-selected-border'].some(name => tokens[name])
    && !template.includes('border: var(--pod-nav-menu-item-border,')) {
    const source = fs.readFileSync(path.resolve(__dirname,
      '../../yida-skills/skills/yida-design/references/theme/app-custom-theme-template.css'), 'utf8');
    const shapeRules = source.match(/\/\* openyida-navigation-shape:start \*\/[\s\S]*?\/\* openyida-navigation-shape:end \*\//)[0];
    template += `\n${shapeRules}\n`;
  }
  ...
  if (previous) {
    const source = fs.readFileSync(path.resolve(__dirname,
      '../../yida-skills/skills/yida-design/references/theme/app-custom-theme-template.css'), 'utf8');
    template = restoreRemovedNavigation(template, source, tokens, previous);
  }
  ...
  let tail = template.slice(root.index + root[0].length);
