...
function navigationTone(metadata, tokens) {
  if (!NAVIGATION_COLOR_TOKENS.every(name => tokens[name])) {return null;}
...
function applyNavigationTokens(css, tokens, changed, tone, previousTone) {
  ...
  for (const mode of ['light', 'dark']) {
    ...
    if (shell && !tokens['--pod-page-header-bg-color']) {
      background['--pod-page-header-bg-color'] = mode === tone ? shell : modeDefaults['--pod-page-header-bg-color'];
    }
...
function applyDesignTokens(template, markdown, previousMarkdown) {
  ...
  if (tokens['--pod-nav-menu-item-border'] && !template.includes('border: var(--pod-nav-menu-item-border,')) {
    const source = fs.readFileSync(path.resolve(__dirname,
      '../../yida-skills/skills/yida-design/references/theme/app-custom-theme-template.css'), 'utf8');
