...
function readDesignTokens(markdown) {
  const tokens = extractDesignTokens(designMetadata(markdown), { strict: false });
...
function resolveDesignTokens(markdown) {
...
function navigationTone(markdown, tokens) {
  if (!NAVIGATION_TOKENS.every(name => tokens[name])) {return null;}
  const tone = designMetadata(markdown).themeProfile?.navTheme;
...
function applyNavigationTokens(css, tokens, changed, tone, previousTone) {
  ...
  for (const mode of ['light', 'dark']) {
    ...
    if (shell) {
      const background = { '--pod-shell-theme-bg-color': shell, '--pod-page-header-bg-color': shell };
...
function applyDesignTokens(template, markdown, previousMarkdown) {
  const tokens = resolveDesignTokens(markdown);
  const previous = previousMarkdown ? resolveDesignTokens(previousMarkdown) : null;
  const tone = navigationTone(markdown, tokens);
  const previousTone = previous ? navigationTone(previousMarkdown, previous) : null;
