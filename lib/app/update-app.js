...
/** updateApp 整包保存；与基础设置页一致，读取最新配置后合并，并核验主题资源。 */
async function saveAppSettings(params, authRef) {
  ...
  if (!params.colour && !params.themeColor && !params.customThemeStyle && !expected.themeColor && !expected.customThemeStyle) {
    return response;
  }
  let actual;
  let readbackError;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      actual = getThemeSettings(await fetchCurrentApp(params.appType, authRef));
      readbackError = undefined;
      if (themeSettingsMatch(expected, actual)) {
        return { ...response, themeVerification: { verified: true, ...actual } };
      }
    } catch (error) {
  ...
  return {
    success: false,
    errorCode: 'APP_THEME_NOT_PERSISTED',
    errorMsg: t('update_app.theme_not_persisted'),
    themeVerification: { verified: false, expected, actual, readbackError },
  };
}
...
async function run(args) {
  ...
  if (response && response.success) {
    ...
    throwCommandError(errorMsg, { code: response && response.errorCode, details: response && response.themeVerification });
  }
}
