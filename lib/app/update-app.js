...
const { assertAppThemeKey } = require('./theme-presets');
const {
...
function buildUpdateAppPostData(params, currentApp = {}, authRef) {
  assertAppThemeKey(params.colour);
  if (params.colour && params.colour !== 'custom' && (params.themeFile || params.customThemeStyle || params.themeColor)) {
    throw new Error(t('update_app.theme_preset_conflict'));
  }
  ...
  const currentNavTheme = currentApp.navTheme || (currentApp.config && currentApp.config.NAV_THEME);
  ...
  const postDataObj = {
    ...
    colour: params.customThemeStyle || params.themeColor
      ? 'custom'
      : (params.colour || currentApp.colour || (currentApp.config && currentApp.config.COLOUR) || (currentThemeColor ? 'custom' : 'deepBlue')),
  ...
  const navTheme = params.navTheme || currentNavTheme;
  if (navTheme) {postDataObj.navTheme = navTheme;}
  ...
  if (params.hideAppNav !== null && params.hideAppNav !== undefined) {
    ...
    const hideAppNav = pickAppField(currentApp, 'hideAppNav', 'HIDE_APP_NAV');
    if (hideAppNav !== undefined && hideAppNav !== null) {
      postDataObj.hideAppNav = hideAppNav;
    }
  }
  if (params.themeColor) {
    postDataObj.themeColor = params.themeColor;
  } else if (params.colour && params.colour !== 'custom') {
    postDataObj.themeColor = '';
  } else if (currentThemeColor !== undefined && currentThemeColor !== null) {
    postDataObj.themeColor = currentThemeColor;
  }

  if (params.colour && params.colour !== 'custom' && !params.customThemeStyle) {
...
function getThemeSettings(app) {
  const rawStyle = pickAppField(app, 'customThemeStyle', 'CUSTOM_THEME_STYLE');
  let customThemeStyle = rawStyle || null;
  if (typeof customThemeStyle === 'string') {
    try { customThemeStyle = JSON.parse(customThemeStyle); } catch { /* Keep malformed data visible to verification. */ }
  }
  return {
    colour: pickAppField(app, 'colour', 'COLOUR') || '',
    themeColor: pickAppField(app, 'themeColor', 'THEME_COLOR') || '',
    customThemeStyle,
  };
}

function themeSettingsMatch(expected, actual) {
  const normalizeColor = (value) => value ? normalizeCssColorToHex(value) || value : '';
  if (expected.colour !== actual.colour || normalizeColor(expected.themeColor) !== normalizeColor(actual.themeColor)) {
    return false;
  }
  const expectedStyle = expected.customThemeStyle;
  const actualStyle = actual.customThemeStyle;
  if (!expectedStyle || !actualStyle || typeof expectedStyle !== 'object' || typeof actualStyle !== 'object') {
    return expectedStyle === actualStyle;
  }
  return ['enabled', 'iframePropagation'].every((key) => Boolean(expectedStyle[key]) === Boolean(actualStyle[key])) &&
    ['cssUrl', 'cssFileName'].every((key) => (expectedStyle[key] || '') === (actualStyle[key] || ''));
}

function getRequestedNavigationSettings(params, payload) {
  return Object.fromEntries(['navTheme', 'layoutDirection', 'hideAppNav', 'logoSource']
    .filter((name) => params[name] !== undefined && params[name] !== null && params[name] !== '')
    .map((name) => [name, payload[name]]));
}

function getNavigationSettings(app, fields) {
  const configKeys = { navTheme: 'NAV_THEME', layoutDirection: 'LAY_OUT_DIRECTION', hideAppNav: 'HIDE_APP_NAV', logoSource: 'LOGO_SOURCE' };
  return Object.fromEntries(fields.map((name) => {
    let value = pickAppField(app, name, configKeys[name]);
    if (name === 'layoutDirection') {
      const navType = pickAppField(app, 'navType', 'NAVTYPE');
      // A missing response field is not evidence that the requested default was saved.
      value = value || navType ? normalizeStoredLayoutDirection(value, navType) : null;
    }
    return [name, value === undefined ? null : value];
  }));
}

/** updateApp 整包保存；读取最新配置后合并，并核验主题资源和本次请求的导航设置。 */
async function saveAppSettings(params, authRef) {
  const currentApp = await fetchCurrentApp(params.appType, authRef);
  const payload = buildUpdateAppPostData(params, currentApp, authRef);
  const response = await requestWithAutoLogin((auth) => httpPost(
    ...
    querystring.stringify({ ...payload, _csrf_token: auth.csrfToken })
  ), authRef);
  if (!response || !response.success) { return response; }

  const expected = getThemeSettings(payload);
  const verifyTheme = !!(params.colour || params.themeColor || params.customThemeStyle || expected.themeColor || expected.customThemeStyle);
  const expectedNavigation = getRequestedNavigationSettings(params, payload);
  const navigationFields = Object.keys(expectedNavigation);
  if (!verifyTheme && !navigationFields.length) {return response;}
  let actual;
  let actualNavigation;
  let readbackError;
  let themeVerified = !verifyTheme;
  let navigationVerified = !navigationFields.length;
  // 只重试查询，避免重复上传资源或用旧快照再次覆盖设置。
  for (let attempt = 0; attempt < 3; attempt++) {
    if (attempt) { await new Promise((resolve) => setTimeout(resolve, attempt * 250)); }
    try {
      const app = await fetchCurrentApp(params.appType, authRef);
      actual = getThemeSettings(app);
      actualNavigation = getNavigationSettings(app, navigationFields);
      readbackError = undefined;
      themeVerified = !verifyTheme || themeSettingsMatch(expected, actual);
      navigationVerified = navigationFields.every((name) => expectedNavigation[name] === actualNavigation[name]);
      if (themeVerified && navigationVerified) {
        return { ...response,
          ...(verifyTheme ? { themeVerification: { verified: true, ...actual } } : {}),
          ...(navigationFields.length ? { navigationVerification: { verified: true, ...actualNavigation } } : {}),
        };
      }
    } catch (error) {
      readbackError = error.message;
      themeVerified = !verifyTheme;
      navigationVerified = !navigationFields.length;
    }
  }
  return {
    success: false,
    errorCode: themeVerified ? 'APP_NAVIGATION_NOT_PERSISTED' : 'APP_THEME_NOT_PERSISTED',
    errorMsg: t(themeVerified ? 'update_app.navigation_not_persisted' : 'update_app.theme_not_persisted'),
    ...(verifyTheme ? { themeVerification: themeVerified
      ? { verified: true, ...actual } : { verified: false, expected, actual, readbackError } } : {}),
    ...(navigationFields.length ? { navigationVerification: navigationVerified
      ? { verified: true, ...actualNavigation }
      : { verified: false, expected: expectedNavigation, actual: actualNavigation, readbackError } } : {}),
  };
}
...
async function applyCustomThemeUpdate(appType, themeParams, authRef) {
  const params = {
    ...themeParams,
    appType,
    layoutDirection: normalizeLayoutDirection(themeParams.layoutDirection),
  };
  ...
  return saveAppSettings(params, authRef);
}
...
async function run(args) {
  ...
  assertAppThemeKey(params.colour);
  if (params.colour && params.colour !== 'custom' && (params.themeFile || params.themeColor)) {
    throw new Error(t('update_app.theme_preset_conflict'));
  }
  ...
  const response = params.themeFile ? await applyCustomThemeUpdate(params.appType, params, authRef)
    : shellUpdate ? await saveAppSettings(params, authRef) : await requestWithAutoLogin((auth) => {
      const postDataObj = buildUpdateAppNamePostData(params, auth);
  ...
  if (response && response.success) {
    ...
    console.log(JSON.stringify({
      success: true,
      appType: params.appType,
      themeVerification: response.themeVerification,
      navigationVerification: response.navigationVerification,
      updatedFields: {
    ...
    console.log(JSON.stringify({ success: false, appType: params.appType, error: errorMsg,
      errorCode: response && response.errorCode, themeVerification: response && response.themeVerification,
      navigationVerification: response && response.navigationVerification }));
    throwCommandError(errorMsg, { code: response && response.errorCode,
      details: response && (response.errorCode === 'APP_NAVIGATION_NOT_PERSISTED' ? response.navigationVerification : response.themeVerification) });
  }
}
