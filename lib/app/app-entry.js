...
/** Business backend entry is the application workbench root; drop any trailing page segment. */
function canonicalizeManagementUrl(url, appType) {
  const match = new RegExp(`^(https?://[^/]+/${appType}/workbench)/([^/?#]+)((?:[?#].*)?)$`, 'i').exec(url);
  if (!match) { return url; }
  console.error(t('app_entry.downgrade_workbench', match[2]));
...
async function resolveEntryUrl(authRef, appType, key, formUuid) {
  ...
  return normalizeUrl(`${authRef.baseUrl}/${appType}/workbench`, appType);
}
...
async function run(args = []) {
  ...
  for (const [key, spec] of Object.entries(values)) {
    if (spec.kind === 'clear') {
      ...
      patch[key] = { url: key === 'management' ? canonicalizeManagementUrl(url, appType) : url };
    }
  }
...
module.exports = { run, parseArgs, normalizeUrl, canonicalizeManagementUrl, resolveEntryUrl };
