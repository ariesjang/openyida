...
// ── 保存 Schema（create/update 共用）─────────────────

async function saveFormSchema(authRef, appType, formUuid, schema, version, stepOffset, failureContext) {
...
async function mainCreate(parsedArgs, authRef) {
  ...
  try {
    ...
    postCreateStage = 'saveFormSchema';
    await saveFormSchema(authRef, appType, formUuid, schema, serverRevision, 4, {
      appType,
      formTitle,
      formUuid,
      fieldCount,
  ...
  console.log(JSON.stringify(withBrowserHandoff(
    { success: true, formUuid, formTitle, appType, fieldCount, icon: formIcon, iconSource: iconResolution.source, url: formUrl, appUrl },
    formUrl,
...
async function createFormForLegacyProcessQuiet(context, input) {
  ...
  return {
    ...
    appUrl: authRef.baseUrl + '/' + appType + '/workbench',
  };
}
