...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const filteredArgs = args.filter(arg => arg !== '--skip-lint' && arg !== '--health-check' && arg !== '--check' && arg !== '--compat' && arg !== '--modern' && arg !== '--force' && arg !== '--canvas' && arg !== '--allow-fixed-brand' && arg !== '--auto-nav-order' && arg !== '--json' && arg !== '--help' && arg !== '-h');
...
async function runMain(argv) {
  ...
  const { appType, formUuid, sourceFile, skipLint, healthCheck, compat, force, canvas: canvasFlag, allowFixedBrand, autoNavOrder, browserOpenMode } = options;
