...
const { assertPageTemplateCustomized } = require('./page-template-guard');
...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const allowFixedBrand = args.includes('--allow-fixed-brand');
  const fixTheme = args.includes('--fix-theme');
  const autoNavOrder = args.includes('--auto-nav-order');
  const help = args.includes('--help') || args.includes('-h');
  const filteredArgs = args.filter(arg => arg !== '--skip-lint' && arg !== '--health-check' && arg !== '--check' && arg !== '--compat' && arg !== '--modern' && arg !== '--force' && arg !== '--canvas' && arg !== '--allow-fixed-brand' && arg !== '--fix-theme' && arg !== '--auto-nav-order' && arg !== '--json' && arg !== '--help' && arg !== '-h');
...
// Ask the human (via stdin) whether to migrate. Returns false in non-TTY
// environments so agent-driven runs must opt in explicitly with --fix-theme.
function confirmFixedBrandMigration() {
  if (!process.stdin.isTTY) { return Promise.resolve(false); }
  const readline = require('readline');
  const rl = readline.createInterface({ input: process.stdin, output: process.stderr });
  return new Promise((resolve) => {
    rl.question(`${t('publish.fix_theme_confirm')} `, (answer) => {
      rl.close();
...
// Offer to un-pin fixed brand colors so the app theme takes over. Confirms with
// the user (or auto-applies when --fix-theme is set), rewrites the source file
// in place, and returns the migrated source; returns null when nothing changed
// or the user declined, letting the caller surface the original guard error.
async function offerFixedBrandThemeMigration(sourcePath, source, { autoConfirm } = {}) {
  const { transformFixedBrandTheme } = require('./fix-theme');
  const result = transformFixedBrandTheme(source);
  if (!result.changed) { return null; }

  warn(t('publish.fix_theme_detected', result.violations.length));
  result.violations.forEach((item) => hint(t('publish.fix_theme_item', item.line, item.field, item.color)));

  const proceed = autoConfirm === true || await confirmFixedBrandMigration();
  if (!proceed) {
    if (!process.stdin.isTTY) { info(t('publish.fix_theme_flag_hint')); }
    else { info(t('publish.fix_theme_skipped')); }
    return null;
  }

  fs.writeFileSync(sourcePath, result.output);
  success(t('publish.fix_theme_applied', sourcePath));
...
async function runMain(argv) {
  ...
  const { appType, formUuid, sourceFile, skipLint, healthCheck, compat, force, canvas: canvasFlag, allowFixedBrand, fixTheme, autoNavOrder, browserOpenMode } = options;
  ...
  assertPageTemplateCustomized(initialSourceCode, sourcePath);
  ...
  if (isCanvas) {
    ...
    try {
      canvasResult = await compileCanvas(initialSourceCode, { sourcePath, allowFixedBrand });
    } catch (canvasCompileError) {
      const migratedSource = canvasCompileError && canvasCompileError.code === 'OPENYIDA_CANVAS_THEME_FIXED_BRAND'
        ? await offerFixedBrandThemeMigration(sourcePath, initialSourceCode, { autoConfirm: fixTheme })
        : null;
      if (migratedSource === null) {
        throw buildCanvasCompileCliError(canvasCompileError, sourcePath);
      }
      sourceCode = migratedSource;
      try {
        canvasResult = await compileCanvas(migratedSource, { sourcePath, allowFixedBrand });
      } catch (recompileError) {
        throw buildCanvasCompileCliError(recompileError, sourcePath);
      }
    }
