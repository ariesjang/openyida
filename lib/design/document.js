...
function flattenTokens(node, target, field, strict, scopes, scope) {
  ...
  for (const [key, value] of Object.entries(node)) {
    if (key.startsWith('--')) {
      ...
      target[key] = scalar;
      if (scopes) {scopes[key] = scope;}
    } else {
      flattenTokens(value, target, `${field}.${key}`, strict, scopes, scope);
    }
  }
}

function validateTokenReferences(tokens, scopes) {
  ...
  for (const [name, refs] of dependencies) {
    for (const ref of refs) {
      ...
      if (scopes[name] === 'application-global' && scopes[ref] === 'custom-page') {
        invalid(name, 'GLOBAL_DEPENDS_ON_CUSTOM_TOKEN', { reference: ref });
      }
    }
  }
...
function extractDesignTokens(metadata, options = {}) {
  ...
  if (strict) {
    ...
    for (const scope of ['application-global', 'custom-page']) {
      flattenTokens(metadata.tokens[scope], tokens, `tokens.${scope}`, true, scopes, scope);
    }
    ...
    validateTokenReferences(tokens, scopes);
  } else {
...
function validatePrd(prdMarkdown, metadata, pageIds, options) {
  ...
  for (const page of prdHandoffPages(prdMarkdown)) {
    ...
    // Exported review bundles may live outside their eventual project directory.
    // Only compare relative design paths when the caller supplies that base.
    if (options.designFile && (options.baseDir || require('path').isAbsolute(handoff.designFile))
      && require('path').resolve(options.baseDir || process.cwd(), handoff.designFile) !== require('path').resolve(options.designFile)) {
      invalid(`prd.pages.${page.pageId}.designFile`, 'DESIGN_FILE_MISMATCH', { actual: handoff.designFile });
    }
...
function validateDesignDocument(markdown, options = {}) {
  ...
  function reference(value, field) {
    if (!object(value) || !ANCHOR.test(value.anchor || '') || !anchors.has(value.anchor)) {
      invalid(field, 'EXISTING_ANCHOR_REQUIRED', { anchor: value?.anchor });
    }
  }
  for (const root of ['components', 'states']) {
    ...
    for (const [key, value] of Object.entries(metadata[root])) {
      ...
      if (position < chapterThree || position >= chapterFour) {invalid(`${root}.${key}`, 'ANCHOR_MUST_BE_IN_CHAPTER_THREE');}
    }
  }
  ...
  for (const [key, scene] of Object.entries(metadata.sceneRecipes)) {
    ...
    for (const page of scene.pages) {
      ...
      if (!page.anchor.startsWith('#page-') || pageAnchors.has(page.anchor)) {invalid(page.pageId, 'UNIQUE_PAGE_ANCHOR_REQUIRED');}
  ...
  for (let index = 0; index < positions.length; index++) {
    ...
    if (start < chapterFive) {invalid(anchor, 'PAGE_MUST_BE_IN_CHAPTER_FIVE');}
    const section = text.slice(start, positions[index + 1]?.[1] || text.length);
    for (const label of PAGE_LABELS) {
      const pattern = new RegExp(`^(?:[ \\t]*[-*][ \\t]+)?(?:\\*\\*)?${label}[：:](?:\\*\\*)?[ \\t]*(\\S[^\\n]*)$`, 'm');
      const match = pattern.exec(section);
      if (!match || /草稿待补充/.test(match[1]) || /^(?:待定|待补充|TBD|TODO|[-—])(?:[。.!！]*)$/i.test(match[1])) {invalid(anchor, 'PAGE_LABEL_REQUIRED', { label });}
    }
  }
