...
const { REQUIRED_BRAND_SCALE_TOKENS: BRAND_TOKENS } = require('../core/theme-brand-scale');
...
function validateTokenReferences(tokens, metadata) {
  // Platform-owned aliases need not be copied into every authored design.
  const css = fs.readFileSync(path.resolve(__dirname, '../../yida-skills/skills/yida-design/references/theme/app-custom-theme-template.css'), 'utf8');
  const blocks = [...css.matchAll(/(^:root|^\.pod-premium\.(?:nav|is)-(?:light|dark|white|gray))\s*\{([^}]+)\}/gm)];
  const tone = metadata.themeProfile?.navTheme || metadata.navTheme || 'light';
  const defaults = {};
  for (const [, , body] of [...blocks.filter(block => block[1] === ':root'), ...blocks.filter(block => [`.pod-premium.nav-${tone}`, `.pod-premium.is-${tone}`].includes(block[1]))]) {
    Object.assign(defaults, Object.fromEntries([...body.matchAll(/(--[\w-]+)\s*:\s*([^;]+);/g)].map(match => [match[1], match[2].trim()])));
  }
  const combined = { ...defaults, ...tokens };
  const dependencies = new Map(Object.entries(combined).map(([name, value]) => [name, [...value.matchAll(/var\(\s*(--[\w-]+)/g)].map(match => match[1])]));
  for (const name of Object.keys(tokens)) {
    const refs = dependencies.get(name);
    for (const ref of refs) {
      if (!(ref in combined)) {invalid(name, 'UNDECLARED_TOKEN_REFERENCE', { reference: ref });}
    }
  }
...
function extractDesignTokens(metadata, options = {}) {
  ...
  if (strict) {
    ...
    validateTokenReferences(tokens, metadata);
  } else {
...
function validateDesignDocument(markdown, options = {}) {
  ...
  const issues = [];
  const collect = (field, code, details = {}) => issues.push({
    sourcePath: options.designFile, path: field, code, ...details,
  });
  function reference(value, field) {
    if (!object(value) || !ANCHOR.test(value.anchor || '') || !anchors.has(value.anchor)) {
      collect(field, 'EXISTING_ANCHOR_REQUIRED', { anchor: value?.anchor, nextStep: t('design_document.anchor_hint') });
      return false;
    }
    return true;
  }
  for (const root of ['components', 'states']) {
    ...
    for (const [key, value] of Object.entries(metadata[root])) {
      ...
      if (!reference(value, `${root}.${key}`)) { continue; }
      if (Object.keys(value).join('|') !== 'anchor') {invalid(`${root}.${key}`, 'ANCHOR_ONLY_REQUIRED');}
      const position = anchors.get(value.anchor);
      if (position < chapterThree || position >= chapterFour) {collect(`${root}.${key}`, 'ANCHOR_MUST_BE_IN_CHAPTER_THREE', { nextStep: t('design_document.anchor_chapter_three_hint') });}
    }
  }
  ...
  for (const [key, scene] of Object.entries(metadata.sceneRecipes)) {
    ...
    for (const page of scene.pages) {
      ...
      if (!reference(page, `sceneRecipes.${key}.${page.pageId}`)) { continue; }
      if (Object.keys(page).sort().join('|') !== 'anchor|pageId') {invalid(`sceneRecipes.${key}.${page.pageId}`, 'PAGE_ID_AND_ANCHOR_ONLY_REQUIRED');}
      if (!page.anchor.startsWith('#page-') || pageAnchors.has(page.anchor)) {collect(page.pageId, 'UNIQUE_PAGE_ANCHOR_REQUIRED', { nextStep: t('design_document.page_anchor_prefix_hint') });}
  ...
  for (let index = 0; index < positions.length; index++) {
    ...
    if (start < chapterFive) {collect(anchor, 'PAGE_MUST_BE_IN_CHAPTER_FIVE', { nextStep: t('design_document.page_anchor_chapter_five_hint') });}
    const section = text.slice(start, positions[index + 1]?.[1] || text.length);
    for (const label of PAGE_LABELS) {
      const pattern = new RegExp(`^(?:[ \\t]*[-*][ \\t]+)?(?:\\*\\*)?${label}[：:](?:\\*\\*)?[ \\t]*(\\S[^\\n]*)$`, 'm');
      const match = pattern.exec(section);
      if (!match || !concretePageText(match[1])) {collect(anchor, 'PAGE_LABEL_REQUIRED', { label, nextStep: t('design_document.page_label_hint', label) });}
    }
  }
  if (issues.length) {
    const first = issues[0];
    invalid(first.path, first.code, { ...first, issues });
  }
