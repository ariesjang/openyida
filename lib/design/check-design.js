...
function parseArgs(args = []) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    else if (arg === '--prd' && args[index + 1] && !args[index + 1].startsWith('-')) {
      if (options.prd) {throw new CliError(t('design_document.usage'), { code: 'CHECK_DESIGN_USAGE' });}
      options.prd = args[++index];
...
async function run(args = []) {
  ...
  const output = { ...result, designFile, ...(prdFile ? { prdFile } : {}) };
