...
function collectIssues(plan) {
  ...
  for (const [index, page] of (Array.isArray(pages) ? pages : []).entries()) {
    ...
    for (const key of ['firstScreenFocus', 'layout', 'responsive', 'primaryAction']) {
      ...
      check(filled(value) && !/^(?:继承主题|继承全局|遵循主题|遵循全局|同全局|同上|按主题执行|按全局执行|待补充|待定)[。.!！]?$/.test(value.trim()),
    ...
    check(Array.isArray(application?.acceptanceChecks) && application.acceptanceChecks.length > 0
      && application.acceptanceChecks.every(filled), `${p}.acceptanceChecks`, `为 ${page?.name || page?.pageId} 填写非空验收检查数组`);
  }
  if (/^2(?:\.|$)/.test(String(plan.schemaVersion))) {
    ...
    if (user?.colorStrategy?.surfaceTone !== undefined) {
      check(['theme', 'brand-tinted'].includes(user.colorStrategy.surfaceTone), 'visualStyle.forUser.colorStrategy.surfaceTone', '选择 theme 或 brand-tinted');
    }
