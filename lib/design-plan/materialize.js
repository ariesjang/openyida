...
const { buildEntryNavigation } = require('./entry-navigation');
...
const { PLAN_SKILL_ROOT, loadThemeIndex, themeTemplatePath, resolveThemeColors, resolveColorToken } = require('./themes');
...
function pageHandoff(page, plan) {
  ...
  const entryMode = supplied.entryMode || page.entryMode || (customNavigation || supplied.navigation ? 'standalone' : 'platform-shell');
...
function pageNavigationRows(page, plan) {
  ...
  const entryMode = handoff.entryMode || page.entryMode || (applicationCustom || menu ? 'standalone' : 'platform-shell');
...
function buildExecution(plan) {
  ...
  if (entryNavigation) {
    ...
    handoff.navigationFallback = skipOrder ? '当前范围不执行平台导航排序' : '按业务管理入口的默认任务和菜单顺序统一排序';
    if (!resourceOnly) {
...
function renderEntryNavigation(recommendation) {
  ...
  const lines = ['### 访问态入口与权限', '访客端与业务管理端共享资源，分别组织菜单；页面隐藏导航不授予业务权限。', ''];
...
function deriveSurfacePalette(primaryColor, surfaceTone, template) {
  if (!['theme', 'brand-tinted'].includes(surfaceTone)) {
    throw new CliError('colorStrategy.surfaceTone 必须是 theme 或 brand-tinted', { code: 'DESIGN_PLAN_INVALID_SURFACE_TONE' });
  }
  const surface = resolveColorToken(readDesignTokens(template), '--pod-card-bg-color');
  // 深色表面保持其既有明度层级，不能因为品牌配色而变成白卡。
  ...
  return {
    ...
    '--color-fill1-2': mixHex(primaryColor, [255, 255, 255], 0.91),
    '--color-fill1-3': mixHex(primaryColor, [255, 255, 255], 0.84),
    '--color-line1-1': mixHex(primaryColor, [255, 255, 255], 0.86),
    '--color-line1-2': mixHex(primaryColor, [255, 255, 255], 0.76),
  };
}

function navigationPalette(primaryColor, tone, tokens) {
  const shell = resolveColorToken(tokens, '--pod-shell-theme-bg-color');
  const isDark = shell && [1, 3, 5].every(index => parseInt(shell.slice(index, index + 2), 16) < 128);
  if (!shell || isDark === (tone === 'dark')) {return {};}
  const dark = tone === 'dark';
  const background = dark
    ? mixHex(primaryColor, [23, 23, 23], 0.62)
    : mixHex(primaryColor, [255, 255, 255], 0.96);
  return {
    '--pod-shell-theme-bg-color': background,
    '--pod-nav-item-text-color': dark ? '#D6D6D6' : '#606060',
    '--pod-nav-item-text-hover-color': dark ? '#FFFFFF' : '#303030',
    '--pod-nav-item-text-selected-color': dark ? '#FFFFFF' : '#171717',
    '--pod-nav-menu-bg-hover-color': mixHex(background, dark ? [255, 255, 255] : [0, 0, 0], dark ? 0.08 : 0.04),
    '--pod-nav-menu-bg-selected-color': mixHex(background, dark ? [255, 255, 255] : [0, 0, 0], dark ? 0.16 : 0.08),
  };
}
...
function renderNavigationDesign(plan, navigationStyle, tokens) {
  ...
  return [
    ...
    `- 选择依据：${text(navigationStyle.selectionReason)}`, '',
    mdTable(['角色', '变量', '本项目取值'], Object.entries(roles).map(([name, role]) => [role, name, tokens[name]])), '',
...
function renderProjectApplications(plan, tokens, color, chapter) {
  ...
  const colors = {
    ...
    '--color-fill1-5': '独立浮层', '--color-fill1-10': 'Tooltip背景', '--color-text1-5': 'Tooltip前景',
  };
  return [
    '## 5. 项目应用与调整规则', '',
    themeImplementationRules(chapter), '',
    '### 本项目实现边界', '',
    '本项目显式 Token 与页面决定优先于默认值；原生导航采用第二章列出的六项实际配色，自绘导航按本项目入口方案实施。业务内容、操作和数据来自 PRD，条件配方仅用于本项目已有内容。',
...
function renderDesign(plan, options = {}) {
  ...
  const palette = deriveSurfacePalette(primaryColor, color.surfaceTone || 'theme', source);
  const overrides = { ...palette, ...navigationPalette(primaryColor, navigationStyle.tone, themeTokens), ...explicitTokens };
  ...
  chapters[0] = chapters[0].replace(/(^## 1\. 风格摘要\n\s*)\*\*([^\n]+)\*\*[ \t]*\n(?:[ \t]*\n)?/m,
  ...
  if (Object.keys(palette).length) {
    chapters[1] += `\n\n### 项目配色适配\n\n本项目填充和边界以 ${primaryColor} 形成浅色氛围；页面与卡片保留主题表面层级，项目显式变量优先。正文、独立数据色与平台状态色保持各自语义。`;
  }
  ...
  const metadata = {
    ...
    themeProfile: {
      ...
      navTheme: navigationStyle.tone, themeDelivery: 'app-custom-theme-file',
...
function prepareArtifacts(sourcePlan, outputDir, themeSeed) {
  ...
  measure('designValidationMs', () => validateDesignDocument(design, { prdMarkdown: prd }));
...
function materializeUnlocked(inputPath, options = {}) {
  ...
  return { success: true, checked: options.check === true, input, outputDir, revision: artifacts.revision, outputs: artifacts.outputs, assetTasks: artifacts.assetTasks, timings, ...(updatesSource ? { merged: true, previousRevision: source.meta.revision } : {}) };
}
