...
function renderDesign(plan, options = {}) {
  ...
  const colorPairing = fs.readFileSync(THEME_CONSISTENCY_PATH, 'utf8')
    .match(/(?:^|\n)(## 指标卡与按钮配色\n[\s\S]*?)(?=\n## |$)/)?.[1];
  if (!colorPairing) {
    throw new CliError(t('design_document.invalid', THEME_CONSISTENCY_PATH, '## 指标卡与按钮配色'), {
      code: 'DESIGN_PLAN_THEME_CONTEXT_MISSING', details: { section: '指标卡与按钮配色' },
    });
  }
  chapters[1] += '\n\n' + colorPairing.trim().replace(/^## /, '### ');
