...
function renderDesign(plan, options = {}) {
  ...
  const overrides = { ...navigationPalette(primaryColor, navigationStyle.tone, themeTokens), ...explicitTokens };
  ...
  chapters[2] += '\n\n### 图标与相邻底色\n\n图标前景和底盒分别取色，对比度至少为 3:1；浅底配深图标，实色底按实际对比度选择前景。颜色配对记录在 iconSystem.colorPairs，页面按同一变量实现，并检查颜色继承、描边和透明度。';
...
function prepareArtifacts(sourcePlan, outputDir, themeSeed, options = {}) {
  ...
  if (options.renderHtml === false && authoringIssues.length) {
    throw new CliError(t('design_document.invalid', 'build-plan.json', authoringIssues.map(issue => `${issue.path}: ${issue.message}`).join('; ')), {
      code: 'DESIGN_PLAN_VALIDATION_FAILED', details: { issues: authoringIssues },
    });
  }
...
function prepareArtifactUpdate(artifacts, outputDir, previousArtifacts) {
  ...
  validateDesignDocument(design, { prdMarkdown: content(artifacts.outputs.prd), designFile: path.resolve(artifacts.outputs.design) });
  ...
  if (fs.existsSync(artifacts.outputs.theme)) {
    ...
    const css = applyDesignTokens(current, design, previous);
...
function materializeUnlocked(inputPath, options = {}) {
  ...
  return { success: true, checked: options.check === true, input, outputDir, revision: artifacts.revision, outputs: artifacts.outputs, updated: options.check ? [] : updates.updated, assetTasks: artifacts.assetTasks, timings, ...(updatesSource ? { merged: true, previousRevision: source.meta.revision } : {}) };
}
