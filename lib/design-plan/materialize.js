...
function renderNavigationDesign(plan, navigationStyle, tokens) {
  const roles = {
    ...
    '--pod-nav-menu-font-size': '菜单字号', '--pod-nav-menu-gap': '菜单间距',
  ...
  return [
    ...
    '- 导航、表单、自定义页面和记录详情共用本项目设计；完整导航状态与尺寸见 tokens.application-global.appearance.navigation。', '',
...
function renderProjectApplications(plan, tokens, color, chapter) {
  ...
  return [
    ...
    '### 本项目实现边界', '',
    '本项目显式 Token 与页面决定优先于默认值；原生导航采用第二章的导航设计和完整导航 token，自绘导航按本项目入口方案实施。导航、应用框架、表单、自定义页面与详情保持统一风格。业务内容、操作和数据来自 PRD，条件配方用于本项目已有内容。',
