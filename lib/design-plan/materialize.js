...
const { PLAN_SKILL_ROOT, loadThemeIndex, themeTemplatePath, themeNavTheme, resolveThemeColors } = require('./themes');
...
function visualSelections(plan) {
  ...
  const navigationStyle = {
    ...
    ...(theme ? { tone: themeNavTheme(theme), toneSource: 'theme_derived' } : {}),
  };
...
function renderDesign(plan, options = {}) {
  ...
  if (parsed.metadata.navTheme !== navigationStyle.tone) {
    throw new CliError('navigationStyle.tone 必须由所选主题模板的 navTheme 派生', {
      code: 'DESIGN_PLAN_NAVIGATION_TONE_MISMATCH',
      details: { themeId: theme.themeId, expected: parsed.metadata.navTheme, actual: navigationStyle.tone },
    });
  }
