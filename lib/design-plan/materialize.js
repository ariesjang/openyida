...
function pageNavigationRows(page, plan) {
  ...
  return [
    ...
    ['页面菜单', menu ? (menu.type === 'custom' ? `自定义${variants[menu.variant] || ''}菜单` : '不设菜单')
      : applicationCustom ? '沿用应用自定义菜单' : entryMode === 'standalone' ? '按页面任务设计' : '沿用平台导航'],
    ['页面实现边界', entryMode === 'platform-shell' ? '只实现业务内容；同任务分类用页内 Tab，跨模块切换交给平台菜单，不重复自绘管理导航' : '按独立入口方案实现页面菜单，不嵌套平台导航'],
...
function buildExecution(plan) {
  ...
  const pageHandoffs = pages.map(page => {
    ...
    const platformShell = spec.entryMode === 'platform-shell';
    return { pageId: page.pageId, name: page.name, sceneKey: page.sceneKey || spec.scene, pageSpecHandoff: spec,
      navigationPolicy: {
        applicationMenuOwner: platformShell ? 'platform' : spec.navigation?.type === 'none' ? 'none' : 'page',
        pageLayout: platformShell ? 'content-only' : 'standalone',
        renderApplicationMenu: !platformShell && spec.navigation?.type !== 'none',
        localTabs: platformShell ? 'same-task-only' : 'planned-views',
        duplicatePlatformMenu: false,
      },
    };
  });
...
function deriveSurfacePalette(primaryColor, surfaceTone, template) {
  ...
  const surface = normalizeHex(readDesignTokens(template)['--color-white']);
...
function assetSummary(assetStrategy) {
  ...
  return pages.map((page) => {
    ...
    return `${page.pageId || '未命名页面'}: ${page.imageNeed || 'none'}，${slotCount} 个槽位`;
  }).join('；') || '无图片需求';
}
...
function renderDesign(plan) {
  ...
  const replacements = {
    ...
    PAGE_IMAGE_NEEDS: assetSummary(forUser.assetStrategy),
  ...
  output = injectAfterFrontmatter(
    ...
    renderProjectVisualSection(visualDirection, color, navigationStyle, navigationConfig(plan))
      + '\n\n## 各入口的导航\n\n'
      + (plan.pages?.customPageDetails || []).map(page => `### ${text(page.name)}\n\n${mdTable(['项目', '方案'], pageNavigationRows(page, plan))}`).join('\n\n')
      // Fast 与 Plan 消费同一份规则；纯原生应用无需追加自定义页面契约。
      + (plan.pages?.customPageDetails?.length ? '\n\n' + fs.readFileSync(PAGE_CONTINUITY_PATH, 'utf8').trim() : '')
  );
  const derived = deriveBrandColors(primaryColor, selected.themeId);
  ...
  output = replaceTokenValue(output, '--pod-shell-theme-bg-color',
    navigationStyle.tone === 'dark' ? 'var(--color-brand1-5)' : 'var(--color-brand1-3)');
  ...
  const palette = deriveSurfacePalette(primaryColor, color.surfaceTone || 'theme', output);
  const overrides = { ...palette, ...explicitTokens };
  ...
  if (Object.keys(palette).length) {
    output = injectAfterFrontmatter(output, [
      ...
      '页面背景消费 --pod-page-bg-color，卡片消费 --pod-card-bg-color（回退 --color-white），边界使用 1px solid var(--color-line1-1)；近白页面与白卡必须保留可见细边框，不能只靠阴影分层。导航、表头、嵌套区域与选中态消费同一组品牌/填充 token。',
  ...
  output = output.replace(/^---\n/, () => [
    '---',
    `assetStrategy: ${JSON.stringify({ pages: [], ...forUser.assetStrategy })}`,
