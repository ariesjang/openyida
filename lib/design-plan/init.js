...
function initialize(inputPath, options = {}) {
  ...
  const navTheme = themeNavTheme(theme);
  ...
  const plan = {
    ...
    visualStyle: {
      ...
      forUser: { visualDirection: { ...(choice.visualDirection || { label: theme.label, description: theme.styleSummary }), source: visualSource },
        ...
        navigationStyle: { structure: '', source: '', selectionReason: '', ...(choice.navigationStyle || {}), tone: navTheme, toneSource: 'theme_derived' },
  ...
  const context = [
    ...
    '前台、后台及共用工作区都要确定默认页面、菜单及分组顺序、打开后的首屏任务。menu 数组记录展示顺序，defaultMenuKey 记录默认任务；按角色需求决定，不按资源创建先后排列。平台导航通过排序命令落实，自定义导航通过页面菜单落实，frontend-only 也必须完成入口排序与首页验收。', '',
