...
function catalog() {
  ...
  return {
    success: true,
    themes: themes.map(({ themeId, label, templatePath, styleSummary }) => ({ themeId, label, templatePath, styleSummary })),
...
function authoringGuidance(plan, business, visual, businessReady, visualReady) {
  ...
  return {
    ...
    examples: {
      colorStrategy: { primaryColor: '#1677FF', primaryColorName: '蓝色', usage: '用于主要操作' },
...
function initialize(inputPath, options = {}) {
  ...
  const context = [
    ...
    `读取本上下文、编写契约和 business.json；按 authoring.pendingFields 补齐事实，保留 base 与已有需求。business facts 填写 overview、dataModels、businessFlows、pages 和可选 execution；视觉填写 visual.json。复核业务覆盖后设置片段 ready=true，再执行：${materializeCommand}。使用成功结果中的 HTML 路径和 revision 展示确认。视觉选择不完整或有品牌稿、参考图、逐页特殊视觉要求时，补齐相应视觉内容。`, '',
    ...
    '入口方案沿用 execution.entryRecommendation；逐入口补齐 role、menu 和 defaultMenuKey，访客端关联独立页面 sceneKey。管理端可直接落到原生业务视图，不强制创建首页。菜单按资源、视图和操作记录 access 权限依赖，平台导航排序从管理菜单派生。详见 yida-app/references/entry-navigation.md。', '',
  ...
  return { success: true, input, output, context: guide, contract: path.join(ROOT, 'references/build-plan-compact-schema.md'), draft: true,
    ...
    authoring: { pendingFields: authoring.pendingFields, completion: authoring.completion },
