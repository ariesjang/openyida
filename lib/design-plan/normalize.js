...
const { PLAN_SKILL_ROOT, loadThemeIndex } = require('./themes');
...
function assertVisualFieldObjects(forUser, options = {}) {
  ...
  const check = (value, field, example) => {
    ...
    throw new CliError(t('cli.design_plan_visual_object_required', field, JSON.stringify(example)), {
      code: 'DESIGN_PLAN_VISUAL_FIELD_TYPE_INVALID',
      details: { path: field, expectedType: 'object', receivedType: value === null ? 'null' : Array.isArray(value) ? 'array' : typeof value,
        example, ...(options.sourcePath ? { sourcePath: options.sourcePath } : {}) },
    });
  };
