...
const { PLAN_SKILL_ROOT, loadThemeIndex, themeNavTheme } = require('./themes');
...
// Validate before normalization writes properties (a string previously caused
// a raw TypeError), and reuse this contract at fragment / init boundaries.
function assertVisualFieldObjects(forUser, options = {}) {
  const prefix = options.prefix || 'visualStyle.forUser';
  const object = value => value !== null && typeof value === 'object' && !Array.isArray(value);
  const examples = {
    visualDirection: { label: '<label>', description: '<description>' },
    navigationStyle: { structure: 'top' },
  };
  const issues = [];
  const check = (value, field, example) => {
    if (value === undefined || object(value)) { return; }
    issues.push({ path: field, expectedType: 'object', receivedType: value === null ? 'null' : Array.isArray(value) ? 'array' : typeof value,
      example, message: t('cli.design_plan_visual_object_required', field, JSON.stringify(example)) });
  };
  check(forUser, prefix, examples);
  if (object(forUser)) {
    for (const [field, example] of Object.entries(examples)) { check(forUser[field], `${prefix}.${field}`, example); }
  }
  if (issues.length) {
    throw new CliError(issues.map(issue => issue.message).join('\n'), {
      code: 'DESIGN_PLAN_VISUAL_FIELD_TYPE_INVALID',
      details: { ...issues[0], issues, ...(options.sourcePath ? { sourcePath: options.sourcePath } : {}) },
    });
  }
}

// Ordered business blocks provide one source for content, priority and coverage.
// Explicit overrides remain authoritative; incomplete blocks stay visible to validation.
function derivePageContent(page) {
  const blocks = page?.blocks;
  const filled = value => typeof value === 'string' && value.trim().length > 0;
  if (!Array.isArray(blocks) || !blocks.length || !blocks.every(block => block && typeof block === 'object'
    && !Array.isArray(block) && filled(block.name) && filled(block.purpose)
    && Object.keys(block).every(key => ['name', 'purpose'].includes(key)))) {return page;}
  const descriptions = blocks.map(block => `${block.name}：${block.purpose}`);
  return {
    ...page,
    blocks: descriptions,
    contentPriority: page.contentPriority === undefined ? blocks.map(block => block.name) : page.contentPriority,
    contentRichness: {
      ...page.contentRichness,
      contentLayers: page.contentRichness?.contentLayers === undefined ? descriptions : page.contentRichness.contentLayers,
    },
  };
}
...
function ensurePageRules(plan, pagePatternIndex) {
  ...
  for (const page of list((plan.pages || {}).customPageDetails)) {
    Object.assign(page, derivePageContent(page));
...
function assertThemeProfile(profile, theme) {
  if (profile === undefined) {return;}
  if (!profile || typeof profile !== 'object' || Array.isArray(profile)
    || Object.entries(profile).some(([key, value]) => value !== (theme.defaultProfile || {})[key])) {
    throw new CliError('themeProfile 是只读主题摘要；具体视觉调整请写 visualStyle.tokens，主色请改 colorStrategy.primaryColor', {
      code: 'DESIGN_PLAN_DERIVED_THEME_PROFILE',
    });
  }
}
...
function selectedTheme(plan, themeIndex) {
  ...
  selected.summary = selected.summary || [theme.styleSummary, selected.customText].filter(Boolean).join('；');
  assertThemeProfile(forUser.themeProfile, theme);
  forUser.themeProfile = { ...(theme.defaultProfile || {}) };
  return { ...theme, navTheme: themeNavTheme(theme) };
}

function pageApplicationDefaults(page) {
...
function ensureVisual(plan, theme) {
  ...
  const navigation = forUser.navigationStyle = forUser.navigationStyle || {};
  if (theme.mode !== 'creative') {navigation.tone = theme.navTheme;}
  navigation.toneSource = theme.mode === 'creative' ? 'project_defined' : 'theme_derived';
  ...
  forUser.pageApplications = list((plan.pages || {}).customPageDetails).map(page => {
    ...
    const application = { ...pageApplicationDefaults(page), ...source };
  ...
  forUser.assetStrategy = forUser.assetStrategy || {
    materialStatus: 'none',
    pages: list((plan.pages || {}).customPageDetails).map(page => ({
      pageId: page.pageId,
      imageNeed: 'none',
      reason: '无图片槽位。',
      slots: [],
    ...
    missingAssets: [],
    notes: '需要图片时使用 yida-image-assets。',
  };
  forUser.designMdReady = forUser.designMdReady || '页面配色、布局和组件样式将整理为完整设计说明。';
...
function normalizePlan(sourcePlan) {
  ...
  assertVisualFieldObjects(plan.visualStyle?.forUser);
  normalizeEquivalentAuthoring(plan);
  const themeIndex = loadThemeIndex();
  const pagePatternIndex = readJson(PAGE_PATTERN_INDEX_PATH, '页面模式索引');
  ensureOverview(plan);
  ensurePageRules(plan, pagePatternIndex);
  const theme = selectedTheme(plan, themeIndex);
  ensureVisual(plan, theme);
  return plan;
}

module.exports = {
  normalizePlan,
  assertVisualFieldObjects,
  derivePageContent,
  assertThemeProfile,
  ensureOverview,
};
