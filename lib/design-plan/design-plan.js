...
function usage() {
  return [
    ...
    '  openyida design-plan materialize <build-plan.json> [--from-preview | --business-file <json> --visual-file <json>] [--output-dir <dir>] [--check] [--json]',
...
const OPTION_SCHEMA = Object.freeze({
  ...
  materialize: { flags: ['--from-preview', '--check', '--json'], values: ['--business-file', '--visual-file', '--output-dir'] },
  ...
  const result = subcommand === 'init'
    ...
    : subcommand === 'preview'
      ...
      : subcommand === 'materialize'
        ? materialize(input, { outputDir, check: options.includes('--check'), fromPreview: options.includes('--from-preview'), businessFile: optionValue(options, '--business-file'), visualFile: optionValue(options, '--visual-file') })
