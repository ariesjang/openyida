  ...
  },
  {
    ...
    commands: [
      ...
      command('design-plan.materialize', ['design-plan', 'materialize'], 'design-plan materialize <build-plan.json> [--from-preview | --business-file <json> --visual-file <json>] [--output-dir <dir>] [--check] [--json]', 'help.cmd_design_plan_materialize', {
        notes: ['Validates image slots before writing artifacts. JSON assetTasks lists independent searches within each page, collection-wide concurrency, two-round limits, resume policy, output paths and resolve argv. The host starts these tasks after matching plan confirmation, alongside app/form creation; replace <appType> with the real ID for upload. This command performs no search or upload. Local asset inputs resolve against the command cwd; absolute paths survive page-task directory changes.'],
        requiresLogin: false,
        output: 'text|json',
        args: [
          { name: 'buildPlan', type: 'file', required: true, source: 'positional', position: 0, builder_options: ['--build-plan'] },
      ...
      command('design-plan.patch', ['design-plan', 'patch'], 'design-plan patch <build-plan.json> --set <path=value> [--set <path=value> ...] [--materialize] [--output-dir <dir>] [--json]', 'help.cmd_design_plan_patch', {
        notes: ['Asset progress updates to visualStyle.forUser.assetStrategy.materialStatus and missingAssets preserve the revision and existing approval; design changes invalidate approval. Use --materialize to refresh derived documents.'],
        requiresLogin: false,
        output: 'text|json',
        args: [
          { name: 'buildPlan', type: 'file', required: true, source: 'positional', position: 0, builder_options: ['--build-plan'] },
  ...
  },
  {
    ...
    commands: [
      ...
      command('compile', ['compile'], 'compile <src> [--canvas] [--json]', 'help.cmd_compile', {
        requiresLogin: false,
        output: 'text|json',
        notes: ['Canvas compilation checks theme provider mounting, theme reads before the provider is active, and module-level theme context declarations. Runtime rendering still requires browser verification.'],
      }),
  ...
  },
  {
    ...
    commands: [
      ...
      command('asset', ['asset'], 'asset <status|resolve|sources> [options]', 'help.cmd_asset', {
        notes: ['resolve --page-id <pageId> selects a page from --design or input assetStrategy; output contains only that page. Use distinct --manifest paths for concurrent page jobs. Host search follows collection_policy; resolve performs one attempt per supplied candidate.'],
        args: [
    ...
    core_workflows: {
      full_app_build: {
        ...
        optional_asset_branch: {
          ...
          dependency_policy: 'After Plan confirmation, start materialize.assetTasks alongside app and form creation. Fast derives the same tasks from design.md. Search independent slots within each page concurrently using scheduling_policy; one writer assembles each page draft. Upload when its draft and appType are ready. Other pages and layout work continue; wait for images only at their own binding and acceptance.',
        },
        artifact_generation: {
          mode: 'parallel',
          audience: 'internal',
