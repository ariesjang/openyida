  ...
  },
  {
    ...
    commands: [
      ...
      command('design-plan.init', ['design-plan', 'init'], 'design-plan init <requirement-brief.json> [--theme-id <id>] [--output-dir <dir>] [--json]', 'help.cmd_design_plan_init', {
        requiresLogin: false,
        output: 'text|json',
        notes: ['Returns preserved business facts, authoring.pendingFields with file/path/message, typed examples in authoring-context.md, and materialize.command. Complete the pending facts, review business coverage, then set ready=true. Init guidance is preliminary; materialize still validates the completed plan.'],
        args: [
      ...
      command('update-app', ['update-app'], 'update-app <appType> [--name "..."] [--theme-file <css>] [--nav-theme light|dark|white|gray] [--logo-source appIcon|customImage] [--layout side|top|l_shape] [--hide-app-nav|--show-app-nav]', 'help.cmd_update_app'),
    ...
    core_workflows: {
      full_app_build: {
        ...
        optional_asset_branch: {
          ...
          dependency_policy: 'Resolve independent pages concurrently with --page-id and distinct manifest paths; hand off each ready page immediately. Image downloads and uploads within a page run concurrently.',
        },
        artifact_generation: {
          mode: 'parallel',
          audience: 'internal',
        ...
        default_nav_order_policy: 'For application-wide custom navigation, implement PRD navigation order in the custom shell and verify app/page navigation hiding. For platform application navigation with a standalone frontend, implement the frontend menu in that page and preserve platform ordering for the workspace. For platform navigation: after the primary page is successfully published, perform exactly one navigation order operation. If the PRD names a navigation order, publish without --auto-nav-order and then call openyida nav-group order <appType> <items...>. If the PRD only gives broad groups or is missing navigation order, use openyida publish ... --auto-nav-order and do not call nav-group order or auto-order afterward. Explicit and automatic ordering are mutually exclusive; never generate per-item move loops. The fallback priority is portal/home/workbench entry > business handling > data management > business analytics > system configuration.',
        final_link_policy: 'Return exactly one user-visible application entry group, never one artifact or link card per form, process, report, schema, manifest, PRD, design file, or intermediate artifact. Include the app workbench URL when a management or unified workspace is in scope; frontend-only delivery includes only its verified frontend entry. Add the clean custom page URL only when PRD entryMode=standalone and get-form-config readback confirms isRenderNav=false. Add the admin URL only when application management is in scope and agent-capabilities application_entry_policy.entries.admin=include. Summarize other business resources in prose by default, but every resource count, seed-record count, and completed/verified claim must come from that resource\'s own successful result or readback. When the user or caller explicitly requests a resource manifest, resource UUIDs/IDs, publish status, or test-data summary, include one concise verified delivery manifest in the final prose and do not substitute link cards for it.',
