...
const COMMAND_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      command('app-entry.set', ['app-entry', 'set'], 'app-entry set <appType> [--frontend <url>] [--management <url>] [--clear-frontend] [--clear-management] [--json]', 'app_entry.desc_set'),
  ...
  },
  {
    ...
    commands: [
      ...
      command('compile', ['compile'], 'compile <src> [--canvas] [--compat] [--skip-lint] [--json]', 'help.cmd_compile', {
        requiresLogin: false,
        output: 'text|json',
        args: [
          { name: 'src', type: 'file', source: 'positional', position: 0, required: true },
        ...
        notes: ['Canvas compilation checks theme provider mounting, theme reads before the provider is active, module-level theme context declarations, fixed ConfigProvider brand colors, and known navigation destinations missing appType. Extract canvas-theme and canvas-navigation before coding. Form actions also require the maintained form-open-container fragment, hook and rendered container. Dynamic colors, resource identity and runtime rendering still require verification.'],
      }),
      command('publish', ['publish'], 'publish <src> <appType> <formUuid> [--health-check] [--force] [--canvas] [--compat] [--skip-lint] [--auto-nav-order] [--open|--no-open] [--json]', 'help.cmd_publish', {
        args: [
          { name: 'src', type: 'file', source: 'positional', position: 0, required: true },
        ...
        notes: ['Canvas publishing runs the same theme and form-open-container structure checks as compile before login or remote writes. --force does not bypass Canvas compilation checks. --health-check verifies saved content; inspect the actual page for runtime behavior.'],
      }),
