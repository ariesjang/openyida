  ...
  },
  {
    ...
    commands: [
      command('data', ['data'], 'data <query|get|create|update> <resource> ... | delete form <appType> <formUuid> --inst-id <id> --expect-form-name <name> --expect-form-type receipt --confirm [--json]', 'help.cmd_data', {
        output: 'json',
        notes: ['For create/update payloads, DateField uses a numeric Unix timestamp in milliseconds; CascadeDateField uses an array of millisecond timestamps. Convert dates using the business timezone before writing; date strings and Unix seconds are not accepted date values. --resolve-aliases resolves field names, not date formats.'],
        examples: [
  ...
  },
  {
    ...
    commands: [
      ...
      command('create-process', ['create-process'], 'create-process <appType> <formTitle> <fieldsJsonFile> <processDefinitionFile> [--replace] | create-process <appType> --formUuid <formUuid> <processDefinitionFile> [--replace]', 'help.cmd_create_process', {
        output: 'json',
        notes: ['Results include formMode=create|reuse. With --formUuid, formTitle and fieldCount are null because reuse does not read form metadata; null does not mean zero fields or a failed conversion. New-form results report the supplied title and the field count returned by creation. The same metadata semantics apply to configuration failure output; check success and verificationLevel for the process result.'],
        examples: [
