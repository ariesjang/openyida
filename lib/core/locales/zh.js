...
module.exports = {
  app_entry: {
    desc_get: '读取应用前台和业务工作台入口',
    desc_set: '登记应用访问入口（局部更新并回读）',
    usage: '用法：openyida app-entry get <appType> [--json]\n      openyida app-entry set <appType> [--frontend <url>|--frontend-page <formUuid>] [--management <url>|--management-page <formUuid>] [--clear-frontend] [--clear-management] [--json]\n仅登记已发布并验证的当前应用访问态地址；完整 URL 原样保留；--frontend-page 仅支持已上线的 display 页，--management-page 保留工作台页面 ID；页面参数生成的 URL 仍需访问验收，公开入口请传完整 URL。未指定的入口保持不变。',
    invalid_url: '请提供当前组织、当前应用的完整访问 URL（http:// 或 https://）；支持页面地址、公开地址和短链接，不支持相对路径、开发后台和临时鉴权参数。',
    invalid_response: '访问入口响应不完整，请重新读取后再决定是否重试，并确认服务端支持入口配置接口。',
    readback_failed: '访问入口保存后的回读结果不一致，请重新读取并确认；不要自动覆盖。',
    frontend_display_required: '前台页面 {0} 必须是 display 自定义页面；表单入口请传入已验证的完整 URL。',
    page_not_in_app: '页面 {0} 不属于当前应用，无法登记为访问入口；请用 list-forms 确认页面归属。',
    page_offline: '前台 custom 页 {0} 尚未上线到独立访问地址，请先发布/上线后再登记。',
  },
  design_plan: {
    rebase_required: '首次物化前请修正 business.json/visual.json 中的对应 facts，保留主计划基线。如果主计划已改变，在原 materialize 命令加 --rebase-parts，CLI 会根据保存的基线核对差异；不要手填 digest、删除草稿或重新 init。',
    baseline_missing: '未找到与片段匹配的可信原始基线。保留当前计划与 facts；从原工作区恢复 .build-plan-base.json 后重试。无法恢复时明确告知用户阻塞，不伪造摘要或自动重建。',
    rebase_stage_invalid: '方案已确认，不再合并初始化片段。请沿用当前 build-plan.json，通过 patch --materialize 调整。',
    rebase_conflict: '主计划和片段对同一字段有不同修改。按 conflicts 核对并在片段中明确采用当前值或原基线值后重试；其他 facts 保留。本次未保存文件。',
    repair_page_bindings: '按 business.json 的 customPageDetails 逐一修复 visual.json 的 pageApplications；原生表单不加入绑定。每项 visualMemoryApplications 必须为数组，无适用项用 []。保留现有草稿，按列出的字段修复后重试原命令；不要删除目录或重新 init。',
  },
  design_document: {
    update_conflict: '无法更新 {0}（{1}）：现有内容与上次生成结果冲突。请读取这一部分，将本地修改与计划源对齐后重试；本次未保存任何文件。',
    theme_css_invalid: '主题 CSS 第 {0} 行附近存在未闭合或不匹配的括号、字符串或注释，请修复后重试。',
    theme_css_unresolved: '主题 CSS 的 {0} 仍含模板占位或色值生成说明。请先完成 design.md，执行 openyida sample yida-design app-theme --design-file <design.md> --output <app-theme.css>，再上传生成文件；不能只替换 PRIMARY_COLOR。',
    invalid: '设计文档校验失败：{0}（{1}）',
    yaml: '设计文档 frontmatter 不是有效的 YAML',
    token_value: 'token {0} 必须是已确定的单行 CSS 值',
    token_conflict: 'token {0} 存在重复或冲突值',
    ...
    read_error: '无法读取设计检查文件：{0}',
    checked: '设计文档校验通过：{0}；{1} 个页面，{2} 个 token',
    anchor_hint: '锚点必须在正文相应位置写成空内容 HTML 锚点标签，例如 <a id="component-button"></a>（标签 id 不带 #，frontmatter 引用写作 #component-button）。Markdown 标题 {#id}、链接 [文字](#id) 或纯文本不算锚点。',
    anchor_chapter_three_hint: '该锚点的 <a id="…"></a> 标签必须放在「## 3. 基础组件表达」章节内、「## 4. 特色表达配方」之前。',
    page_anchor_chapter_five_hint: '页面锚点的 <a id="page-…"></a> 标签必须放在「## 5. 项目应用与调整规则」章节内。',
    page_anchor_prefix_hint: '页面锚点的标签 id 必须以 page- 开头，例如 <a id="page-workbench"></a>（frontmatter 引用写作 #page-workbench），且不同页面的锚点不得重复。',
    page_label_hint: '在本页锚点 <a id="page-…"></a> 之后、下一个页面锚点之前，逐行写出八项要点：页面任务/首屏焦点/布局/表面与组件/主操作/状态/响应式/验收，格式如 - **{0}：** 具体内容；值必须具体可实现，不能写“按主题执行”“TODO”等占位说法。',
  },
  asset: {
    sourceRecords: '在 --input 的 assets[] 中填写 assetId、creator、sourcePage、license、licenseUrl、licenseCheckedAt（YYYY-MM-DD）和 authorizationEvidence（凭证链接或文件路径数组）。清单保留这些记录；未提供的留空，不自动认定已获商用授权。',
    executionReview: '素材执行记录需复核：{0}。请检查后台派发回执、业务执行时间及同步退化原因。',
    localFileUnavailable: '找不到图片文件：{0}。请检查命令工作目录，或使用绝对路径。',
    invalidStrategy: '图片要求应填写为对象，按页面列出图片位置。',
  },
  // ── 通用 ──────────────────────────────────────────

  // ── bin/yida.js printHelp() ────────────────────────
  help: {
    cmd_check_design: '检查设计文档、主题变量与 PRD 交接',
    ...
    group_app: '应用管理',
    cmd_app_list: '分页查询我管理的或我创建的应用',
    cmd_corp_efficiency: '查询企业效能概览和明细报表',
    cmd_create_app: '创建宜搭应用',
    cmd_design_plan_preview: '按模块更新方案草稿',
    design_plan_preview_invalid: '方案草稿更新失败，请检查错误详情',
    cmd_design_plan_init: '从已确认需求初始化计划草稿',
    cmd_design_plan_catalog: '查询规划可用的主题与页面模式',
    cmd_design_plan_materialize: '生成或更新方案文档和主题，并校验一致性',
    cmd_design_plan_patch: '按字段修改方案，按需同步文档和主题',
    ...
    group_form: '表单 & 页面',
    cmd_create_form_batch: '按依赖并行创建表单',
    ...
    data_notes: 'DateField 传毫秒时间戳数字；CascadeDateField 传毫秒时间戳数组。先按业务时区转换，不接受日期字符串或秒级时间戳。--resolve-aliases 只转换字段名称。',
    ...
    create_process_notes: 'formMode=create|reuse 区分新建和复用。复用时 formTitle、fieldCount 为 null，表示未查询；新建时返回名称和字段数。成功和配置失败输出均遵守此约定，结果看 success 与 verificationLevel。--replace 仅用于已获明确授权的现有草稿或已发布流程整图替换。',
  ...
  app_list: {
    ...
    options: '选项：',
    option_type: '  --type TYPE  查询范围：managed（我管理的，默认）或 created（我创建的）',
    option_page: '  --page N     页码，默认：1',
    option_size: '  --size N     每页数量，默认：16',
    option_help: '  --help, -h   显示帮助',
    invalid_type: '无效的 --type 值“{0}”，有效值：{1}',
    invalid_positive_integer: '{0} 必须是正整数，当前值：“{1}”',
    invalid_argument: '无效参数：{0}',
    query_failed: '查询应用列表失败：{0}',
    auth_required: '登录态已失效，请重新登录',
    unknown_error: '未知错误',
    scope_managed: '我管理的',
    scope_created: '我创建的',
    found: '{0}应用：第 {1}/{2} 页，本页 {3} 个，共 {4} 个',
    next_page: '还有更多应用，请继续执行：{0}',
  },

  cli: {
    design_plan_entry_mode_required: '请明确入口方式：不分前后台（unified）、分前后台（service-management）、只有访问前台（frontend-only）或只有访问后台（backend-only）。',
    design_plan_backend_only_roles: '只有访问后台（backend-only）时，每个入口的 role 必须为 management。',
    design_plan_local_menu_binding: '本地菜单 {0} 必须关联承载页面和 viewKey：入口 {1} 的 sceneKey 对应页面 sceneKey，菜单 resource 等于页面 name，并填写非空 viewKey。此规则适用于所有入口身份。',
    design_plan_visual_object_required: '{0} 必须为对象，不能写成字符串或数组。沿用 init 生成的字段结构，只补齐值；示例：{1}。',
    help: `
openyida - 宜搭命令行工具
...
命令：
  ...
  logout                                                       退出登录 / 切换账号
  create-app --name "<名称>" 创建应用并输出 appType；主题资源与 Logo 来源使用 update-app 配置
  ...
  create-form create <appType> "<表单名>" <字段JSON> [--layout single|double|card|section] [--theme default|compact|comfortable] [--label-align top|left|right]  创建表单页面
  ...
  create-process <appType> <formTitle> <fieldsJsonFile> <processDefinitionFile> [--replace]  创建流程表单（一体化）
...
`,
    ...
    compile_usage: '用法: openyida compile <源文件路径> [--canvas] [--compat] [--skip-lint] [--strict-theme|--allow-fixed-brand] [--json]',
    ...
    publish_usage: '用法: openyida publish <源文件路径> <appType> <formUuid> [--health-check] [--force] [--canvas] [--compat] [--skip-lint] [--auto-nav-order] [--open|--no-open] [--json]',
  ...
  integration: {
    ...
    create_opt_connector_system_token_app: '  --connector-system-token-app <appType>  为宜搭 OpenAPI 在服务端绑定 systemToken',
    ...
    connector_input_ambiguous: '连接器输入字段在多个参数分组中存在，请使用完整路径：{0}',
    connector_assignment_duplicate: '多个连接器赋值指向同一输入字段：{0}',
    connector_assignment_value_required: '连接器输入字段的赋值不能为空：{0}',
    connector_required_input_missing: '连接器必填输入字段尚未赋值：{0}',
    readback_connector_assignments_mismatch: '集成自动化远端回读的连接器输入赋值与发布内容不一致。',
  ...
  // ── lib/create-app.js ──────────────────────────────
  create_app: {
    update_only_option: '{0} 仅用于更新应用设置，请先创建应用，再使用 openyida update-app <appType>。',
    title: '  openyida create-app - 宜搭应用创建工具',
    usage: '用法: openyida create-app "<appName>" [description] [icon] [iconColor] 或 openyida create-app --name "<appName>" [--desc "<description>"]',
    example: '示例: openyida create-app --name "考勤管理" --desc "员工考勤打卡系统"',
    available_icons: '\n可用图标:',
  ...
  create_page: {
    ...
    navigation_unverified: '页面已创建，但未能回读确认导航已隐藏。请使用返回的 pageId 修复配置并回读，不要重复创建页面。',
  ...
  // ── lib/create-form.js ─────────────────────────────
  create_form: {
    divider_type_invalid: '不支持的分割线样式：{0}。可选值：{1}',
    batch_invalid: '表单批量任务配置无效，请检查错误详情',
    ...
    error: '\n❌ 错误: {0}',
    usage_create: '用法: openyida create-form create <appType> <formTitle> <fieldsJsonFile> [--layout single|double|card|section] [--theme default|compact|comfortable] [--label-align top|left|right] [--icon auto|<iconName>] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]',
    ...
    usage_label: '用法:',
    usage_create_short: '  创建: openyida create-form create <appType> <formTitle> <fieldsJsonFile> [--layout single|double|card|section] [--theme default|compact|comfortable] [--label-align top|left|right] [--icon auto|<iconName>] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]',
  ...
  // ── lib/update-app.js ─────────────────────────────
  update_app: {
    theme_preset_conflict: '平台预置 colour 不能与自定义 CSS 或 themeColor 同传；请使用 colour=custom 或省略 --colour。',
    custom_theme_color_required: 'colour=custom 需要主题文件或有效 themeColor；请传 --theme-file 或 --theme-color。',
    theme_not_persisted: '应用主题保存后回读不一致或查询失败，CSS 资源尚未确认绑定。请查看 themeVerification，并使用 update-app <appType> --theme-file <css> 重试；不要重复创建应用。',
    navigation_not_persisted: '应用导航保存后回读不一致或查询失败，导航设置尚未确认生效。请查看 navigationVerification 中的期望值、实际值和查询错误，核实应用当前设置。',
    ...
    name_label: '  应用名称: {0}',
    failed: '  ❌ 更新失败: {0}',
    layout_notice: '提示：layoutDirection 由宜搭应用外壳在创建/刷新时消费；若后台切换后顶部操作栏未立即恢复，请刷新或重新打开工作台，不要重新创建应用。',
  },
  ...
  create_process: {
    invalid_argument: '参数 {0} 缺失或无效，请按以下用法修正。',
    title: '宜搭流程表单一体化创建',
    app_id: '应用 ID',
    mode: '模式',
    reuse_form: '复用已有表单',
  ...
  publish: {
    canvas_path_missing_app_type: '第 {0} 行的跳转地址缺少 appType。请提取 canvas-navigation，使用真实 appType 与页面 ID 构造 /{appType}/{pageType}/{formUuid}；不要直接跳转 /custom/ 等裸路径。',
    canvas_theme_fixed_brand: '第 {0} 行的 ConfigProvider 将品牌交互色固定为字符串。请提取 canvas-theme，使用应用主题解析值；CSS var 仅直接用于 DOM 样式，不能替代 antd 色值解析。这是本地编译期校验，此时尚未发起任何网络请求；如需临时放行请加 --allow-fixed-brand 或设置 OPENYIDA_CANVAS_ALLOW_FIXED_BRAND=1。',
    canvas_theme_fixed_brand_allowed: '已保留第 {0} 行 {1} 的固定品牌色覆盖；该色值不会跟随应用主题。',
    canvas_theme_fixed_brand_relaxed: '第 {0} 行的 ConfigProvider 写死了品牌交互色（{1}）。默认不再阻断发布，已按原样继续；但该色值不会跟随应用主题。建议加 --fix-theme 自动迁移，或手动迁移到 canvas-theme；如需在 CI 强制拦截可加 --strict-theme。',
    theme_canvas_only: '主题参数仅适用于 Canvas 页面；请使用 .canvas.jsx/.canvas.tsx 或 --canvas。',
    fix_theme_source_changed: '主题迁移前源码已被修改；未写回文件，请重新读取源码后检查。',
    fix_theme_applied: '已完成主题迁移并写回源码：{0}',
    canvas_navigation_local_platform: '第 {0} 行把页内菜单交给平台导航过滤。请使用 mode=local 或直接切换本地视图；真实页面绑定 formUuid/navUuid，声明的权限仍需校验。',
    canvas_navigation_document_flex: '第 {0} 行的导航内容仍使用工作区 flex 零基准布局，会压缩 document 长页。请重新提取 canvas-nav-content：document 使用自然高度与块布局，workspace 才使用定高 flex。',
    canvas_theme_not_assembled: '第 {0} 行的主题标记尚未装配。请运行 build-canvas-theme.js 生成独立页面并编译发布输出文件；已直接接好 Provider 的页面无需此标记。',
    canvas_theme_provider_missing: '第 {0} 行的页面未接入主题容器。请在 YidaComp 中返回 <CanvasThemeProvider><PageContent /></CanvasThemeProvider>，让业务内容读取应用主题。',
    canvas_theme_root_hook: '第 {0} 行在主题容器生效前读取了主题。请把 useCanvasThemeContext 移到 PageContent 中，并在 YidaComp 中用 CanvasThemeProvider 包住 <PageContent />。',
    canvas_theme_context_scope: '第 {0} 行的 CanvasThemeContext 定义在函数内。请将主题上下文和 Provider 放在模块顶层，由页面入口包住业务组件。',
    canvas_inline_css_invalid: '第 {0} 行附近的内嵌 CSS 存在未闭合或不匹配的括号、字符串或注释，请修复后再发布。',
    ...
    lint_form_open_container: '普通表单提交/详情必须使用完整抽屉模板。执行 openyida sample openyida-page-template form-open-container，合并 CanvasDrawer、FormOpenContainer、useYidaFormOpen，调用 openForm 并渲染 formOpenContainer。PC 使用 iframe 抽屉，移动端由模板处理；不能用普通链接或自绘弹层代替。',
    ...
    lint_searchformdata_bridge_unwrap: 'Yida searchFormDatas 的行数据位于 payload.data，总数位于 payload.totalCount。不能只读取 payload.data.data；请先归一化数组，分页使用 currentPage。参考 yida-canvas-data-binding。',
    ...
    lint_system_token_frontend_forbidden: '自定义页面不得读取或传入 systemToken；请改用宜搭集成自动化，在服务端安全绑定凭据。',
    lint_connector_runtime_name_required: '自定义页面调用连接器时必须使用 connectorName（Http_*）；数字 connectorId 只用于 CLI 管理。',
    ...
    canvas_icon_export_unavailable: '第 {2} 行：宜搭运行时图标库 {0} 未导出 {1}。请选择运行时支持的图标后重新编译；可选替代：{3}。',
    ...
    usage: '用法: openyida publish <源文件路径> <appType> <formUuid> [--health-check] [--force] [--canvas] [--compat] [--skip-lint] [--fix-theme] [--strict-theme] [--allow-fixed-brand] [--auto-nav-order] [--open|--no-open] [--json]',
...
module.exports.connector_contract = {
  ...
  test_business_failed: '连接器测试返回业务失败，错误码：{0}',
...
module.exports.cli_argument = { invalid: '参数无效或缺少值：{0}。请按命令帮助填写。' };

module.exports.sample_options = {
  'theme_help': 'app-theme：--output 指向 CSS 文件；--design-file 读取已完成的设计。省略设计文件会重置 CSS，主题值通过设计填写。',
  'style_help': 'application-style：--style-id 必填，支持目录中全部整套应用主题，不单选导航风格；design.md 统一定义五类界面的视觉 token，--output 指向目录，输出三文件，有同名文件时报错。'
};
