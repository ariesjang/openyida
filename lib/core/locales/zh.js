...
module.exports = {
  ...
  design_document: {
    ...
    anchor_hint: '锚点必须在正文相应位置写成自闭合 HTML 标签，例如 <a id="component-button"></a>（标签 id 不带 #，frontmatter 引用写作 #component-button）。Markdown 标题 {#id}、链接 [文字](#id) 或纯文本不算锚点。',
...
`,
    ...
    compile_usage: '用法: openyida compile <源文件路径> [--canvas] [--compat] [--skip-lint] [--json]',
  ...
  publish: {
    ...
    canvas_theme_fixed_brand_allowed: '第 {0} 行的 ConfigProvider 写死了品牌交互色（{1}），因已设置 --allow-fixed-brand / OPENYIDA_CANVAS_ALLOW_FIXED_BRAND 暂时放行。请尽快迁移到 canvas-theme（应用主题解析），此旁路将在后续版本移除。',
    ...
    fix_theme_detected: '检测到 {0} 处写死的品牌交互色覆盖，可自动迁移：移除这些硬编码覆盖，交由应用主题解析。',
    fix_theme_item: '  · 第 {0} 行 {1}：{2}',
    fix_theme_confirm: '是否自动迁移主题？此操作会改写你的源码文件（移除写死的品牌色覆盖）。[y/N]',
    fix_theme_applied: '已完成主题迁移并写回源码：{0}',
    fix_theme_skipped: '已跳过主题迁移，未改动源码；你可以手动迁移到 canvas-theme，或加 --allow-fixed-brand 临时放行。',
    fix_theme_flag_hint: '非交互环境未自动迁移；如需自动改写请重新执行并加 --fix-theme。',
    ...
    usage: '用法: openyida publish <源文件路径> <appType> <formUuid> [--health-check] [--force] [--canvas] [--compat] [--skip-lint] [--fix-theme] [--strict-theme] [--auto-nav-order] [--open|--no-open] [--json]',
...
module.exports.sample_options = {
  ...
  'style_help': 'application-style：--style-id 必填，支持目录中全部主题；--output 指向目录，输出三文件，有同名文件时报错。'
};
