...
module.exports = {
  app_entry: {
    ...
    usage: 'Usage: openyida app-entry get <appType> [--json]\n       openyida app-entry set <appType> [--frontend <url>|--frontend-page <formUuid>] [--management <url>|--management-page <formUuid>] [--clear-frontend] [--clear-management] [--json]\nRegister only published, verified runtime URLs for this application. Complete URLs are preserved. --frontend-page requires a published display page; --management-page retains the workbench page ID. Generated URLs still need runtime verification; pass the complete URL for public entries. Omitted entries remain unchanged.',
    invalid_url: 'Provide a complete http:// or https:// URL for a page, public page, or short link in the current application. Relative paths, designer URLs, and temporary authentication parameters are unsupported.',
    ...
    frontend_display_required: 'Frontend page {0} must be a display page; use a verified complete URL for native form entries.',
  ...
  design_plan: {
    rebase_required: 'Before first materialization, repair the matching facts in business.json/visual.json and preserve the source baseline. If the source changed, add --rebase-parts to the original materialize command to reconcile against the saved baseline. Do not invent a digest, delete drafts, or run init again.',
    baseline_missing: 'No trusted baseline matches these parts. Preserve the plan and facts; restore .build-plan-base.json from the original workspace. If unavailable, report the blocker instead of inventing a digest or rebuilding.',
    rebase_stage_invalid: 'The plan is confirmed, so initial parts are no longer merged. Keep the current build-plan.json and adjust it through patch --materialize.',
    rebase_conflict: 'The source and part changed the same field differently. Review conflicts and explicitly select the current or baseline value in the part before retrying; preserve other facts. No files were saved.',
    repair_page_bindings: 'Repair pageApplications in visual.json to match customPageDetails in business.json one to one; exclude native forms. Each visualMemoryApplications must be an array; use [] when none apply. Keep the existing draft, fix the listed fields and retry the original command. Do not delete the directory or run init again.',
  },
  design_document: {
    update_conflict: 'Cannot update {0} ({1}): the existing content differs from the last generated version. Read this section, reconcile the local edit with the plan source, and retry. No files were saved.',
    theme_css_invalid: 'Theme CSS near line {0} has an unclosed or mismatched delimiter, string, or comment. Fix it and retry.',
    theme_css_unresolved: 'Theme CSS token {0} still contains a template placeholder or color-generation instruction. Complete design.md, run openyida sample yida-design app-theme --design-file <design.md> --output <app-theme.css>, then upload the generated file. Replacing PRIMARY_COLOR alone is insufficient.',
    invalid: 'Design validation failed: {0} ({1})',
    yaml: 'Design document frontmatter is not valid YAML',
    token_value: 'Token {0} must be a resolved single-line CSS value',
    token_conflict: 'Token {0} has duplicate or conflicting values',
    tokens_required: 'design.md requires frontmatter containing tokens',
    brand_required: 'design.md is missing brand tokens: {0}',
    usage: 'Usage: openyida check-design <design.md> [--prd <prd.md>] [--base-dir <dir>] [--json]',
    read_error: 'Cannot read design validation file: {0}',
    checked: 'Design validation passed: {0}; {1} pages, {2} tokens',
    anchor_hint: 'An anchor must be written in the body as a empty HTML anchor element, e.g. <a id="component-button"></a> (the tag id has no leading #; the frontmatter reference is #component-button). Markdown heading ids, links like [text](#id), or plain text do not count as anchors.',
    anchor_chapter_three_hint: 'The <a id="…"></a> tag for this anchor must be placed inside chapter "## 3. 基础组件表达" and before "## 4. 特色表达配方".',
    page_anchor_chapter_five_hint: 'The page anchor tag <a id="page-…"></a> must be placed inside chapter "## 5. 项目应用与调整规则".',
    page_anchor_prefix_hint: 'Page anchor tag ids must start with page-, e.g. <a id="page-workbench"></a> (referenced as #page-workbench in frontmatter), and must not repeat across pages.',
    page_label_hint: 'After this page anchor <a id="page-…"></a> and before the next page anchor, write all eight labels line by line: 页面任务/首屏焦点/布局/表面与组件/主操作/状态/响应式/验收, in the form - **{0}：** specific content; values must be concrete and actionable, never placeholders like 按主题执行 or TODO.',
  },
  asset: {
    sourceRecords: 'Record assetId, creator, sourcePage, license, licenseUrl, licenseCheckedAt (YYYY-MM-DD), and authorizationEvidence (an array of evidence URLs or file paths) in assets[] via --input. The manifest preserves these records; missing values remain empty and do not establish commercial authorization.',
    executionReview: 'Review asset execution evidence: {0}. Check background dispatch receipts, business intervals and synchronous fallback reasons.',
    localFileUnavailable: 'Image file not found: {0}. Check the command working directory or use an absolute path.',
    invalidStrategy: 'Image requirements must be an object listing image slots by page.',
  },
  // ── 通用 ──────────────────────────────────────────

  // ── bin/yida.js printHelp() ────────────────────────
  help: {
    cmd_check_design: 'Validate design documents, theme tokens and PRD handoff',
    ...
    cmd_app_list: 'Page through apps I manage or created',
    cmd_corp_efficiency: 'Query enterprise efficiency overview and detail reports',
    cmd_create_app: 'Create a Yida app',
    cmd_design_plan_preview: 'Update plan drafts by module',
    design_plan_preview_invalid: 'Draft update failed; check the error details',
    cmd_design_plan_init: 'Initialize a plan draft from confirmed requirements',
    cmd_design_plan_catalog: 'List available planning themes and page patterns',
    cmd_design_plan_materialize: 'Generate or update plan documents and theme with consistency checks',
    cmd_design_plan_patch: 'Patch plan fields and optionally sync documents and theme',
    ...
    cmd_create_form_batch: 'Create forms concurrently by dependency',
    ...
    data_notes: 'DateField uses numeric Unix timestamps in milliseconds; CascadeDateField uses an array of millisecond timestamps. Convert using the business timezone; date strings and Unix seconds are not accepted. --resolve-aliases only resolves field names.',
    ...
    create_process_notes: 'formMode=create|reuse identifies new and reused forms. On reuse, formTitle and fieldCount are null (not queried); creation returns the title and field count. This also applies to configuration failure output; check success and verificationLevel. Use --replace only for explicitly authorized whole-flow replacement of an existing draft or published process.',
    ...
    cmd_report_inspect: 'Inspect report runtime bindings (read-only)',
    ...
    cmd_connector_update_action: 'Safely update action query defaults',
    ...
    cmd_asset: 'Detect asset capability / resolve materials',
  ...
  // ── bin/yida.js ────────────────────────────────────
  app_list: {
    usage: 'Usage: openyida app-list [--type managed|created] [--page N] [--size N]',
    options: 'Options:',
    option_type: '  --type TYPE  Scope: managed (default) or created',
    option_page: '  --page N     Page number, default: 1',
    option_size: '  --size N     Page size, default: 16',
    option_help: '  --help, -h   Show this help',
    invalid_type: 'Invalid --type value "{0}"; valid values: {1}',
    invalid_positive_integer: '{0} must be a positive integer; received: "{1}"',
    invalid_argument: 'Invalid argument: {0}',
    query_failed: 'Failed to query the app list: {0}',
    auth_required: 'Login has expired. Please log in again.',
    unknown_error: 'Unknown error',
    scope_managed: 'Managed',
    scope_created: 'Created',
    found: '{0} apps: page {1}/{2}, {3} on this page, {4} total',
    next_page: 'More apps are available. Continue with: {0}',
  },

  cli: {
    design_plan_entry_mode_required: 'Choose an entry mode: shared business pages (unified), separate frontend and business backend (service-management), frontend only (frontend-only), or business backend only (backend-only).',
    design_plan_backend_only_roles: 'For backend-only, every entry must use role management.',
    design_plan_local_menu_binding: 'Local menu {0} requires a host page and viewKey: entry {1} sceneKey must match the page sceneKey, menu resource must equal the page name, and viewKey must be nonempty. This applies to every entry role.',
    design_plan_visual_object_required: '{0} must be an object, not a string or array. Preserve the structure generated by init and fill in its values. Example: {1}.',
    help: `
...
Commands:
  ...
  create-app --name "<name>" Create an app and output appType; configure theme resources and Logo source with update-app
  create-page <appType> "<pageName>" [--mode dashboard] [--hide-nav] Create a custom page, output pageId
  update-app <appType> [--name "..."] [--hide-app-nav|--show-app-nav] Update app info
  create-form create <appType> "<formName>" <fieldsJSON> [--layout single|double|card|section] [--theme default|compact|comfortable] [--label-align top|left|right]  Create a form page
  ...
  check-prd-completeness <prd.md> --app-type <appType>          Check PRD page/resource count risk
  formula evaluate <formula|file> [--schema <schema.json>]      Static-check Yida formulas
  publish <sourceFile> <appType> <formUuid> [--health-check] [--auto-nav-order] Compile and publish a custom page
...
Examples:
  openyida login
  openyida logout
  openyida create-app --name "Attendance"
...
`,
    ...
    forbidden_alias_get_app: '`{0}` is ambiguous; use `{1}` for app name search, `{2}` for form/schema lookup, or `{3}` for resource context preflight.',
    ...
    integration_list_usage: 'Usage: openyida integration list <appType> [--form-uuid <uuid>] [--status y|n] [--key <kw>] [--size <n>] [--json]',
    integration_list_example: 'Example: openyida integration list APP_XXX --form-uuid FORM-XXX --json',
    integration_enable_usage: 'Usage: openyida integration enable <appType> <formUuid> <processCode>',
    integration_enable_example: 'Example: openyida integration enable APP_XXX FORM-XXX LPROC-XXX',
    integration_disable_usage: 'Usage: openyida integration disable <appType> <formUuid> <processCode>',
    integration_disable_example: 'Example: openyida integration disable APP_XXX FORM-XXX LPROC-XXX',
    compile_usage: 'Usage: openyida compile <sourceFile> [--canvas] [--compat] [--skip-lint] [--strict-theme|--allow-fixed-brand] [--json]',
    compile_example: 'Example: openyida compile pages/src/home.oyd.jsx',
    ...
    update_app_usage: 'Usage: openyida update-app <appType> [--name "New Name"] [--theme-file <css>] [--nav-theme light|dark|white|gray] [--logo-source appIcon|customImage] [--layout side|top|l_shape]',
    update_app_example: 'Example: openyida update-app APP_XXX --theme-file ./app-theme.css --nav-theme light --logo-source appIcon --layout side',
    ...
    first_run_footer1: '  Supported AI tools: Codex / Claude Code / Cursor / OpenCode / QwenWork',
  ...
  update: {
    ...
    installing: 'Installing openyida@{0}...',
    success: 'Update successful! Current version: {0}',
    install_failed: 'Update failed: {0}',
    manual_hint: 'Please try manually: npm install -g openyida@{0}',
    ...
    done_hint: 'Run openyida --version to confirm the new version',
  },
  ...
  login: {
    ...
    oauth_success_message: 'Sign-in is complete. Return to the previous window to continue.',
    oauth_closing: 'Closing this window automatically…',
    oauth_manual_close: 'This window could not close automatically. Close it manually, then return to the previous window to continue.',
    oauth_failure_title: 'Sign-in not completed',
    oauth_invalid_state: 'The sign-in request could not be verified. Return to the terminal and try again.',
    oauth_missing_code: 'No authorization information was received. Return to the terminal and try again.',
  ...
  // ── lib/create-app.js ──────────────────────────────
  create_app: {
    update_only_option: '{0} is only available in update-app. Create the app first, then use openyida update-app <appType>.',
    title: '  openyida create-app - Yida App Creation Tool',
    usage: 'Usage: openyida create-app "<appName>" [description] [icon] [iconColor] or openyida create-app --name "<appName>" [--desc "<description>"]',
    example: 'Example: openyida create-app --name "Attendance" --desc "Employee attendance system"',
    available_icons: '\nAvailable icons:',
  ...
  create_page: {
    ...
    example: 'Example: openyida create-page "APP_XXX" "Game Home" --mode dashboard; explicitly hide page nav: openyida create-page "APP_XXX" "Fullscreen Dashboard" --mode dashboard --hide-nav',
    app_id: '  App ID:    {0}',
    page_name: '  Page name: {0}',
    step_create: '\n📄 Step 2: Create Custom Page\n',
    sending: '  Sending saveFormSchemaInfo request...',
    success: '  ✅ Page created successfully!',
    step_dashboard_config: '\n🖥️  Step 3: Configure page-level hidden navigation',
    dashboard_config_ok: '  ✅ Page navigation hidden by explicit request, chromeless custom URL enabled',
    dashboard_config_failed: '  ⚠️  Page-level hidden navigation config failed: {0}',
    navigation_unverified: 'The page was created, but readback did not confirm hidden navigation. Repair and verify the returned pageId; do not create another page.',
    err_mode_invalid: 'Unsupported page mode: {0}',
    mode_hint: 'Available modes: default, dashboard. Page navigation is visible by default; pass --hide-nav or --render-nav false to hide page navigation. Use update-app --hide-app-nav to hide app navigation.',
  ...
  get_schema: {
    ...
    usage: 'Usage: openyida get-schema <appType> <formUuid> or openyida get-schema <appType> --all [--output-dir <dir>]',
  ...
  // ── lib/create-form.js ─────────────────────────────
  create_form: {
    divider_type_invalid: 'Unsupported divider style: {0}. Supported values: {1}',
    batch_invalid: 'Invalid form batch; check the error details',
    ...
    schema_got_fields: '  ✅ Schema retrieved, field count: {0}',
    ...
    error: '\n❌ Error: {0}',
    usage_create: 'Usage: openyida create-form create <appType> <formTitle> <fieldsJsonFile> [--layout single|double|card|section] [--theme default|compact|comfortable] [--label-align top|left|right] [--icon auto|<iconName>] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]',
    ...
    usage_label: 'Usage:',
    usage_create_short: '  create: openyida create-form create <appType> <formTitle> <fieldsJsonFile> [--layout single|double|card|section] [--theme default|compact|comfortable] [--label-align top|left|right] [--icon auto|<iconName>] [--locale zh_CN|en_US|ja_JP] [--open|--no-open]',
    ...
    data_exists_force_hint: 'Usage: openyida create-form update <appType> <formUuid> <changesJson> --force',
  ...
  save_permission: {
    ...
    package_uuid_create_conflict: '--package-uuid only selects an existing group and cannot be used with --create',
    package_uuid_not_found: 'No permission group has packageUuid={0}; aborted with zero writes',
    before_unknown: 'The complete pre-write permission state could not be read; aborted with zero writes: {0}',
    verify_failed: 'Exact permission readback differs after the write (packageUuid={0}); success will not be reported',
    verify_unknown: 'Permission write outcome is unknown (packageUuid={0}); query it exactly before any retry',
  },
  ...
  save_share_config: {
    ...
    open_auth_hint: '  openAuth: y=require auth, n=no auth required; omit to preserve the complete current auth config',
    ...
    open_auth_preserve: 'preserve current value',
    ...
    current_state_incomplete: 'Save aborted: current openPageAuthConfig is missing or unparsable, so safe preservation cannot be proven',
  },

  // ── lib/update-app.js ─────────────────────────────
  update_app: {
    theme_preset_conflict: 'A preset colour cannot be combined with custom CSS or themeColor. Use --colour custom or omit --colour.',
    custom_theme_color_required: 'colour=custom requires a theme file or a valid themeColor. Pass --theme-file or --theme-color.',
    theme_not_persisted: 'App theme readback failed or did not match the saved settings. The CSS resource binding is unverified. Inspect themeVerification and retry update-app <appType> --theme-file <css>; do not refresh the app.',
    navigation_not_persisted: 'App navigation readback failed or did not match the requested settings. Navigation changes remain unverified. Inspect the expected values, actual values and readback error in navigationVerification, then check the current app settings.',
    usage: 'openyida update-app <appType> [--name "New Name"] [--theme-file ./theme.css] [--nav-theme light|dark|white|gray] [--logo-source appIcon|customImage] [--layout side|top|l_shape]',
    example: 'openyida update-app APP_XXX --theme-file ./app-theme.css --nav-theme light --logo-source appIcon --layout side',
    options: 'Options:\n  --name, -n      App name (specify at least one update field)\n  --desc, -d      App description\n  --icon          Icon name (e.g. xian-yingyong)\n  --icon-color    Icon color (default: #0089FF)\n  --colour, --theme  podBlue | podGreen | podOrange | deepBlue | teal | black | custom ...\n  --theme-color   Update the app primary color; the CSS primary color takes precedence when a theme file is supplied\n  --theme-file    Custom app theme CSS file (up to 20 MB; must declare --color-brand1-6)\n  --nav-theme     Navigation theme: light, dark, white, or gray\n  --logo-source   Navigation logo source: appIcon or customImage\n  --layout        Modern navigation layout: side, top, or l_shape\n  --hide-app-nav  Hide app navigation in visit mode, only for custom pages with self-rendered top/side navigation\n  --show-app-nav  Restore app navigation in visit mode',
    missing_app_type: 'Error: missing appType argument',
    missing_update_field: 'Error: at least one update field must be specified (--name, --desc, --icon, --theme, --theme-color, --theme-file, --nav-theme, --logo-source, --layout, --hide-app-nav, --show-app-nav)',
    ...
    layout_notice: 'Note: layoutDirection is consumed by the Yida app shell during creation/refresh. If the top action bar does not recover immediately after a backend switch, reopen the workbench or refresh the app with the target layout.',
  },
  ...
  configure_process: {
    replace_required: 'A published process or saved draft exists. Full replacement requires explicit --replace; no remote write was sent.',
    ownership_unverified: 'The processCode could not be proven to belong to this form. No remote write was sent.',
    published_unverified: 'Publish returned success, but the exact PUBLISHED platform view could not be fully verified. Read back platform state before any retry.',
  },
  ...
  copy: {
    ...
    no_ai_tool: '\n❌ No active AI tool environment detected\n   Supported tools: Codex, OpenCode, Claude Code, Cursor, QwenWork, QoderWork, Qoder\n\n   Current detection results:',
    no_active_tool: '\n❌ No active AI tool environment detected',
    supported_tools: '   Supported tools: Codex, OpenCode, Claude Code, Cursor, QwenWork, QoderWork, Qoder',
    current_result: '\n   Current detection results:',
  ...
  generate_page: {
    ...
    unknown_template: 'Unknown page sample: {0}',
    available_templates: 'Available samples: {0}',
    template_not_found: 'Sample file not found: {0}',
  ...
  publish: {
    canvas_path_missing_app_type: 'Line {0}: navigation is missing appType. Use the canvas-navigation sample and verified IDs to build /{appType}/{pageType}/{formUuid}, not a bare /custom/ route.',
    canvas_theme_fixed_brand: 'Line {0}: ConfigProvider fixes a brand interaction color to a string. Use canvas-theme and resolved application colors; CSS var belongs in DOM styles, not unresolved antd color tokens. This is a local compile-time check and no network request has been made yet; pass --allow-fixed-brand or set OPENYIDA_CANVAS_ALLOW_FIXED_BRAND=1 to bypass temporarily.',
    canvas_theme_fixed_brand_allowed: 'Preserved the fixed brand override at line {0} ({1}); this value does not follow the application theme.',
    canvas_theme_fixed_brand_relaxed: 'Line {0}: ConfigProvider hardcodes a brand interaction color ({1}). Publishing is no longer blocked by default and continues as-is, but this color will not follow the application theme. Add --fix-theme to migrate automatically, or migrate to canvas-theme manually; use --strict-theme to hard-fail in CI.',
    theme_canvas_only: 'Theme options apply only to Canvas pages; use .canvas.jsx/.canvas.tsx or --canvas.',
    fix_theme_source_changed: 'The source changed before theme migration was saved. No file was written; read the current source and check again.',
    fix_theme_applied: 'Theme migration applied and written back to source: {0}',
    canvas_navigation_local_platform: 'Line {0} filters local views through platform navigation. Use mode=local or local view state; bind real pages to formUuid/navUuid and continue checking declared access.',
    canvas_navigation_document_flex: 'Line {0} uses a zero-basis workspace flex layout for document content. Refresh canvas-nav-content: document pages need natural height and block flow; fixed-height flex belongs to workspace.',
    canvas_theme_not_assembled: 'The theme marker on line {0} is not assembled. Run build-canvas-theme.js and compile/publish its separate output file. Pages with an integrated Provider do not need this marker.',
    canvas_theme_provider_missing: 'The page near line {0} has no mounted theme provider. Return <CanvasThemeProvider><PageContent /></CanvasThemeProvider> from YidaComp so the content can read the application theme.',
    canvas_theme_root_hook: 'Theme is read before its provider is active near line {0}. Move useCanvasThemeContext into PageContent and wrap <PageContent /> with CanvasThemeProvider in YidaComp.',
    canvas_theme_context_scope: 'CanvasThemeContext is declared inside a function near line {0}. Declare the context and provider at module scope and wrap the business component at the page entry.',
    canvas_inline_css_invalid: 'Inline CSS near line {0} has an unclosed or mismatched delimiter, string, or comment. Fix it before publishing.',
    ...
    lint_jsx_text_identifier: 'JSX copy cannot be written as {{0}}; it is treated as a variable and causes {0} is not defined. Use plain text {0} or a quoted string {\'{0}\'} instead.',
    ...
    lint_native_select_ui: 'Found a native <select>. In normal custom pages, use a button+menu+option custom dropdown with an arrow, selected marker, and focus reset; do not render a form SelectField as a page filter component.',
    ...
    lint_form_open_container: 'Form submission/detail entries require the complete drawer template. Run openyida sample openyida-page-template form-open-container; integrate CanvasDrawer, FormOpenContainer and useYidaFormOpen, call openForm and render formOpenContainer. Use the iframe drawer on desktop and template handling on mobile; do not substitute links or custom overlays.',
    ...
    lint_searchformdata_http_post: 'A direct searchFormDatas.json call must use GET + query params (put formUuid/appType in the URL query). Using POST with formUuid in the body triggers "参数校验失败formUuid" and dashboards/lists render all zeros',
    lint_searchformdata_http_pagenumber: 'searchFormDatas.json paginates with currentPage (not pageNumber); using pageNumber breaks pagination',
    lint_searchformdata_http_query_params: 'The direct searchFormDatas.json URL query is missing required params: {0}. Use URLSearchParams with appType, formUuid, currentPage, pageSize, and searchFieldJson',
    lint_searchformdata_http_csrf: 'A direct searchFormDatas.json call must put the runtime CSRF token in both the _csrf_token URL query and the global_csrf_token request header',
    lint_searchformdata_http_credentials: 'A direct searchFormDatas.json call must set credentials: "include" so the browser sends same-origin login cookies',
    lint_searchformdata_bridge_unwrap: 'Yida searchFormDatas returns rows in payload.data and the count in payload.totalCount. Do not read only payload.data.data; normalize array data before rendering, and use currentPage for pagination. See yida-canvas-data-binding.',
    lint_searchformdata_http_unwrap: 'The browser searchFormDatas.json response nests the list under content.data (returns { content: { data: [...] } }); reading json.data only yields 0 rows — unwrap via (json.content && json.content.data)',
    lint_searchformdata_dynamic_order_metadata: 'searchFormDatas.dynamicOrder cannot use record metadata field {0}; use a real business field ID returned by get-schema. If no sortable business date field exists, remove dynamicOrder and only sort the fetched page by row.createTime for display',
    lint_canvas_yida_api_bridge_missing: 'YidaCodeCanvas form data reads must consume window.__OPENYIDA_YIDA_API__ first (the publish layer injects the this.utils.yida bridge from outer didMount); do not default to handwritten internal searchFormDatas fetch',
    lint_setstate_non_timestamp: 'this.setState writes a non-timestamp field. Custom page business state should live in _customState and be updated via forceUpdate()/setCustomState(); this.setState should only carry the timestamp contract field',
    lint_self_binding_missing: 'renderJsx uses this without declaring var self = this;. Add var self = this; at the top of renderJsx and use self inside closures/callbacks to avoid losing this',
    lint_echarts_dom_ready: 'echarts.init is not wrapped in setTimeout; the container may not be mounted yet so the chart may not render. Use setTimeout(function(){ /* echarts.init */ }, 300)',
    ...
    lint_system_token_frontend_forbidden: 'Custom pages must not read or pass systemToken. Use Yida Integration Automation to bind it on the server side.',
    ...
    step_canvas_compile: '\n📦 Step 1: Compile custom page source\n',
    canvas_compiling: '  🎨 Compiling custom page source locally...',
    canvas_compile_done: '  ✅ Custom page compiled!',
    canvas_compile_failed: '  ❌ Custom page compile failed: {0}',
    canvas_icon_export_unavailable: 'Line {2}: the Yida runtime icon library {0} does not export {1}. Choose a supported runtime icon and recompile. Suggested alternatives: {3}.',
    ...
    usage: 'Usage: openyida publish <sourceFile> <appType> <formUuid> [--health-check] [--force] [--canvas] [--compat] [--skip-lint] [--fix-theme] [--strict-theme] [--allow-fixed-brand] [--auto-nav-order] [--open|--no-open] [--json]',
    example: 'Example: openyida publish pages/src/xxx.js APP_XXX FORM-XXX --health-check --auto-nav-order',
  },
  ...
  postinstall: {
    ...
    ai_mode_desc: '  In Codex / Claude Code / Cursor and other AI tools, just chat:',
...
Options:
  ...
  create_process: {
    invalid_argument: 'Missing or invalid argument {0}. Correct it using the syntax below.',
  ...
  report_runtime: require('../../report/i18n-messages').en,
  safe_json: {
  ...
  codex_login: {
    ...
    cache_available: 'Existing CLI token cache is available; no login needed.',
  },
};
...
Object.assign(module.exports.permission_common || (module.exports.permission_common = {}), {
  unsupported_argument: 'Unsupported argument: {0}',
});

Object.assign(module.exports.permission_list || (module.exports.permission_list = {}), {
  query_failed: 'Failed to query permission groups',
  query_success: '  ✅ Permission query succeeded: {0} groups',
  query_success_message: 'Permission query succeeded',
  invalid_structure: 'Permission-group query returned an unrecognized list structure',
  duplicate_uuid: 'Permission-group pagination repeated packageUuid={0}',
  pagination_limit: 'Permission-group query reached the safe pagination limit ({0} pages); completeness cannot be proven',
});

Object.assign(module.exports.save_permission || (module.exports.save_permission = {}), {
  write_accepted_readback: '    ✅ Write accepted; starting exact readback',
  saved_message: 'Permission configuration saved',
});
Object.assign(module.exports.process_errors || (module.exports.process_errors = {}), {
  platform_view_json_invalid: 'getProcessById content must be valid JSON: {0}',
  platform_view_request_failed: 'getProcessById did not return a successful response.',
  platform_view_content_invalid: 'getProcessById content must resolve to an object.',
  platform_view_schema_missing: 'The platform view must contain a CanvasEngine schema.',
  platform_view_nodes_missing: 'Platform view schema.children must be an array.',
  multi_approval_mode_invalid: 'Multi-approval mode must be all, or, or oneByOne: {0}',
  form_mode_operation_failed: 'Form mode operation failed.',
});

Object.assign(module.exports.process_diagnostics || (module.exports.process_diagnostics = {}), {
  preflight_form_mode: 'Verify appType, formUuid, and the process binding before retrying.',
  authorize_replacement: 'Show the complete replacement summary and obtain explicit confirmation before passing --replace.',
  verify_published_view: 'Publishing may have succeeded. Read back the exact PUBLISHED version and platform view before attempting any write again.',
  unknown_result: 'The write result is unknown. Use only read-only platform checks or manual verification; do not retry the write directly.',
});
...
module.exports.connector_test = {
  ...
  ambiguous_flat_param: 'Parameter {0} belongs to multiple action locations; use structured JSON options',
  auth_account_required: 'This connector requires auth; create an account and pass its owned ID with --account-id',
  auth_account_not_owned: 'Account {0} does not belong to this connector',
  arguments_required: '--connector-id and --action are required',
  connector_not_found: 'Connector ID not found: {0}',
  operations_invalid: 'Connector operations are not valid JSON',
  action_not_found: 'Action not found: {0}',
  success: '✅ Test succeeded',
  status_label: 'HTTP status:',
  headers_label: 'Response headers:',
  content_label: 'Response body:',
};

module.exports.connector_auth = {
  create_rejects_credentials: 'connector create does not accept App Key or App Secret. Create the connector first, then let the user configure an account in Yida or enter secrets locally with --interactive.',
  create_credentials_next_step: 'Remove credential flags while creating the connector, then run openyida connector create-connection <connector-id> <account-name> --interactive.',
  tty_required: '--interactive requires an interactive TTY. No remote write was issued.',
  tty_next_step: 'Ask the user to run the command in their local terminal, or open the connector detail page to configure the auth account.',
  input_cancelled: 'Credential input was cancelled. No remote write was issued.',
  interactive_conflict: '--interactive cannot be combined with --app-key or --app-secret. No remote write was issued.',
  interactive_auth_unsupported: '--interactive currently supports only DingAuth and DingTrustGW; current auth type: {0}.',
  prompt_app_key: 'App Key (hidden): ',
  prompt_app_secret: 'App Secret (hidden): ',
  input_empty: 'App Key and App Secret must both be non-empty. No remote write was issued.',
};

module.exports.connector_contract = {
  invalid_json: 'Connector definition contains invalid JSON.',
  readback_mismatch: 'Connector readback does not match the submitted definition.',
  security_schemes_invalid: 'Connector securitySchemes must be a JSON object.',
  test_envelope_failed: 'Connector test envelope reported failure.',
  test_response_invalid: 'Connector test returned an unrecognized response contract.',
  test_business_failed: 'Connector test returned a business failure with error code: {0}',
  test_status_invalid: 'Connector test response is missing a canonical HTTP status line.',
  test_http_failed: 'Connector test failed with {0}.',
};

module.exports.connector_api = {
  connector_pagination_limit: 'Connector list exceeded the safe pagination limit.',
  write_identity_missing: 'Connector write succeeded without a recoverable identity.',
  connection_pagination_limit: 'Connection list exceeded the safe pagination limit.',
  connection_name_exists: 'A connection with the requested owned name already exists.',
  connection_readback_mismatch: 'Connection creation could not be recovered by an exact owned readback.',
  save_failed: 'Failed to save the connector.',
};

module.exports.connector_action_update = {
  preflight_incomplete: 'The platform connector detail or complete action collection is incomplete; no write was issued.',
  operation_id_duplicate: 'The action collection contains a duplicate stable ID: {0}.',
  operation_id_conflict: 'Action ID {0} already exists; add-action cannot overwrite it. Use update-action.',
  query_invalid: 'The query update must be a non-empty object whose values are non-empty scalars.',
  target_not_unique: 'The target operationId is missing, absent, or not unique.',
  query_schema_incomplete: 'The target query parameter is incomplete or non-unique across inputs and parameters.',
  query_unknown: 'The target action does not declare query parameter {0}.',
  readback_mismatch: 'Action update readback mismatched the connector configuration, action collection, or stable IDs.',
  connector_not_found: 'A unique target connector was not found.',
  no_changes: 'The query update already matches the platform state; no remote write was issued.',
  write_outcome_unknown: 'The remote outcome is unknown after the action update request; it will not be retried automatically. Inspect residual state manually.',
};

module.exports.connector_update_action = {
  usage: 'Usage: openyida connector update-action --connector-id <id> --action <operationId> --query-json <JSON> --confirm [--json]',
  invalid_arguments: 'connector-id, action, a non-empty query-json, and --confirm are required; unknown arguments are rejected.',
  query_json_invalid: '--query-json must be a non-empty JSON object.',
  success: '✅ Action {0} was safely updated and read back exactly; action count: {1}',
};

module.exports.connector_action_e2e = {
  skip_missing: 'Skipping connector action-update real E2E; missing: {0}',
  resource_plan: 'Connector action-update E2E resource plan: {0}',
  response_invalid: 'The controlled fixture did not return a proven 2xx application/json contract.',
  count_mismatch: 'The controlled fixture data count did not match the expectation.',
  current_page_mismatch: 'The controlled fixture did not echo the expected currentPage exactly.',
  cli_contract_invalid: 'A connector E2E command did not return a successful JSON contract.',
  action_invalid: 'The target action lacks a complete and unique query/header contract.',
  prewrite_missing: 'Manifest, registry, or artifact evidence is incomplete before the first remote write.',
  state_incomplete: 'Connector detail, the complete action collection, or NONE-auth account ownership readback is incomplete.',
  update_unverified: 'The action update did not return exact readback evidence with an unchanged connector fingerprint.',
  create_unverified: 'The test connector creation did not return exact owned readback evidence.',
  restore_mismatch: 'The final action collection was not restored exactly to baseline.',
  failed: 'Connector action-update real E2E failed; redacted evidence was preserved and no automatic retry will occur.',
};

module.exports.connector_e2e = {
  skip_missing: 'Skipping connector real E2E; missing: {0}',
  resource_plan: 'Redacted connector E2E resource plan: {0}',
  probe_required: 'PLATFORM_PROBE_REQUIRED: {0}; no remote writes were issued.',
  result_contract_invalid: '{0} did not return a successful JSON contract.',
  step_login_check: 'Login check',
  step_connector_create: 'Connector creation',
  step_connection_create: 'Connection creation',
  step_action_readback_before: 'Action readback before testing',
  step_connector_test: 'Connector test',
  step_action_readback_after: 'Action readback after testing',
  auth_selection_unverified: 'The selected auth profile could not be proven to belong to the target organization.',
  prewrite_evidence_missing: 'The pre-write evidence checkpoint is missing.',
  connector_identity_unverified: 'Connector creation did not return a verified owned identity.',
  connection_identity_unverified: 'Connection creation did not return a verified owned identity.',
  action_readback_mismatch: 'Connector action readback did not match the deterministic fixture before testing.',
...
module.exports.cli_argument = { invalid: 'Invalid or missing argument value: {0}. Follow the command help.' };

module.exports.sample_options = {
  'theme_help': 'app-theme: --output is a CSS file; --design-file reads a completed design. Omitting it resets CSS. Set theme values in the design.',
  'style_help': 'application-style: --style-id is required and accepts every complete application theme, never a separate navigation style. design.md owns visual tokens for navigation, shell, custom pages, forms and details; --output is a directory for three files. Existing bundle files cause an error.'
};
