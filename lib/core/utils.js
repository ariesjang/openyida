...
const AGENT_BROWSER_TOOLS = new Set(['codex', 'qwenwork', 'qoderwork', 'qoder']);
...
function qoderBundleId(env) {
  return normalizeSignal(env.__CFBundleIdentifier);
}

function hasNewQoderAppSignal(env) {
  return qoderBundleId(env) === 'com.qoder.app' || (
    normalizeSignal(env.QODER_PRODUCT_ID) === 'qoder' &&
    normalizeSignal(env.QODER_SESSION_TYPE) === 'app'
  );
}

function hasQoderIdeSignal(env) {
  return qoderBundleId(env) === 'com.qoder.ide';
}

function hasQoderWorkSignal(env) {
  return normalizeSignal(env.QODERCLI_INTEGRATION_MODE) === 'qoder_work' ||
    qoderBundleId(env) === 'com.qoder.work';
}
...
function detectRuntimeCapabilities(options = {}) {
  ...
  // 新 Qoder 桌面应用。产品 ID + session type 是 Windows/Linux 的跨平台信号，
  // macOS Bundle ID 作为宿主进程的强信号。
  if (hasNewQoderAppSignal(env)) {
    return buildRuntimeTool({
      tool: 'qoder',
      displayName: 'Qoder',
      dirName: '.qoder',
  ...
  // 原 Qoder 桌面开发产品已更名为 Qoder IDE。精确 Bundle ID 必须优先于
  // QoderWork 的旧兼容变量，避免真实 IDE 环境被归入 .qoderwork。
  if (hasQoderIdeSignal(env)) {
    return buildRuntimeTool({
      tool: 'qoder',
  ...
  // QoderWork 是独立产品，必须使用精确信号。它可能继承 QODER_IDE / CLAUDE_CODE
  // 兼容变量，因此在这些通用信号之前检测。
  if (hasQoderWorkSignal(env)) {
    return buildRuntimeTool({
      tool: 'qoderwork',
  ...
  // Qoder IDE / Qoder Agent（无宿主 Bundle ID 的 CLI 集成模式）
...
function buildSkillsDiagnostics(options = {}) {
  ...
  if (activeTool && activeTool.dirName) {
    addCandidate(
...
async function fetchWithTimeout(requestUrl, requestOptions, timeout, readResponse) {
...
async function httpPostMultipart(baseUrl, requestPath, fields, files, optionsOrLegacyCookies, maybeOptions) {
  ...
  return fetchWithTimeout(requestUrl, {
    ...
    body: form,
  }, optionsOverride.timeout, response => parseJsonFetchResponse(response, optionsOverride));
}
