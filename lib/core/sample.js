...
const SAMPLE_HINTS = {
  ...
  'openyida-page-template/canvas-theme': '以 CanvasThemeProvider 包住业务组件；Tabs 用于内容切换，Segmented/Radio 用于筛选，Button 用于动作。默认轻底选中态与可读文字，controls 可供自绘控件复用；避免内层固定色覆盖。',
  ...
  'openyida-page-template/form-open-container': '将 import、抽屉组件和辅助函数合并到现有 Canvas 页面；表单使用 useYidaFormOpen，其他内容使用 CanvasDrawer；抽屉默认使用 --pod-shell-theme-bg-color，正文容器透明承接。',
...
async function run(args) {
  ...
  chalkHint(designFile ? '已按 design.md 应用 token；使用 --theme-file 在应用级配置主题。' :
    (isNavSample ? '平台导航页默认只实现业务内容，不合并第二套应用菜单；页内 Tab 仅切当前任务的分类或状态。确认独立入口或应用级自绘导航后再使用导航壳片段。' : '') + (SAMPLE_HINTS[sampleKey] || (isNavSample ? '已输出所选导航组件；合并 import 后接入 items、activeKey、onSelect 和现有页面内容，样式直接消费应用 token。' :
