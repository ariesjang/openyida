...
const SAMPLE_HINTS = {
  ...
  'openyida-page-template/form-open-container': '将 import、CanvasDrawer、FormOpenContainer、useYidaFormOpen 和辅助函数合并到现有 Canvas 页面；表单按钮调用 openForm(request)，页面必须渲染返回的 formOpenContainer；保留 iframe、标题栏操作、调宽和关闭刷新。普通业务内容使用 CanvasDrawer；抽屉背景使用 --pod-page-bg-color，文字使用 --color-text1-4，background 可按设计覆盖。',
...
function printSampleList() {
  ...
  console.log('  Application style: openyida sample yida-design application-style --style-id <themeId|free-creative> --output <directory>');
}
...
function parseOptions(rest, defaultOutputPath) {
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    if (arg === '--style-id') {
      styleId = rest[++i];
      if (!styleId || styleId.startsWith('--')) {throw new CliError(t('design_document.invalid', '--style-id', 'APPLICATION_STYLE_REQUIRED'), { code: 'APPLICATION_STYLE_REQUIRED' });}
      continue;
    }
  ...
  return { outputPath, variables, designFile, styleId };
}
...
async function run(args) {
  ...
  if (skill === 'yida-design' && name === 'application-style') {
    const options = parseOptions(rest, path.join(process.cwd(), '.cache/samples/application-style'));
    if (options.designFile || Object.keys(options.variables).length) {
      throw new CliError(t('design_document.invalid', 'application-style', 'APPLICATION_STYLE_SAMPLE_INVALID'), { code: 'APPLICATION_STYLE_SAMPLE_INVALID' });
    }
    const result = require('../app/application-style').exportApplicationStyle(options.styleId, options.outputPath);
    console.log(JSON.stringify(result, null, 2));
    return result;
  }
  if (rest.includes('--style-id')) {throw new CliError(t('design_document.invalid', '--style-id', 'APPLICATION_STYLE_SAMPLE_INVALID'), { code: 'APPLICATION_STYLE_SAMPLE_INVALID' });}
