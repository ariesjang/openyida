...
const { buildHostAssetCapabilities } = require('../asset/host-capabilities');
...
function buildAssetCapabilities(runtime, env = process.env) {
  return buildHostAssetCapabilities({ runtime, env });
}
...
function buildBuilderPath(login, projectRoot, manifest, env = process.env, runtimeSnapshot = null) {
  ...
  return {
    ...
    resource_context_resolution: {
      ...
      app_name_search: {
        command_id: 'app-list',
        usage: 'openyida app-list [--type managed|created] [--page N] [--size N]',
      },
...
function buildFullAppArtifactRoute(manifest) {
  ...
  return {
    ...
    visual_decision_policy: workflow.visual_decision_policy,
    ...
    form_entry_policy: workflow.form_entry_policy,
...
function buildIdentity(manifestDigest, env = process.env) {
  ...
  return {
    ...
    build_id: value('OPENYIDA_BUILD_ID', version),
    commit_sha: value('OPENYIDA_COMMIT_SHA', 'unknown'),
    bundle_digest: value('OPENYIDA_BUNDLE_DIGEST', 'unknown'),
...
function buildAgentCapabilitiesSummary() {
  ...
  const manifestDigest = buildCommandManifestDigest(manifest);
  ...
  const assetCapabilities = buildAssetCapabilities(runtime);
  ...
  return {
    ...
    build_identity: buildIdentity(manifestDigest),
    command_manifest_digest: manifestDigest,
...
function buildAgentCapabilities() {
  ...
  const applicationEntryPolicy = buildApplicationEntryPolicy(builderPath, process.env);
  const manifestDigest = buildCommandManifestDigest(manifest);
  const buildIdentitySnapshot = buildIdentity(manifestDigest);
